					...
					describe(testName, function () {
						...
						beforeAll(() => {
							...
							optionsArr.forEach((options, idx) => {
								...
								// CHANGE: use rustbolt internal swc minimizer
								...
								// CHANGE: rustbolt does not support `snapshot.managedPaths` yet
						...
						if (config.cache) {
							it(
								`${testName} should pre-compile to fill disk cache (1st)`,
									...
									require("@rustbolt/core").rustbolt(options, err => {
										deprecationTracker();
							...
							it(
								`${testName} should pre-compile to fill disk cache (2nd)`,
									...
									infraStructureLog.length = 0;
									const deprecationTracker = deprecationTracking.start();
									require("@rustbolt/core")(options, (err, stats) => {
										deprecationTracker();
										if (err) return handleFatalError(err, done);
										if (!stats.hasErrors()) {
											const infrastructureLogging = stderr.toString();
						...
						it(
							`${testName} should compile`,
								...
								if (config.cache) {
									try {
										const compiler = require("@rustbolt/core")(options);
										compiler.run(err => {
											if (err) return handleFatalError(err, done);
									...
									require("@rustbolt/core")(options, onCompiled);
								}
							},
							TIMEOUT
						);
