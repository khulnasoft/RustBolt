...
const { Stats } = require("@rustbolt/core");
...
describe("Compiler", () => {
	...
	function compile(entry, options, callback, done = () => {}) {
		...
		c.outputFileSystem = {
			...
			// accommodating Rustbolt's requirement that differs from webpack's usage
	...
	it("should compile a single file to deep output", done => {
		compile(
			...
			(stats, files) => {
				// CHANGE: Rustbolt utilizes the `recursive = true` option for `mkdir`, creating nested directories as needed
	...
	describe("methods", () => {
		...
		beforeEach(() => {
			...
			compiler = webpack({
				...
				output: {
					path: "/directory"
					// CHANGE: The pathinfo is currently not supported in rustbolt
					// pathinfo: true
				}
			});
		});
	...
	it("should bubble up errors when wrapped in a promise and bail is true", async () => {
		try {
			...
			expect(err.toString()).toMatch(
				// CHANGE: Error messages from Rustbolt differ from those in webpack
	...
	// CHANGE: skip with custom test name for tracking alignment status
	// CHANGE: skip as rustbolt does not currently emit correct error code
