	...
	tests.forEach(testName => {
		it("should print correct stats for " + testName, done => {
			...
			(Array.isArray(options) ? options : [options]).forEach(options => {
				// CHANGE: rustbolt does not enable AMD by default
				...
				if (!options.experiments.rustboltFuture) options.experiments.rustboltFuture = {};
				if (!options.experiments.rustboltFuture.bundlerInfo) options.experiments.rustboltFuture.bundlerInfo = {};
				if (options.experiments.rustboltFuture.bundlerInfo.force === undefined) options.experiments.rustboltFuture.bundlerInfo.force = false;
			});
			...
			compilers.forEach(c => {
				...
				// CHANGE: The checkConstraints() function is currently not implemented in rustbolt
			...
			c.run((err, stats) => {
				...
				if (!hasColorSetting) {
					...
					actual = actual
						.replace(/\u001b\[[0-9;]*m/g, "")
						// CHANGE: The time unit display in Rustbolt is second
