...
for (const binding of bindings) {
	...
	assert(
		...
		`Binding is not matched with triple (expected: rustbolt.${platformArchABI}.node, got: ${file})`
	);
	...
	const coreJson = require(
		path.resolve(__dirname, "../packages/rustbolt/package.json")
	);
	const pkgJson = {};
	pkgJson.name = `@rustbolt/binding-${platformArchABI}`;
	pkgJson.version = coreJson.version;
	pkgJson.license = coreJson.license;
	pkgJson.description = "Node binding for rustbolt";
	pkgJson.main = `rustbolt.${platformArchABI}.node`;
