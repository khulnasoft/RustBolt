...
const RUSTBOLT_TEST = "packages/rustbolt/tests";
...
async function main() {
	const [identical, difference] = await recursiveCompare(
		path.join(WORKSPACE_ROOT, RUSTBOLT_TEST),
	...
	if (difference.size > 0) {
		...
		if (retained.length > 0) {
			errored = true;
			console.log(
				"1. Mixed test cases: cases below share the same filename with different content on rustbolt and webpack side.\n"
			);
			console.log(toSet(retained.sort()));
			console.log(
				help(`Due to the historical fact that rustbolt mixed webpack tests and rustbolt tests together, 
...
This would cause misunderstandings between those tests. This file can be removed after the old tests are no longer coupled with webpack tests.`)
	...
	// 2. Calculate cases that can be safely removed from cases in rustbolt.
	{
		...
		const result = zip(
			...
			await Promise.all(
				maybeIdentical.map(testPath =>
					recursiveCompareStrict(
						path.join(WORKSPACE_ROOT, RUSTBOLT_TEST, testPath),
		...
		if (result.length) {
			console.log(
				"2. Identical test cases: cases below share the identical structure and content in both rustbolt and webpack.\n"
			);
			...
			console.log(
				suggestion(
					"Remove the test cases from rustbolt and TURN ON webpack test case to suppress this warning."
				)
			);
		}
	}
