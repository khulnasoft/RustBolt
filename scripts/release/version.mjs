...
export async function getLastVersion(root) {
	const pkgPath = path.resolve(root, "./packages/rustbolt/package.json");
...
export function getNextName(name) {
	...
	if (name === "create-rustbolt") {
		return "create-rustbolt-canary";
	}
	const nextName = name.replace(/^@rustbolt/, "@rustbolt-canary");
	return nextName;
}
