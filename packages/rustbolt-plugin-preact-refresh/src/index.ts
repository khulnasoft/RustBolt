...
export interface IPreactRefreshRustboltPluginOptions {
  /**
   * Include files to be processed by the plugin.
   * The value is the same as the `rule.test` option in Rustbolt.
   * @default /\.([jt]sx?)$/
   */
  include?: RuleSetCondition | null;
  /**
   * Exclude files from being processed by the plugin.
   * The value is the same as the `rule.exclude` option in Rustbolt.
...
interface NormalizedPluginOptions extends IPreactRefreshRustboltPluginOptions {
  include: NonNullable<IPreactRefreshRustboltPluginOptions['include']>;
  exclude: NonNullable<IPreactRefreshRustboltPluginOptions['exclude']>;
  preactPath: NonNullable<IPreactRefreshRustboltPluginOptions['preactPath']>;
}
...
const NAME = 'PreactRefreshRustboltPlugin';

class PreactRefreshRustboltPlugin implements RustboltPluginInstance {
  name = NAME;
  private options: NormalizedPluginOptions;

  constructor(options: IPreactRefreshRustboltPluginOptions) {
    this.options = {
  ...
  apply(compiler: Compiler) {
    ...
    compiler.hooks.thisCompilation.tap(NAME, (compilation) => {
      compilation.hooks.runtimeModule.tap(NAME, (runtimeModule) => {
        // rustbolt does not have addRuntimeModule and runtimeRequirements on js side
        if (
...
export { PreactRefreshRustboltPlugin };
export default PreactRefreshRustboltPlugin;
