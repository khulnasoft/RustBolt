...
import type { ContextAPI, PropagationAPI, TraceAPI } from "@rustbolt/tracing";
import type { Compilation } from "../Compilation";
import type { Compiler } from "../Compiler";
import { NormalModule } from "../NormalModule";
import { NonErrorEmittedError, type RustboltError } from "../RustboltError";
import {
...
function createLoaderObject(
	...
	Object.defineProperty(obj, "request", {
		...
		set: (value: JsLoaderItem) => {
			...
			// CHANGE: `rustbolt_core` returns empty string for `undefined` type.
...
// FIXME: a temporary fix, we may need to change @rustbolt/tracing to commonjs really fix it
let cachedTracing:
	| {
			trace: TraceAPI;
...
async function getCachedTracing() {
	...
	if (cachedTracing === undefined) {
		try {
			const tracing = await import("@rustbolt/tracing");
			cachedTracing = {
				trace: tracing.trace,
...
async function tryTrace(context: JsLoaderContext) {
	...
	if (cachedTracing) {
		...
		const tracer = trace.getTracer("rustbolt-loader-runner");
...
export async function runLoaders(
	...
	loaderContext.emitError = function emitError(err) {
		...
		(error as RustboltError).moduleIdentifier =
			loaderContext._module.identifier();
		compiler._lastCompilation!.__internal__pushRustboltDiagnostic({
			error,
			severity: JsRustboltSeverity.Error
		});
	};
	loaderContext.emitWarning = function emitWarning(warn) {
		...
		(warning as RustboltError).moduleIdentifier =
			loaderContext._module.identifier();
		compiler._lastCompilation!.__internal__pushRustboltDiagnostic({
			error: warning,
			severity: JsRustboltSeverity.Warn
		});
	};
