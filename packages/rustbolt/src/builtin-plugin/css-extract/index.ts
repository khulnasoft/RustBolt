...
export type { CssExtractRustboltLoaderOptions } from "./loader";

export interface CssExtractRustboltPluginOptions {
	...
	// workaround for pathinto, deprecate this when rustbolt supports pathinfo
	pathinfo?: boolean;
	// when using relativePath, add './' prefix, e.g:  url('static/img/foo.png') => url('./static/img/foo.png')
	enforceRelative?: boolean;
}

export class CssExtractRustboltPlugin {
	static pluginName = PLUGIN_NAME;
	static loader: string = LOADER_PATH;

	options: CssExtractRustboltPluginOptions;

	constructor(options?: CssExtractRustboltPluginOptions) {
	...
	apply(compiler: Compiler) {
		...
		compiler.__internal__registerBuiltinPlugin({
			name: BuiltinPluginName.CssExtractRustboltPlugin,
...
export default CssExtractRustboltPlugin;
