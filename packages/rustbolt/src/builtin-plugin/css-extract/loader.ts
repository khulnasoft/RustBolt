...
export interface CssExtractRustboltLoaderOptions {
...
export function hotLoader(
	...
	context: {
		...
		options?: CssExtractRustboltLoaderOptions;
...
export const pitch: LoaderDefinition["pitch"] = function (request, _, data) {
	if (
		...
		const e = new Error(
			`use type 'css' and \`CssExtractRustboltPlugin\` together, please set \`experiments.css\` to \`false\` or set \`{ type: "javascript/auto" }\` for rules with \`CssExtractRustboltPlugin\` in your rustbolt config (now \`CssExtractRustboltPlugin\` does nothing).`
		);
		e.stack = undefined;
		this.emitWarning(e);
	...
	const options = this.getOptions() as CssExtractRustboltLoaderOptions;
