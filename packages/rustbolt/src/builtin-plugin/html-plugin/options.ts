...
const compilationOptionsMap: WeakMap<Compilation, HtmlRustboltPluginOptions> =
	new WeakMap();
...
export type HtmlRustboltPluginOptions = {
	...
	/**
	 * If `true` then append a unique Rustbolt compilation hash to all included scripts and CSS files. This is useful for cache busting.
	 */
	hash?: boolean;
};
...
const pluginOptionsSchema = z.strictObject({
	...
	template: z
		...
		.refine(
			...
			() => ({
				message:
					"HtmlRustboltPlugin does not support template path with loader yet"
			})
		)
		.optional(),
...
}) satisfies z.ZodType<HtmlRustboltPluginOptions>;

export function validateHtmlPluginOptions(options: HtmlRustboltPluginOptions) {
...
export const setPluginOptions = (
	...
	options: HtmlRustboltPluginOptions
) => {
