...
import {
	BuiltinPluginName,
	type JsRustboltError,
	type RawIntegrityData,
	type RawSubresourceIntegrityPluginOptions
} from "@rustbolt/binding";
import type { AsyncSeriesWaterfallHook } from "@rustbolt/lite-tapable";
import { z } from "zod";
...
const NATIVE_HTML_PLUGIN = "HtmlRustboltPlugin";
			...
			if (this.validateError) {
				compiler.hooks.compilation.tap(PLUGIN_NAME, compilation => {
					compilation.errors.push(
						this.validateError as unknown as JsRustboltError
					);
				});
			}
			return;
