...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	 ...
	 * @internal
	 */
	__internal__pushRustboltDiagnostic(diagnostic: binding.JsRustboltDiagnostic) {
	...
	get errors(): RustboltError[] {
		const inner = this.#inner;
		type ErrorType = RustboltError;
				...
				handler(
					...
					for (let i = 0; i < errs.length; i++) {
						...
						inner.pushDiagnostic(
							JsRustboltDiagnostic.__to_binding(error, JsRustboltSeverity.Error)
						);
					}
	...
	set errors(errors: RustboltError[]) {
		const inner = this.#inner;
	...
	get warnings(): RustboltError[] {
		const inner = this.#inner;
		type WarnType = Error | RustboltError;
				...
				handler(
					...
					return Reflect.apply(
						...
						processWarningsHook.call(warns as any).map(warn => {
							inner.pushDiagnostic(
								JsRustboltDiagnostic.__to_binding(warn, JsRustboltSeverity.Warn)
							);
							return warn;
						})
					);
	...
	set warnings(warnings: RustboltError[]) {
		const inner = this.#inner;
