import type { AssetInfo, RawFuncUseCtx } from "@rustbolt/binding";
...
//#region Context
/**
 * The context configuration is used to set the base directory for Rustbolt builds.
 * @default process.cwd()
 * */
...
/** A unique name of the Rustbolt build */
...
/** Tell Rustbolt to remove a module from the module instance cache (require.cache) if it throws an exception when it is required. */
...
/** Tells Rustbolt to add IIFE wrapper around emitted code. */
...
/** Tell Rustbolt what kind of ES-features may be used in the generated runtime-code. */
...
export type Output = {
	 ...
	 * Rustbolt adds some metadata in CSS to parse CSS modules, and this configuration determines whether to compress these metadata.
	 *
...
/** Used to configure the Rustbolt module resolution */
...
export type RuleSetRule = {
	...
	/** Used to mark the type of the matching module, which affects how the module is handled by Rustbolt's built-in processing. */
	type?: string;
...
/** Used to configure the target environment of Rustbolt output and the ECMAScript version of Rustbolt runtime code. */
...
//#region Plugins
export interface RustboltPluginInstance {
...
type SharedOptimizationSplitChunksCacheGroup = {
	...
	/**
	 * Tell Rustbolt what delimiter to use for the generated names.
	 *
	 * @default '-''
	 */
...
export type OptimizationSplitChunksCacheGroup = {
	...
	/**
	 * Tells Rustbolt to ignore `splitChunks.minSize`, `splitChunks.minChunks`, `splitChunks.maxAsyncRequests` and `splitChunks.maxInitialRequests` options and always create chunks for this cache group.
	 */
	enforce?: boolean;
...
/** Tell Rustbolt how to splitting chunks. */
...
export type Optimization = {
	 ...
	 * By default, `rustbolt.SwcJsMinimizerRustboltPlugin` and `rustbolt.LightningCssMinimizerRustboltPlugin` are used.
	 */
	...
	/**
	 * Tells Rustbolt to recognise the sideEffects flag in package.json or rules to skip over modules which are flagged to contain no side effects when exports are not used.
	 *
	 * The value is `true` in production mode.
	 * The value is `false` in development mode.
	 * */
	...
	/**
	 * Tells Rustbolt to find segments of the module graph which can be safely concatenated into a single module.
	 *
	 * The value is `true` in production mode.
	 * The value is `false` in development mode.
	 */
	...
	/**
	 * Tells Rustbolt whether to perform a more detailed analysis of variable assignments.
	 *
	 * The value is `true` in production mode.
	 * The value is `false` in development mode.
	 */
	...
	/**
	 * Tells Rustbolt to determine used exports for each module.
	 *
	 * The value is `true` in production mode.
	 * The value is `false` in development mode.
	 * */
	...
	/**
	 * Tells Rustbolt to set process.env.NODE_ENV to a given string value.
	 * @default false
	 */
...
/**
 * Options for future Rustbolt features.
 */
export type RustboltFutureOptions = {
	/**
...
export type Experiments = {
	 ...
	 *
	 * @description
	 * Once enabled, Rustbolt will enable native CSS support, and CSS related parser and generator options.
	...
	/**
	 * Enable future Rustbolt features default options.
	 */
	rustboltFuture?: RustboltFutureOptions;
	/**
...
//#region Performance
/** Options to control how Rustbolt notifies you of assets and entry points that exceed a specific file limit.   */
...
export type RustboltOptions = {
	/**
	 * The name of the Rustbolt configuration.
	 */
	name?: Name;
	/**
	 ...
	 * with the rightmost configuration taking precedence(only works when using @rustbolt/cli).
	 */
	 ...
	 * Setting `amd` to false will disable rustbolt's AMD support.
	 */
	amd?: Amd;
	/**
...
/** Configuration for Rustbolt */
export type Configuration = RustboltOptions;
