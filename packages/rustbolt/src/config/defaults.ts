...
export const applyRustboltOptionsDefaults = (
	...
	// but Rustbolt currently does not support this option
...
export const applyRustboltOptionsBaseDefaults = (
	options: RustboltOptionsNormalized
) => {
	F(options, "context", () => process.cwd());
...
const applyExperimentsDefaults = (
	...
	// IGNORE(experiments.incremental): Rustbolt specific configuration for incremental
	...
	// IGNORE(experiments.rustboltFuture): Rustbolt specific configuration
	D(experiments, "rustboltFuture", {});
	// rustboltFuture.bundlerInfo default value is applied after applyDefaults

	// IGNORE(experiments.parallelCodeSplitting): Rustbolt specific configuration for new code splitting algorithm
...
const applybundlerInfoDefaults = (
	...
	if (typeof rustboltFuture === "object") {
		D(rustboltFuture, "bundlerInfo", {});
		if (typeof rustboltFuture.bundlerInfo === "object") {
			D(
				rustboltFuture.bundlerInfo,
				"version",
				require("../../package.json").version
			);
			D(rustboltFuture.bundlerInfo, "bundler", "rustbolt");
			// don't inject for library mode
			D(rustboltFuture.bundlerInfo, "force", !library);
		}
	}
};
...
const applyModuleDefaults = (
	...
	// IGNORE(module.defaultRules): Rustbolt does not support `rule.assert`
...
const applyOutputDefaults = (
	...
	// IGNORE(output.hashFunction): Rustbolt uses faster xxhash64 by default
...
const applyOptimizationDefaults = (
	...
	// IGNORE(optimization.avoidEntryIife): to update the default value of webpack and bump webpack version in Rustbolt.
	D(optimization, "avoidEntryIife", false);
	D(optimization, "minimize", production);
	D(optimization, "concatenateModules", production);
	// IGNORE(optimization.minimizer): Rustbolt use `SwcJsMinimizerRustboltPlugin` and `LightningCssMinimizerRustboltPlugin` by default
	A(optimization, "minimizer", () => [
		new SwcJsMinimizerRustboltPlugin(),
		new LightningCssMinimizerRustboltPlugin()
	]);
	...
	if (splitChunks) {
		// IGNORE(optimization.splitChunks.defaultSizeTypes): Rustbolt enables `experiments.css` by default currently
...
const getResolveDefaults = ({
	...
	const resolveOptions: ResolveOptions = {
		...
		// IGNORE(resolve.pnp): Rustbolt use `resolve.enable` to enable Yarn PnP feature
		pnp: getPnpDefault(),
	...
	if (css) {
		...
		// IGNORE(resolve.byDependency.css-import): Rustbolt enables `experiments.css` by default currently
