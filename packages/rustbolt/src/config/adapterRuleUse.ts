...
import type { RustboltOptionsNormalized } from "./normalization";
import type {
	Mode,
	PublicPath,
	Resolve,
...
export interface LoaderContext<OptionsType = {}> {
	...
	/**
	 * Tells Rustbolt that this loader will be called asynchronously. Returns `this.callback`.
	 */
	 ...
	 * 4. The fourth parameter is ignored by Rustbolt and can be anything (e.g. some metadata).
	 */
	...
	sourceMap: boolean;
	/**
	 * The base path configured in Rustbolt config via `context`.
	 */
	rootContext: string;
	/**
	...
	/**
	 * The value of `mode` is read when Rustbolt is run.
	 ...
	 * @param schema To provide the best performance, Rustbolt does not perform the schema
	 ...
	 * mark the current module as a compilation failure, it just adds an error to Rustbolt's
	...
	data: unknown;
	/**
	 * Access to the current Compiler object of Rustbolt.
	 */
	_compiler: Compiler;
	/**
	 * Access to the current Compilation object of Rustbolt.
	 */
	...
	_module: Module;
	/**
	 * Note: This is not a Rustbolt public API, maybe removed in future.
...
const getSwcLoaderOptions: GetLoaderOptions = (options, _) => {
	if (options && typeof options === "object") {
		...
		// resolve `rustboltExperiments.import` options
		const { rustboltExperiments } = options;
		if (rustboltExperiments) {
			if (rustboltExperiments.import || rustboltExperiments.pluginImport) {
				rustboltExperiments.import = resolvePluginImport(
					rustboltExperiments.import || rustboltExperiments.pluginImport
				);
			}
		}
	}
...
export function isUseSourceMap(
	devtool: RustboltOptionsNormalized["devtool"]
): boolean {
	if (!devtool) {
		return false;
	}
