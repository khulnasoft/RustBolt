...
export const getNormalizedRustboltOptions = (
	...
	return {
		...
		output: nestedConfig(config.output, output => {
			if ("cssHeadDataCompression" in output) {
				util.deprecate(
					() => {},
					"cssHeadDataCompression is not used now, see https://github.com/khulnasoft/rustbolt/pull/8534, this option could be removed in the future"
				)();
			}
				...
				return {
					...
					storage: {
						...
						directory: path.resolve(
							...
							cache.storage?.directory || "node_modules/.cache/rustbolt"
						)
					}
				};
			}),
...
export interface RustboltOptionsNormalized {
	name?: Name;
