...
import { rustbolt } from "./index";
import { unsupported } from "./util";

import { canInherentFromParent } from "./builtin-plugin/base";
import { applyRustboltOptionsDefaults, getPnpDefault } from "./config/defaults";
...
class Compiler {
	...
	webpack: typeof rustbolt;
	rustbolt: typeof rustbolt;
	name?: string;
	...
	constructor(context: string, options: RustboltOptionsNormalized) {
		this.#initial = true;
		...
		this.webpack = rustbolt;
		this.rustbolt = rustbolt;
		this.root = this;
		this.outputPath = "";
		...
		new JsLoaderRustboltPlugin(this).apply(this);
		new ExecuteModulePlugin().apply(this);

		// this.hooks.shutdown.tap("rustbolt:cleanup", () => {
		// 	// Delayed rustbolt cleanup to the next tick.
		// 	// This supports calls to `fn rustbolt` to do something with `Stats` within the same tick.
		// 	process.nextTick(() => {
		// 		if (!this.running) {
	...
	createChildCompiler(
		...
		plugins: RustboltPluginInstance[]
	): Compiler {
		const options: RustboltOptionsNormalized = {
			...this.options,
			output: {
				...this.options.output,
				...outputOptions
		...
		applyRustboltOptionsDefaults(options);
	 ...
	 * @private this method is only used in Rustbolt core
	 */
	...
	#getInstance(
		...
		const instanceBinding: typeof binding = require("@rustbolt/binding");
