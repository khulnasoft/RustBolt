...
export class RustboltOptionsApply {
	process(options: RustboltOptionsNormalized, compiler: Compiler) {
		assert(
			options.output.path,
			"options.output.path should have value after `applyRustboltOptionsDefaults`"
		);
		...
		if (options.externals) {
			assert(
				...
				"options.externalsType should have value after `applyRustboltOptionsDefaults`"
			);
		...
		if (
			...
			new HttpExternalsRustboltPlugin(
		...
		if (options.experiments.rustboltFuture!.bundlerInfo) {
			new BundlerInfoRustboltPlugin(
				options.experiments.rustboltFuture!.bundlerInfo
			).apply(compiler);
		}
		...
		if (minimize && minimizer) {
			for (const item of minimizer) {
				if (typeof item === "function") {
					(item as RustboltPluginFunction).call(compiler, compiler);
				} else if (item !== "..." && item) {
					item.apply(compiler);
				}
			}
