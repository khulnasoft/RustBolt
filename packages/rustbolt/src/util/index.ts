import type { JsRustboltError, JsStatsError } from "@rustbolt/binding";
...
export function concatErrorMsgAndStack(
	err: Error | JsRustboltError | string
): JsRustboltError {
...
const getDeprecationStatus = () => {
	...
	if (
		process.env.RUSTBOLT_DEP_WARNINGS === "false" ||
		process.env.RUSTBOLT_DEP_WARNINGS === "0"
	) {
		return false;
	}
	return (
...
export const deprecatedWarn = (
	...
	if (enable) {
		...
		console.warn(
			indent(
				"Set env `RUSTBOLT_DEP_WARNINGS` to 'false' to temporarily disable deprecation warnings.\n",
				"    "
			)
		);
	}
...
export const unsupported = (name: string, issue?: string) => {
	let s = `${name} is not supported by rustbolt.`;
	if (issue) {
