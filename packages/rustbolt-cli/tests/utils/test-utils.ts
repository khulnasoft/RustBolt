...
const RUSTBOLT_PATH = path.resolve(__dirname, "../../bin/rustbolt.js");
...
const createProcess = (cwd, args, options, env) => {
	...
	return processExecutor(RUSTBOLT_PATH, args, {
		cwd: path.resolve(cwd),
		reject: false,
		stdio: ENABLE_LOG_COMPILATION ? "inherit" : "pipe",
		maxBuffer: Infinity,
		env: { RUSTBOLT_CLI_HELP_WIDTH: 1024, ...env },
		...options
	});
};

/**
 * Run the rustbolt CLI for a test case.
 *
 * @param {string} cwd The path to folder that contains test
 * @param {Array<string>} args Array of arguments
 * @param {Object<string, any>} options Options for tests
...
/**
 * Run the rustbolt CLI for a test case and get process.
 *
 * @param {string} cwd The path to folder that contains test
 * @param {Array<string>} args Array of arguments
 * @param {Object<string, any>} options Options for tests
...
/**
 * Run the rustbolt CLI in watch mode for a test case.
 *
 * @param {string} cwd The path to folder that contains test
 * @param {Array<string>} args Array of arguments
 * @param {Object<string, any>} options Options for tests
 * @returns {Object} The rustbolt output or Promise when nodeOptions are present
 */
const runWatch = (
	cwd,
	args: string[] = [],
	...
	return new Promise((resolve, reject) => {
		...
		const outputKillStr = options.killString || /rustbolt \d+\.\d+\.\d/;
...
const normalizeStderr = stderr => {
	...
	if (!/On Your Network \(IPv6\)/.test(stderr)) {
		...
		if (ipv4MessageIndex !== -1) {
			normalizedStderr.splice(
				ipv4MessageIndex + 1,
				0,
				"<i> [rustbolt-dev-server] On Your Network (IPv6): http://[<network-ip-v6>]:<port>/"
			);
		}
	...
	if (/Gracefully shutting down/.test(stderr)) {
		normalizedStderr = normalizedStderr.replace(
			"\n<i> [rustbolt-dev-server] Gracefully shutting down. To force exit, press ^C again. Please wait...",
			""
		);
	}
