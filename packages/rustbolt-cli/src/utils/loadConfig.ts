...
import type { RustboltCLIOptions } from "../types";
...
const DEFAULT_CONFIG_NAME = "rustbolt.config" as const;
...
const registerLoader = async (configPath: string) => {
	...
	// For ESM and `.mts` you need to use: 'NODE_OPTIONS="--loader ts-node/esm" rustbolt build --config ./rustbolt.config.mts'
...
export type LoadedRustboltConfig =
	| undefined
	| RustboltOptions
...
export async function loadExtendedConfig(
	config: RustboltOptions | MultiRustboltOptions,
	configPath: string,
	cwd: string,
	...
	const { extends: _, ...configWithoutExtends } = config as RustboltOptions;
...
export async function loadRustboltConfig(
	options: RustboltCLIOptions,
	cwd = process.cwd()
): Promise<LoadedRustboltConfig> {
	let configPath: string | undefined;
	let loadedConfig: LoadedRustboltConfig;
	...
	if (typeof loadedConfig !== "function" && configPath) {
		loadedConfig = await loadExtendedConfig(
			loadedConfig as RustboltOptions | MultiRustboltOptions,
			configPath,
			cwd,
			options
		);
