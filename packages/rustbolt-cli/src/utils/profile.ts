...
`RUSTBOLT_PROFILE='TRACE=filter=trace&output=./rustbolt.trace&layer=chrome|JSCPU=output=./rustbolt.jscpuprofile|LOGGING=output=./rustbolt.logging' rustbolt build`
											 ...
											 ^-----------: trace filter, default to `trace`, more syntax: https://docs.rs/tracing-subscriber/latest/tracing_subscriber/filter/struct.EnvFilter.html#example-syntax
																		^--------------------: trace output, `stderr`, `stdout`, or a file path, default to `./.rustbolt-profile-${timestamp}/trace.json` for layer `chrome` and default to `stdout` for layer `logger`
																													^-----------: trace layer, `chrome` or `logger`, default to `chrome`
																																						 ^---------------------------: js cpuprofile output, `stderr`, `stdout`, or a file path, default to `./.rustbolt-profile-${timestamp}/jscpuprofile.json`
																																																									^----------------------: stats.logging output, default to `./.rustbolt-profile-${timestamp}/logging.json`

`RUSTBOLT_PROFILE='TRACE=filter=trace&output=./rustbolt.trace&layer=chrome' rustbolt build`: only enable trace

`RUSTBOLT_PROFILE=TRACE rustbolt build`: only enable trace, and use default options for trace

`RUSTBOLT_PROFILE='JSCPU=output=./rustbolt.jscpuprofile' rustbolt build`: only enable js cpuprofile

`RUSTBOLT_PROFILE=JSCPU rustbolt build`: only enable js cpuprofile, and use default options for js cpuprofile

`RUSTBOLT_PROFILE='LOGGING=output=./rustbolt.logging' rustbolt build`: only enable stats.logging

`RUSTBOLT_PROFILE=LOGGING rustbolt build`: only enable stats.logging, and use default options for stats.logging

`RUSTBOLT_PROFILE=ALL rustbolt build`: enable all, and use default options

`RUSTBOLT_PROFILE=[rustbolt_node,rustbolt_core] rustbolt build`: enable all, but customize trace filter
...
import { type Compiler, type RustboltOptions, rustbolt } from "@rustbolt/core";
...
type ParametersOfRegisterGlobalTrace = Parameters<
	typeof rustbolt.experiments.globalTrace.register
>;
...
const defaultOutputDirname = path.resolve(
	`.rustbolt-profile-${timestamp}-${process.pid}`
);
...
class RustboltProfileJSCPUProfilePlugin {
	...
	apply(compiler: Compiler) {
		...
		compiler.hooks.done.tapAsync(
			RustboltProfileJSCPUProfilePlugin.name,
			(_stats, callback) => {
...
class RustboltProfileLoggingPlugin {
	constructor(private output: string) {}

	apply(compiler: Compiler) {
		compiler.hooks.done.tapAsync(
			RustboltProfileLoggingPlugin.name,
			(stats, callback) => {
				if (compiler.watchMode) return callback();
...
export async function applyProfile(
	...
	for (const [kind, value] of entries) {
		...
		if (kind === "TRACE" && "filter" in value) {
			await rustbolt.experiments.globalTrace.register(
				value.filter,
				value.layer,
				value.output
			);
			asyncExitHook(rustbolt.experiments.globalTrace.cleanup, {
				wait: 500
			});
		} else if (kind === "JSCPU") {
			(item.plugins ??= []).push(
