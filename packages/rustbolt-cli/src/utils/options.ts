...
/**
 * set builtin env from cli - like `WEBPACK_BUNDLE=true`. also for `RUSTBOLT_` prefixed.
 * @param env the `argv.env` object
 * @param envNameSuffix the added env will be `WEBPACK_${envNameSuffix}` and `RUSTBOLT_${envNameSuffix}`
 * @param value
 */
