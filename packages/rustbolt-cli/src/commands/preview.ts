...
import type { RustboltCommand, RustboltPreviewCLIOptions } from "../types";
...
export class PreviewCommand implements RustboltCommand {
	async apply(cli: RustboltCLI): Promise<void> {
		cli.program.command(
			...
			async options => {
				...
				let config = await cli.loadConfig(rustboltOptions);
				config = await getPreviewConfig(config, options);
				if (!Array.isArray(config)) {
					config = [config as RustboltOptions];
				}

				config = config as MultiRustboltOptions;
...
async function getPreviewConfig(
	...
	options: RustboltPreviewCLIOptions
): Promise<RustboltOptions | MultiRustboltOptions> {
	const internalPreviewConfig = async (item: RustboltOptions) => {
	...
	return internalPreviewConfig(item as RustboltOptions);
}
