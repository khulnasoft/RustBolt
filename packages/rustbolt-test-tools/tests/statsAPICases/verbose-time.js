...
module.exports = {
	...
	async check(stats) {
		...
		LOG from rustbolt.EnsureChunkConditionsPlugin
		<t> ensure chunk conditions: X ms

		LOG from rustbolt.ModuleConcatenationPlugin
		...
		LOG from rustbolt.RealContentHashPlugin
		<t> hash to asset names: X ms

		LOG from rustbolt.RemoveEmptyChunksPlugin
		<t> remove empty chunks: X ms

		LOG from rustbolt.SideEffectsFlagPlugin
