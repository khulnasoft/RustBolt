...
function assertWebpackConfig(config) {
	const rustboltBaseConfig = DefaultsConfigProcessor.getDefaultConfig(
		cwd,
		config
	);
	const webpackBaseConfig = getWebpackDefaultConfig(cwd, config);
	const rustboltSupportedConfig = getObjectPaths(rustboltBaseConfig);
	const defaultsPath = path.resolve(
		__dirname,
		"../../rustbolt/src/config/defaults.ts"
	);
	...
	trimObjectPaths(rustboltBaseConfig, ignoredPaths);
	trimObjectPaths(webpackBaseConfig, ignoredPaths);
	filterObjectPaths(webpackBaseConfig, rustboltSupportedConfig);
	expect(rustboltBaseConfig).toEqual(webpackBaseConfig);
}
