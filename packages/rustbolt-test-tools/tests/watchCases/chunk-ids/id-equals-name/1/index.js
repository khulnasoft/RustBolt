const promise = Promise.all([
  import(/* webpackChunkName: "id-equals-name_js" */ "./id-equals-name"),
  import("./id-equals-name?1"),
]);
...
it("should have correct chunk id", async () => {
  ...
  expect(__STATS__.chunks.map(c => c.id)).toEqual(['id-equals-name_js', 'id-equals-name_js_1', 'main']);
})
...
it("should have correct log when incremental enabled", async () => {
  ...
  const incrementalLog = /LOG from rustbolt\.incremental\.chunkIds[\s\S]*?LOG/.exec(statsString);
  if (incrementalLog) {
