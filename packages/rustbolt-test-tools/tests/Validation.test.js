describe("Validation", () => {
	const createTestCase = (name, config, fn, strategy, fn2) => {
		it(`should fail validation for ${name}`, () => {
			let prevStrategy = process.env.RUSTBOLT_CONFIG_VALIDATE;
			process.env.RUSTBOLT_CONFIG_VALIDATE = strategy;
			let errors = [];
			console.error = (...args) => {
				errors.push(...args);
			};
			try {
				...
				process.env.RUSTBOLT_CONFIG_VALIDATE = prevStrategy;
			}
	...
	describe("loose-unrecognized-keys", () => {
		createTestCase(
			...
			Array [
			  Invalid configuration object. Rustbolt has been initialized using a configuration object that does not match the API schema.
