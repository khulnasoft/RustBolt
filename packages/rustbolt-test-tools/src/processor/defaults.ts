...
export interface IDefaultsConfigProcessorOptions<T extends ECompilerType> {
	...
	name: string;
	diff: (
		diff: jest.JestMatchers<RustboltTestDiff>,
...
> extends SimpleTaskProcessor<T> {
	...
	async check(env: ITestEnv, context: ITestContext) {
		...
		await this._defaultsConfigOptions.diff(
			env.expect(new RustboltTestDiff(diff)),
	...
	static getDefaultConfig(
		...
		(normalizedConfig as any).experiments.rustboltFuture.bundlerInfo.version =
			"$version$";
