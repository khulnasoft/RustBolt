		...
		super({
			options: context => {
				const res = (_statsAPIOptions.options?.(context) ||
					{}) as TCompilerOptions<ECompilerType.Rustbolt>;
				res.experiments ??= {};
				res.experiments!.css ??= true;
				res.experiments!.rustboltFuture ??= {};
				res.experiments!.rustboltFuture!.bundlerInfo ??= {};
				res.experiments!.rustboltFuture!.bundlerInfo!.force ??= false;
