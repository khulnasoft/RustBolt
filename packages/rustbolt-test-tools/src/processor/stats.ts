...
import type { Compiler, Stats } from "@rustbolt/core";
...
> extends MultiTaskProcessor<T> {
	...
	async check(env: ITestEnv, context: ITestContext) {
		...
		if (this.snapshotName) {
			env.expect(new RustboltStats(actual)).toMatchSnapshot(this.snapshotName);
		} else {
			env.expect(new RustboltStats(actual)).toMatchSnapshot();
		}
	...
	static defaultOptions<T extends ECompilerType>(
		...
		if (fs.existsSync(path.join(context.getSource(), "rustbolt.config.js"))) {
			return {
				experiments: {
					css: true,
					rustboltFuture: {
