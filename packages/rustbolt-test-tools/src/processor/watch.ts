...
// This file is used to port step number to rustbolt.config.js/webpack.config.js
const currentWatchStepModulePath = path.resolve(
	__dirname,
	"../helper/util/currentWatchStep"
);

type TRustboltExperiments = TCompilerOptions<ECompilerType>["experiments"];
type TRustboltOptimization = TCompilerOptions<ECompilerType>["optimization"];

export interface IWatchProcessorOptions<T extends ECompilerType>
	extends IMultiTaskProcessorOptions<T> {
	stepName: string;
	tempDir: string;
	experiments?: TRustboltExperiments;
	optimization?: TRustboltOptimization;
}
...
> extends MultiTaskProcessor<T> {
	...
	static overrideOptions<T extends ECompilerType>({
		...
		return (
			...
			(options as TCompilerOptions<ECompilerType.Rustbolt>).experiments ??= {};
			(options as TCompilerOptions<ECompilerType.Rustbolt>).experiments!.css ??=
				true;
			(
				options as TCompilerOptions<ECompilerType.Rustbolt>
			).experiments!.rustboltFuture ??= {};
			(
				options as TCompilerOptions<ECompilerType.Rustbolt>
			).experiments!.rustboltFuture!.bundlerInfo ??= {};
			(
				options as TCompilerOptions<ECompilerType.Rustbolt>
			).experiments!.rustboltFuture!.bundlerInfo!.force ??= false;
