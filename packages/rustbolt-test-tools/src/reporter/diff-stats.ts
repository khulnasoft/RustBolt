	...
	async increment(id: string, data: TModuleCompareResult[]) {
		...
		for (const item of data) {
			...
			if (item.type === ECompareResultType.OnlySource) {
				current[`${ECompilerType.Rustbolt}|${moduleType}|${"modules"}`]++;
			...
			// handle lines
			current[`${ECompilerType.Rustbolt}|${moduleType}|${"lines"}`] +=
				item.lines?.source || 0;
	...
	private stringifySummary(id: string, summary: TCaseSummary) {
		...
		for (const moduleType of ["runtime", "normal"] as TModuleTypeId[]) {
			...
			csv.push(
				...
				} Modules,Rustbolt Only,Common,Webpack Only,Common Percent`
			);
			for (const dimen of [
				"modules",
				"lines",
		...
		if (this.options.report && GITHUB_RUN_ID) {
			output += `> [View diff report](https://khulnasoft.github.io/rustbolt-report-website/diff/${GITHUB_RUN_ID}/diff_${id}.html)`;
			output += "\n\n";
		}
		return output;
	}
