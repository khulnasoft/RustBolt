...
import { type Compiler, MultiCompiler } from "@rustbolt/core";
...
export class RustboltDevServer extends WebpackDevServer {
			...
			if (this.options.hot) {
				...
				compiler.options.resolve.alias = {
					"ansi-html-community": require.resolve(
						"@rustbolt/dev-server/client/utils/ansiHTML"
					),
					...compiler.options.resolve.alias
				};
			}
	...
	getClientEntry(): string {
		return require.resolve("@rustbolt/dev-server/client/index");
	}

	getClientHotEntry(): string | undefined {
		if (this.options.hot === "only") {
			return require.resolve("@rustbolt/core/hot/only-dev-server");
		}
		if (this.options.hot) {
			return require.resolve("@rustbolt/core/hot/dev-server");
		}
	}
}
