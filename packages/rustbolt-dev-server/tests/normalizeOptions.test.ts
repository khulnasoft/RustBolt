import { type RustboltOptions, rustbolt } from "@rustbolt/core";
import { type Configuration, RustboltDevServer } from "@rustbolt/dev-server";
...
describe.skip("normalize options snapshot", () => {
	...
	it("should apply HMR plugin by default", async () => {
		...
		const server = new RustboltDevServer({}, compiler);
		await server.start();
	...
	it("should support custom client transport", async () => {
		const compiler = rustbolt(customConfig);
...
async function match(config: RustboltOptions) {
	const compiler = rustbolt({
		...config,
		entry: ENTRY,
		stats: "none",
	});
	const server = new RustboltDevServer(
...
async function getAdditionEntries(
	...
	const server = new RustboltDevServer(serverConfig, compiler);
	await server.start();
