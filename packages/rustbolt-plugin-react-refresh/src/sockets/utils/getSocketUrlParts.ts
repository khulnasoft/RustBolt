	...
	if (!urlParts.hostname || !urlParts.pathname) {
		throw new Error(
			[
				...
				"You should either specify the socket path parameters under the `devServer` key in your Rustbolt config, or use the `overlay` option.",
				"https://www.rustbolt.dev/guide/tech/react#fast-refresh"
			].join("\n")
		);
	}
