...
```js
new rustbolt.CopyRustboltPlugin(options);
```
...
The destination path of the copy operation, which can be an absolute path, a relative path, or a template string. If not specified, it is equal to Rustbolt's [output.path](/config/output#outputpath).
...
`context` can be an absolute path or a relative path. If it is a relative path, then it will be converted to an absolute path based on Rustbolt's [context](/config/context).
...
Specify the type of [to](#to), which can be a directory, a file, or a template name in Rustbolt. If not specified, it will be automatically inferred.
