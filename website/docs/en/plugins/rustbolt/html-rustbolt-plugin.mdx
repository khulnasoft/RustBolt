...
`rustbolt.HtmlRustboltPlugin` is a high-performance HTML plugin implemented in Rust. You can use it to generate HTML files for Rustbolt projects.

```js
new rustbolt.HtmlRustboltPlugin(options);
```

## Comparison

Before using `rustbolt.HtmlRustboltPlugin`, please note that there are some differences between `rustbolt.HtmlRustboltPlugin` and the community [`html-webpack-plugin`](https://www.npmjs.com/package/html-webpack-plugin).

### Performance

Because `rustbolt.HtmlRustboltPlugin` is implemented in Rust, its build performance is significantly better than `html-webpack-plugin`, especially in scenarios where many HTML files are being built.

### Features

The features of `rustbolt.HtmlRustboltPlugin` are a subset of `html-webpack-plugin`. To ensure the performance of the plugin, we have not implemented all the features provided by `html-webpack-plugin`.
...
:::warning
`rustbolt.HtmlRustboltPlugin` does not support the full [EJS syntax](https://github.com/mde/ejs/blob/main/docs/syntax.md), it only supports a subset of the EJS syntax. If you need the full EJS syntax support, you can use `html-webpack-plugin` directly.
In order to align with the default template syntax of `html-webpack-plugin`, Rustbolt changed the default EJS escape and unescape to be the same as `html-webpack-plugin`'s default syntax.
:::
...
```html title="html"
<p>Hello, Rustbolt&lt;y&gt;.</p>
<p>Hello, the Most Honorable Rustbolt&lt;y&gt;.</p>
```
...
```json title="locals"
{
  "myHtml": "<strong>Rustbolt</strong>",
  "myMaliciousHtml": "</p><script>document.write()</script><p>"
}
```

```html title="html"
<p>Hello, &lt;strong&gt;Rustbolt&lt;/strong&gt;.</p>
<p>Hello, <strong>Rustbolt</strong>.</p>
...
```txt title="ejs"
<% for tag in htmlRustboltPlugin.tags.headTags { %>
...
Just add the plugin to your Rustbolt config like this:
...
If you have multiple entry points in your Rustbolt config, they will all be included with `<script>` tags in the generated HTML.

If you have some CSS assets in the build outputs, they will be included with `<link>` tags in the HTML head.

## Options

You can pass some configuration options to `rustbolt.HtmlRustboltPlugin`. Allowed options are as follows:
...
The easiest way is to use the template option and pass a custom HTML file. The `rustbolt.HtmlRustboltPlugin` will automatically inject all the necessary JS, CSS and favicon files into the HTML.
...
<html>
  <head>
    <meta charset="utf-8" />
    <title><%= htmlRustboltPlugin.options.title %></title>
  </head>
  <body></body>
</html>
```
...
export default {
  plugins: [
    new rustbolt.HtmlRustboltPlugin({
      title: "My HTML Template"
      templateContent: ({ htmlRustboltPlugin }) => `
        <!DOCTYPE html>
        <html>
          <head>
            <title>${htmlRustboltPlugin.options.title}</title>
...
module.exports = ({ htmlRustboltPlugin }) => `
  <!DOCTYPE html>
  <html>
    <head>
      <title>${htmlRustboltPlugin.options.title}</title>
...
- `htmlRustboltPlugin`: Data of the plugin
  - `htmlRustboltPlugin.options`: Configuration object of the plugin
  - `htmlRustboltPlugin.tags`: Prepared tag information for injection in the template
    - `htmlRustboltPlugin.tags.headTags`: List of `<base>`, `<meta>`, `<link>`, `<script>` tags for injection in `<head>`
    - `htmlRustboltPlugin.tags.bodyTags`: List of `<script>` tags for injection in `<body>`
  - `htmlRustboltPlugin.files`: Asset files generated in this compilation
    - `htmlRustboltPlugin.files.js`: List of paths of JS assets generated in this compilation
    - `htmlRustboltPlugin.files.css`: List of paths of CSS assets generated in this compilation
    - `htmlRustboltPlugin.files.favicon`: If `favicon` is configured, here is the calculated final favicon asset path
    - `htmlRustboltPlugin.files.publicPath`: The `publicPath` of the asset files
- `rustboltConfig`: Rustbolt configuration object used in this compilation
- `compilation`: Compilation object of this compilation

:::warning
If `htmlRustboltPlugin.tags` is used to insert tags during template rendering, please configure `inject` as `false`, otherwise the tags will be injected twice.
:::
...
- The `rustboltConfig` object currently only supports `mode`, `output.publicPath` and `output.crossOriginLoading`
- The `compilation` object is currently only supported when [using the template function](#use-template-function)
- When rendering the tag list (such as `htmlRustboltPlugin.tags.headTags`) or a single tag (such as `htmlRustboltPlugin.tags.headTags[0]`) in the template, the `toHtml()` function is required to generate the HTML code
...
If `meta` is set, HtmlRustboltPlugin will inject `<meta>` tags.
...
If `base` is set, HtmlRustboltPlugin will inject the `<base>` tag.
...
If you have multiple entry points and want to generate an HTML file for each entry, you can register multiple `rustbolt.HtmlRustboltPlugin`:
...
HtmlRustboltPlugin provides some hooks that allow you to modify tags or generated HTML code. The hooks object can be obtained through `rustbolt.HtmlRustboltPlugin.getCompilationHooks`:

```js title="rustbolt.config.mjs"
const HtmlModifyPlugin = {
  apply(compiler) {
    compiler.hooks.compilation.tap('HtmlModifyPlugin', compilation => {
      const hooks = HtmlRustboltPlugin.getCompilationHooks(compilation);
...
export default {
  //...
  plugins: [new HtmlRustboltPlugin(), HtmlModifyPlugin],
};
```
...
const AddScriptPlugin = {
  apply(compiler) {
    compiler.hooks.compilation.tap('AddScriptPlugin', compilation => {
      HtmlRustboltPlugin.getCompilationHooks(
        compilation,
...
export default {
  //...
  plugins: [new HtmlRustboltPlugin(), AddScriptPlugin],
};
```
...
export default {
  //...
  plugins: [new HtmlRustboltPlugin(), AddAttributePlugin],
};
```
