...
This is a list of plugins used internally by Rustbolt, aligned with the plugins used internally by webpack.

:::warning
You should only concern yourself with these plugins if you are building your own compiler based on Rustbolt, or for internal inspection.
:::
...
Note that all plugins under `rustbolt.optimize` namespace should only be used when `mode` set to `'none'`. Otherwise you might get into trouble where plugins are applied twice.
