...
Rustbolt uses [indicatif::ProgressBar](https://github.com/console-rs/indicatif) to draw the progress bar.

```js
new rustbolt.ProgressPlugin(options);
```
...
new rustbolt.ProgressPlugin(handler);
```
