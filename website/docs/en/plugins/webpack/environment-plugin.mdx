...
:::warning
If an environment variable is not found during bundling and no default value was provided, Rustbolt will throw an error instead of a warning.
:::
...
When executing `NODE_ENV=production` Rustbolt in the terminal to build, `entry.js` becomes this:
...
Running `DEBUG=false` Rustbolt yields:
