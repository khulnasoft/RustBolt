...
For users of `@rustbolt/cli`, you can enable lazy compilation through [experiments.lazyCompilation](/config/experiments#experimentslazycompilation) configuration. Assuming you are developing a project with multiple entry points, when developing one of these entry points, Rustbolt will only build the entry point you are currently accessing.
...
Only when corresponding entries and modules are executed will Rustbolt compile their respective entries and Modules along with all their dependencies.

![image](https://assets.rustbolt.dev/rustbolt/assets/lazy-proxy-module.png)

## Using custom dev server

In the configuration above, the `experiments.lazyCompilation` option is actually processed by `@rustbolt/cli`. It adds an [Express-style middleware](https://expressjs.com/en/guide/using-middleware.html) to `@rustbolt/dev-server` specifically designed to handle lazy compilation client requests.

If you are using a custom dev server, you will need to manually integrate this middleware into your dev server.

```js title="start.mjs"
import { experiments, rustbolt } from '@rustbolt/core';
import config from './rustbolt.config.mjs';
import DevServer from 'webpack-dev-server';

const compiler = rustbolt(config);
