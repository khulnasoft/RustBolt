...
Rustbolt provides the [output.publicPath](/config/output#outputpublicpath) option, which sets the base URL path prefix for bundled static assets (such as JS, CSS, images, etc.).
...
The `__webpack_public_path__` will override the `output.publicPath` in the Rustbolt config, but it will only take effect for dynamically loaded assets, not for assets loaded via `<script src="...">`.
...
There are chances that you don't know what the `publicPath` will be in advance, and Rustbolt can automatically calculate the `publicPath` value by parsing some variables like [import.meta.url](/api/runtime-api/module-variables#importmetaurl), [document.currentScript](https://developer.mozilla.org/en-US/docs/Web/API/Document/currentScript), `script.src` or `self.location`.
