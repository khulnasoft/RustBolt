...
Assets (e.g. images, fonts, videos, etc.) are first-class citizens of Rustbolt, which means you don't need any loader to process them.
...
The above condition uses `type: 'asset/resource'`, which will direct Rustbolt to complete individual file generation for all matching files and return the final asset URL address.
