...
Rustbolt provides two solutions to support React:
...
- **Manual configure Rustbolt**: You can refer to the current document to manually add configurations for React.

## Configure JSX/TSX

Rustbolt leverages SWC transformer for supporting JSX and TSX.
...
There are currently two ways to enable React Fast Refresh in Rustbolt:

First you need to install [@rustbolt/plugin-react-refresh](https://www.npmjs.com/package/@rustbolt/plugin-react-refresh) to support React Fast Refresh.

<PackageManagerTabs command="add @rustbolt/plugin-react-refresh react-refresh -D" />
...
- Code injection will inject some code from the `react-refresh` package, as well as some custom runtime code, all of which are integrated in the [@rustbolt/plugin-react-refresh](https://github.com/rustbolt-contrib/rustbolt-plugin-react-refresh) plugin and can be injected through this plugin.
...
Compared to the previous approach, this method decouples the React Fast Refresh code injection logic from the transformation logic. The code injection logic is handled uniformly by the @rustbolt/plugin-react-refresh plugin, while the code transformation is handled by loaders. This means that this plugin can be used in conjunction with `builtin:swc-loader`, `swc-loader`, or `babel-loader`:

- For usage with `builtin:swc-loader`, you can refer to the example at [examples/react-refresh](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/react-refresh), When using with `swc-loader`, simply replace `builtin:swc-loader` with `swc-loader`.
- For usage with `babel-loader`, you can refer to the example at [examples/react-refresh-babel-loader](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/react-refresh-babel-loader)
...
The steps to use React Compiler in Rustbolt:
...
3. Register `babel-loader` in your Rustbolt config file:
...
- [Rustbolt + React Compiler](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/react-compiler-babel)
- [Rustbolt + React Compiler + TypeScript](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/react-compiler-babel-ts)
...
The usage of SVGR with Rustbolt is exactly the same as with Webpack.
