# Migrating from Rustbolt 0.x

The document lists all breaking changes from Rustbolt v0.7 to v1.0. You can refer to this document for migration.

> See [Breaking changes in Rustbolt v1.0.0](https://github.com/khulnasoft/rustbolt/discussions/6626) for details.

## Configuration default value adjustments

In Rustbolt 1.x, we have aligned the default configuration values with those of Webpack.
...
In Rustbolt 0.x, `experiments.css` was enabled by default, which means files ending with`*.css`were automatically treated as`type: 'css/auto'` without needing to manually include other loaders to process CSS files.
...
In Rustbolt 1.x, module concatenation optimization has become more stable. Thus, it's now enabled by default in production mode, allowing multiple modules to be concatenated into a single module to reduce output size and improve compression efficiency.
...
> `@rustbolt/cli` overrides the default `devtool` value from `@rustbolt/core`. Therefore, if you are using `@rustbolt/cli`, this change will not affect you.
...
To streamline the core, Rustbolt 1.x has removed the built-in SWC plugins. You now need to manually include them.

### [Important] Removed rustboltExperiments.styledComponents
...
### [Important] Removed rustboltExperiments.emotion
...
### [Important] Removed rustboltExperiments.relay
...
### [Important] Removed rustboltExperiments.preact
...
In Rustbolt 0.x, we used the built-in `rustbolt.SwcCssMinimizerRustboltPlugin` to compress CSS size.
Now, we have removed it and replaced it with [rustbolt.LightningCssMinimizerRustboltPlugin](/plugins/rustbolt/lightning-css-minimizer-rustbolt-plugin) to handle the same functionality.

If you previously manually registered and configured `rustbolt.SwcCssMinimizerRustboltPlugin`, you should to switch to `rustbolt.LightningCssMinimizerRustboltPlugin`:
...
Rustbolt's built-in and default-enabled JavaScript minimizer plugin has had its configuration aligned with [SWC's minification configuration](https://swc.rs/docs/configuration/minification). The breaking changes are as follows:
...
### [Important] @rustbolt/cli

`@rustbolt/cli` has upgraded its dependency on `webpack-dev-server` from v4 to v5. If you are using `@rustbolt/cli`, please be aware of the following breaking changes:
...
:::tip
Rustbolt supports the [NormalModuleFactory](/api/plugin-api/normal-module-factory-hooks)'s [resolve](/api/plugin-api/normal-module-factory-hooks#resolve) hook. In most cases, you can use this hook as a replacement for the `Resolver`'s `resolve` hook to achieve the same functionality.
