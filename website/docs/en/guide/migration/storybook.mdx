...
If you are using React / Vue with [Storybook](https://storybook.js.org/) and building with webpack 5, you can replace the `@storybook/react-webpack5` build with [storybook-rsbuild](https://github.com/rustbolt-contrib/storybook-rsbuild), which is implemented based on Rsbuild and uses Rustbolt as the underlying bundler. It supports out-of-the-box use, and the documentation can be found at [storybook-rsbuild](https://github.com/rustbolt-contrib/storybook-rsbuild).
...
The [rustbolt-contrib/storybook-rsbuild](https://github.com/rustbolt-contrib/storybook-rsbuild/tree/main/sandboxes) repository provides examples of Storybook for React / Vue projects.

## Limitations

Rustbolt is gradually improving full support for Storybook. Currently, there are some capabilities that are not supported, see [storybook-rsbuild - Roadmap](https://github.com/rustbolt-contrib/storybook-rsbuild#roadmap) for details.
