...
Rustbolt supports code splitting, which allows splitting the code into other chunks. You have the full control about size and number of generated assets, which allow you to gain performance improvements in loading time.

Here we introduce a concept called Chunk, representing a resource that a browser needs to load.

## Dynamic import

Rustbolt use the [import()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/import) syntax that conforms to the ECMAScript proposal for dynamic imports.
...
This is the simplest and most intuitive way to split the code. However, this approach requires us to manually configure the Rustbolt. Let's start by looking at how to split multiple Chunks from multiple entry points.
...
![](https://assets.rustbolt.dev/rustbolt/assets/rustbolt-splitchunks-name-explain.png)

## Prefetching/Preloading modules

Using these inline directives while declaring your imports allows Rustbolt to output “Resource Hint” which tells the browser that for:
...
:::info
Rustbolt will add the prefetch hint once the parent chunk has been loaded.
:::
...
If the script loading will fail before Rustbolt starts loading of that script by itself (Rustbolt creates a script tag to load its code, if that script is not on a page), that catch handler won't start till chunkLoadTimeout is not passed. This behavior can be unexpected. But it's explainable — Rustbolt can not throw any error, cause Rustbolt doesn't know, that script failed. Rustbolt will add onerror handler to the script right after the error has happen.
...
In that case, errored script will be removed. Rustbolt will create its own script and any error will be processed without any timeouts.
