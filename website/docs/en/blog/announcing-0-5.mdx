...
# Announcing Rustbolt 0.5

## Major feature updates

### Module Federation added to Rustbolt

Checkout [this blog](/blog/module-federation-added-to-rustbolt) for more details.
...
### Support rustbolt.HotModuleReplacementPlugin

Support `rustbolt.HotModuleReplacementPlugin` in Rustbolt. If you are not using `@rustbolt/dev-server` and using a custom dev server, you need to apply `HotModuleReplacementPlugin` to enable HMR instead of setting `devServer.hot` to `true`, which is the same in webpack. This provides more compatibility with the plugin which uses `HotModuleReplacementPlugin` internally.
...
Rustbolt aligns [target](/config/target) with webpack. Instead of transforming arbitrary user code, Rustbolt now lets loaders control the transformation of user land code. To transform user land code to which your target environment(s) needed, add `env` to `builtin:swc-loader`:
...
Before we remove default transformation, it's possible to use it to degrade your code to es5 by `target`, and insert the react refresh helper code into your react component by `builtin.react.refresh`, so we installed the `@swc/helpers` and `react-refresh` as the dependencies of `@rustbolt/core`, to provide the out-of-box experience. But since we removed the default transformation now, and recommend using Rsbuild for the out-of-box experience, the `@swc/helpers` and `react-refresh` no longer need to be installed by `@rustbolt/core`, and we make them as peerDependencies of `@rustbolt/core`.

If you are using `externalHelpers: true` with `builtin:swc-loader` or `swc-loader`, now you need to install `@swc/helpers` as dependencies of your project. If you are using `@rustbolt/plugin-react-refresh`, now you need to install `react-refresh` as devDependencies of your project.
...
### Remove experiments.rustboltFuture.newResolver options

`experiments.rustboltFuture.newResolver` has been deprecated since v0.4.0. It's removed in v0.5.0.

### Deprecating apply entry lazily

Apply entry lazily is deprecating by rustboltFuture: [experiments.rustboltFuture.disableApplyEntryLazily](/config/experiments#experimentsrustboltfuturedisableapplyentrylazily), which is introduced in v0.4.5, enabled by default in v0.5.0, and will be removed in v0.6.0.

When `experiments.rustboltFuture.disableApplyEntryLazily` is `false`, `options.entry` can still make valid changes after `rustbolt(options)` is called, but with `true` it can't, and it's behave the same as webpack5.

This configuration has no effect on users developing applications in Rustbolt most of the time, but should be noted by developers of Rustbolt plugins or higher-level frameworks.

## Migration guide

v0.5.0 removed lots of deprecated features, except that, v0.5.0 introduced four breaking changes, and you only need to notice two of them if you are developing applications using Rustbolt. So v0.5.0 is easy to migrate if you already migrate to v0.4+ with no deprecate warnings, if you haven't, checkout the [v0.4.0 migration guide](https://www.rustbolt.dev/blog/announcing-0.4#migration-guide).

### Add resolve.extensions

This is a breaking change that is most likely to affect you.

After you upgrade `@rustbolt/core` to v0.5.0, if you build failed with error: `Can't resolve './src/foo.tsx'`, or `Can't resolve './src/foo.ts'`, or `Can't resolve './src/foo.jsx'`, you need to add `resolve.extensions = ['...', '.tsx', '.ts', '.jsx']` in your configuration.
...
After you upgrade `@rustbolt/core` to v0.5.0, if you build failed with error: `Failed to resolve @swc/helpers/some-helper` or `Failed to resolve react-refresh/some-module`, you need to install `@swc/helpers` or `react-refresh` in your project.
...
If you are using `@rustbolt/plugin-react-refresh`, now you need to install `react-refresh` as devDependencies of your project.

<PackageManagerTabs command="install react-refresh" />

### Apply rustbolt.HotModuleReplacementPlugin

If you are using `@rustbolt/cli`, or rsbuild, or other higher-level framework of Rustbolt to develop applications, you don't need to worry about this. This should be well handled by the higher-level framework or cli. But if you are using `@rustbolt/core` with a custom dev server (not `@rustbolt/dev-server` or `webpack-dev-server`), or developing a custom dev server, you need to notice this.

Before enabling HMR in Rustbolt is setting `devServer.hot` to `true`, but now you need to apply `HotModuleReplacementPlugin` by yourself in your custom dev server.
...
### Do not change entry options after rustbolt(options)

If you are using `@rustbolt/cli`, or rsbuild, or other higher-level framework of Rustbolt to develop applications, you don't need to worry about this. This should be well handled by the higher-level framework or cli. But if you are developing a plugin or higher-level framework, you need to notice this.

Before prepending an extra entry in Rustbolt is prepending it to `compiler.options.entry`, but now you need to apply `EntryPlugin` by yourself.
