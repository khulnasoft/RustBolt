...
# Announcing Rustbolt 1.2

![Rustbolt 1.2](https://assets.rustbolt.dev/rustbolt/rustbolt-banner-v1-2.png)
...
Rustbolt v1.2 has been released!
...
Rustbolt v1.2 introduced an experimental cache configuration that supports persistent caching, which can significantly improve hot startup speed.
...
Rustbolt has added support for [Yarn PnP](https://yarnpkg.com/features/pnp), which is enabled by default based on `process.versions.pnp` (that is, when the application is running in a Yarn PnP environment), and can also be forced to enable by configuring `resolve.pnp` to `true`.
...
Special thanks to [@arcanis](https://x.com/arcanis), the lead maintainer for Yarn, for implementing PnP resolution in the Rustbolt resolver.
...
In previous versions of Rustbolt, the code splitting would take up a lot of time under HMR. In Rustbolt v1.2, we implemented a new code splitting algorithm that supports multithreading and more efficient incremental rebuilds. If your code base contains a lot of dynamic imports, and code splitting will take a lot of time. Enabling this new feature can significantly improve the performance of code splitting.
...
Rustbolt v1.2 no longer watching the `node_modules` directory by default. This can greatly reduce the number of files to watch and improve performance.

According to our [benchmark repo](https://github.com/rustbolt-contrib/build-tools-performance), this change will:
...
We have optimized the data structure used to store strings during the [rustbolt-sources](https://github.com/khulnasoft/rustbolt-sources) computation process. Throughout the computation, all string data points to the string heap memory of the root node, effectively avoiding the generation of new string allocations during the computation.

> See [perf: reduce memory consumption of CachedSource](https://github.com/khulnasoft/rustbolt/pull/8666) for more details.

### Smaller minification size

Rustbolt v1.2 set default SWC minimizer `passes` to `2` to reduce bundle size by 1%-7%.
...
[passes](https://swc.rs/docs/configuration/minification#jscminifycompress) is the the maximum number of times to run compress. In some cases, more than one pass leads to further compressed code. Given Rustbolt's inherent speed, we've determined that using `2` passes by default strikes an optimal balance between build performance and bundle size.

> See [feat: set default SWC minimizer passes to 2 to reduce bundle size](https://github.com/khulnasoft/rustbolt/pull/8853) for more details.
...
> See [perf: parallelize side effects optimization](https://github.com/khulnasoft/rustbolt/pull/8781) for more details.

## Ecosystem

### Angular support

Nx team core member [Colum Ferry](https://github.com/Coly010) has brought complete Angular support to the Rustbolt ecosystem.

With the newly released `@ng-rsbuild/plugin-angular` and `@ng-rustbolt/build` packages, developers can now use Rustbolt or Rsbuild to build Angular applications, with faster build performance and module federation support.

Please visit [Angular Rustbolt](https://github.com/nrwl/angular-rustbolt) for more information.

### Rsbuild v1.2

Rsbuild v1.2 has been released alongside Rustbolt v1.2, bringing several new features:
...
> For more details, see [Rsbuild v1.2.0](https://github.com/khulnasoft/rsbuild/releases/tag/v1.2.0).

## Upgrade guide

### Upgrade SWC plugins

In Rustbolt v1.2, the Rust crate `swc_core` has been upgraded to `10.1.0`. Users of the SWC Wasm plugin need to ensure version consistency with `swc_core` being used, otherwise, it may lead to unforeseen issues.
