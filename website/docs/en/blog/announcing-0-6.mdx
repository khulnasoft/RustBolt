...
# Announcing Rustbolt 0.6

## Major feature updates

### Built-in support for mini-css-extract-plugin

Now you can use `rustbolt.CssExtractRustboltPlugin` as a replacement for `mini-css-extract-plugin`.
...
For more details, please see [CssExtractRustboltPlugin](/plugins/rustbolt/css-extract-rustbolt-plugin#cssextractrustboltplugin).
...
> There is an basic [project example](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/react-with-extract-css).

### Enable new tree shaking by default

In Rustbolt 0.1.0, basic tree shaking functionality was introduced. Due to the initial architecture being unstable, we employed a relatively simplistic approach to implement the basic version of tree shaking (only support unused export variables elimination). However, as rustbolt's capabilities improved, the architecture gradually stabilized.
...
To address these issues, we decided to adopt a webpack-like approach, re-implementing the entire optimization process from the bottom up. In version 0.4.2, we introduced the `experiments.rustboltFuture.newTreeshaking` configuration to experimentally enable the new optimization algorithm.
...
### Remove experiments.rustboltFuture.disableApplyEntryLazily

The `experiments.rustboltFuture.disableApplyEntryLazily` option has been enabled by default since v0.5.0 and was removed in v0.6.0.
...
Also, starting from v0.6.0, Rustbolt's experiments CSS will align with webpack's experiments CSS as a target, which means that, like webpack experiments CSS, it will no longer support [browsers that do not support CSS variables](https://caniuse.com/css-variables) in the future. Therefore, for those projects that need to use configurations not yet supported by experiments CSS, or need to support older browsers, we recommend migrating to [`rustbolt.CssExtractRustboltPlugin`](/plugins/rustbolt/css-extract-rustbolt-plugin.html).

In v0.6.0, we introduced three new types of `module.generator` and `module.parser` options: `css/auto`, `css`, and `css/module`, which will only take effect when experiments.css is enabled, checkout [this example](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/css-parser-generator-options) about how to use it.
...
```bash
WARNING in ⚠ chunk src_a_css-src_b_-5c8c53 [css-extract-rustbolt-plugin]
...
### Apply rustbolt.CssExtractRustboltPlugin

If you have used `mini-css-extract-plugin` and webpack before, you can simply replace `mini-css-extract-plugin` by `rustbolt.CssExtractPlugin`.
