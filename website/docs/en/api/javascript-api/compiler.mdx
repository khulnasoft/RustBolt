...
2. Rustbolt's cache invalidation detection relies on the `modifiedFiles` and `removedFiles` parameters. When caching is enabled and you're using a custom watcher to watch file changes, you need to pass these values to Rustbolt via the `options` parameter.
...
Allows running another instance of Rustbolt inside of Rustbolt. However, as a child with different settings and configurations applied. It copies all hooks and plugins from the parent (or top-level compiler) and creates a child `Compiler` instance. Returns the created `Compiler`.
...
- **Type:** `typeof rustbolt`

Get the exports of @rustbolt/core to obtain the associated internal objects. This is especially useful when you cannot directly reference `@rustbolt/core` or there are multiple Rustbolt instances.

A common example is accessing the [sources](/api/javascript-api/index#sources-object) object in a Rustbolt plugin:

```js
const { RawSource } = compiler.rustbolt.sources;
...
Equivalent to `compiler.rustbolt`, this property is used for compatibility with webpack plugins.

If the Rustbolt plugin you are developing needs to be webpack compatible, you can use this property instead of `compiler.rustbolt`.

```js
console.log(compiler.webpack === compiler.rustbolt); // true
```
...
- Created through `rustbolt({})`, it is [context configuration](/config/context).
...
- **Type:** `RustboltOptionsNormalized`
