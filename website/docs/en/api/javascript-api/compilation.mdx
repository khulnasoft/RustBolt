...
import RustboltErrorType from '../../types/rustbolt-error.mdx';
...
:::warning Notice
In Rustbolt, the real compilation object runs on the Rust side, and the JavaScript compilation object is only a **proxy object** used to communicate with the Rust compilation object.
...
createChildCompiler(
  ...
  plugins: RustboltPluginInstance[] // Plugins that will be applied
): Compiler;
```
...
The normalized options used by this Compilation, see [Configure Rustbolt](/config/index) for details.
