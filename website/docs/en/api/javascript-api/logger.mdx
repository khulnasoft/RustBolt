...
Rustbolt logger is available to [loaders](/guide/features/loader) and [plugins](/guide/features/plugin). Emitting as part of the [Stats](/api/javascript-api/stats-json) and configured by the user in [rustbolt configuration](/config/).

Benefits of custom logging API in Rustbolt:
...
- CLI, UI tools for Rustbolt may choose different ways to display logging
- Rustbolt core can emit logging output, e.g. timing data

By introducing Rustbolt logging API we hope to unify the way Rustbolt plugins and loaders emit logs and allow better ways to inspect build problems. Integrated logging solution supports plugins and loaders developers by improving their development experience. Paves the way for non-CLI Rustbolt solutions like dashboards or other UIs.
