...
Tells Rustbolt that this loader will be called asynchronously. Returns [this.callback](#thiscallback).
...
Emit an error. Unlike `throw` and `this.callback(err)` in the loader, it does not mark the current module as a compilation failure, it just adds an error to Rustbolt's Compilation and displays it on the command line at the end of this compilation.
...
:::tip
The parameter `schema` is optional and will not be used in Rustbolt.

To provide the best performance, Rustbolt does not perform the schema validation. If your loader requires schema validation, please call [scheme-utils](https://github.com/webpack/scheme-utils) or other schema validation libraries.
:::
...
The value of [`mode`](/config/mode) is read when Rustbolt is run.
...
The base path configured in Rustbolt config via [context](/config/context).
...
:::warning
Please note that using internal Rustbolt properties like `this._compiler` and `this._compilation` will cause your loader to lose its independence.

Ideally, loaders should focus on file transformation logic, with deterministic output for given input, without depending on Rustbolt's internal state. Relying on these internal objects introduces unpredictable behavior, making testing and maintenance more difficult.
...
Access to the current [Compiler](/api/javascript-api/compiler) object of Rustbolt.

### this.\_compilation

- **Type:** `Compilation`

Access to the current [Compilation](/api/javascript-api/compilation) object of Rustbolt.
