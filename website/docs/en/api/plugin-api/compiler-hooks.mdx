...
<Mermaid title="rustbolt()" style={{width: '20rem'}}>
{`
flowchart TD
  CallRustbolt("rustbolt()") --> CreateCompiler("new Compiler()")
  ...
  HookAfterEnvironment --> ApplyRustboltPlugins(Apply internal plugins)
  ApplyRustboltPlugins <--> HookEntryOptions(<a href="#entryoption">hooks.entryOption</a>)
  ApplyRustboltPlugins --> HookAfterPlugins(<a href="#afterplugins">hooks.afterPlugins</a>)
...
class CallRustbolt flow-start
class compiler flow-end
class CreateCompiler,ApplyNodeEnvPlugin,ApplyDefaultOptions,ApplyCustomPlugins,ApplyRustboltPlugins,ResolveOptions flow-process
...
Called after the [`entry`](/config/entry) configuration from Rustbolt options has been processed.
...
This hook is only triggered when calling [compiler.run()](/api/javascript-api/index#compilerrun) (which is used by the `rustbolt build` command), and will not be executed in watch mode. You can use the [watchRun](#watchrun) hook in watch mode.
...
In the make phase, Rustbolt will build the module graph starting from the entry, and use the loader to handle each module.
...
In the make phase, Rustbolt builds the module graph starting from the entry and uses loaders to handle each module. This hook is called when that process completes.
...
In the seal phase, Rustbolt will create chunk graph from the module graph and then generate the assets.
