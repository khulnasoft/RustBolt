...
:::tip
In minor releases, Rustbolt may make changes to the APIs of these experimental features and provide detailed explanations of these changes in the release notes. So if you are using experimental features, please pay attention to the minor release notes.
:::
...
Once enabled, Rustbolt will output ECMAScript module syntax whenever possible. For instance, `import()` to load chunks, ESM exports to expose chunk data, among others.
...
Use defaults of the next major Rustbolt and show warnings in any problematic places.
...
- [Client for web](https://github.com/khulnasoft/rustbolt/blob/699229b9e7c33b7db7968c2f803f750e0367fe8a/packages/rustbolt/hot/lazy-compilation-web.js)
- [Client for node](https://github.com/khulnasoft/rustbolt/blob/699229b9e7c33b7db7968c2f803f750e0367fe8a/packages/rustbolt/hot/lazy-compilation-node.js)
...
If you do not use Rustbolt's own dev server and instead use your own server as the dev server, you generally need to add another client modules in the entry configuration to enable capabilities such as HMR. It is best to exclude these client module from lazy compilation by configuring `test`.
...
const compiler = rustbolt(options);
...
Currently, incremental for the `make` and `emitAssets` stages is enabled by default. This is also the default behavior since Rustbolt v1.0. As this feature further stabilizes, we will enable incremental for more stages by default.

:::tip
This feature is experimental. You can check its relevant progress at [rustbolt#8106](https://github.com/khulnasoft/rustbolt/issues/8106). You can also report bugs and any related feedback here.
:::
...
Enabling this configuration will activate a new multi-threaded code splitting algorithm. If your project includes many dynamic imports, this can greatly reduce the time spent on the code splitting process. In the future, Rustbolt will have this enabled by default.
...
Used to control whether to enable Rustbolt future default options, check out the details [here](https://github.com/khulnasoft/rustbolt/pull/4107).
...
Used to inject the currently used Rustbolt information into the generated asset:

- `version`: Used to specify the Rustbolt version, defaults to the `version` field in `@rustbolt/core/package.json`.
- `bundler`: Used to specify the name of the packaging tool, defaults to `rustbolt`.
- `force`: Whether to force the injection of Rustbolt information, which will be added to chunk as a runtime module, and defaults to `true` to force injection. An array can be used to select the items to be forced injected.

#### Disable default injection

The default injection can be disabled by setting `force` to `false`. Then injection will only occur when `__rustbolt_version__` and `__rustbolt_unique_id__` are detected in the code:

- [`__rustbolt_version__`](/api/runtime-api/module-variables#__rustbolt_version__): Inject version information.
- [`__rustbolt_unique_id__`](/api/runtime-api/module-variables#__rustbolt_unique_id__): Inject the unique ID of the bundler.

```js title="rustbolt.config.mjs"
export default {
  experiments: {
    rustboltFuture: { bundlerInfo: { force: false } },
  },
};
```
...
`cache.buildDependencies` is an array of files containing build dependencies, Rustbolt will use the hash of each of these files to invalidate the persistent cache.

:::tip
It's recommended to set `cache.buildDependencies`: [__filename] in your rustbolt configuration to get the latest configuration.
:::
...
In addition to [buildDependencies](#cachebuilddependencies) and [version](#cacheversion) configurations that affect persistent cache invalidation, Rustbolt also invalidates persistent cache when the following fields change.
...
- **Default:** `{ type: 'filesystem', directory: 'node_modules/.cache/rustbolt' }`

Configure cache storage. Currently only file system storage is supported. The cache directory can be set through `directory`. The default is `node_modules/.cache/rustbolt`.
...
export default {
  ...
  experiments: {
    cache: {
      ...
      storage: {
        type: 'filesystem',
        directory: 'node_modules/.cache/rustbolt',
      },
    },
  },
};
...
:::tip
Rustbolt will generate a cache folder in the `storage.directory` based on [config.name](/config/other-options#name), [config.mode](/config/mode#mode), the file contents in [buildDependencies](#cachebuilddependencies) and [version](#cacheversion).

Rustbolt will automatically clean up cache folders that have not been accessed for a long time (7 days) at startup.
:::

### Migrating from webpack config

The Rustbolt cache configuration is different from the webpack cache configuration. You can refer to the following steps to migrate the webpack cache configuration.

1. According to the cache type, set the Rustbolt cache type. Continue with the next step for persistent cache, and stop here for other types of cache.
...
```js
function transform(webpackConfig, rustboltConfig) {
  rustboltConfig.experiments = rustboltConfig.experiments || {};
  ...
  rustboltConfig.experiments.cache.storage = {
    type: 'filesystem',
...
After enabling this feature, Rustbolt can build remote resources that start with the `http(s):` protocol. Rustbolt will download the resources to the local machine and then bundle them.

By default, Rustbolt will generate `rustbolt.lock` and `rustbolt.lock.data` in the [context](/config/context) folder as the locations of the Lockfile and the cache respectively. You can also configure them through `lockfileLocation` and `cacheLocation`.
