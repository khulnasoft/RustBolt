...
- `eval` wraps the module generated code with `eval()`, so Rustbolt can internally cache the module generated results, so when `eval` is used in combination with `source-map`, it optimizes the speed of Source Map generation when rebuilding.
