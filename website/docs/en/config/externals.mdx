...
It might be useful to define your own function to control the behavior of what you want to externalize from Rustbolt. [webpack-node-externals](https://www.npmjs.com/package/webpack-node-externals), for example, excludes all modules from the `node_modules` directory and provides options to allowlist packages.
...
Specify the default type of externals as `'commonjs'`. Rustbolt will generate code like `const X = require('...')` for externals used in a module.
...
Specify the default type of externals as `'global'`. Rustbolt will read the external as a global variable on the [`globalObject`](/config/output#outputglobalobject).
...
Specify the default type of externals as `'module'`. Rustbolt will generate code like `import * as X from '...'` for externals used in a module.

Make sure to enable [`experiments.outputModule`](/config/experiments#experimentsoutputmodule) first, otherwise Rustbolt will throw errors.
...
Specify the default type of externals as `'import'`. Rustbolt will generate code like `import('...')` for externals used in a module.
...
Specify the default type of externals as `'module-import'`. This combines [`'module'`](#externalstypemodule) and [`'import'`](#externalstypeimport). Rustbolt will automatically detect the type of import syntax, setting it to `'module'` for static imports and `'import'` for dynamic imports.

Make sure to enable [`experiments.outputModule`](/config/#experimentsoutputmodule) first if static imports exist, otherwise Rustbolt will throw errors.
...
When a module is not imported via `import` or `import()`, Rustbolt will use `"module"` externals type as fallback. If you want to use a different type of externals as fallback, you can specify it with a function in the `externals` option. For example:
...
Specify the default type of externals as `'commonjs-import'`. This combines [`'commonjs'`](#externalstypecommonjs) and [`'import'`](#externalstypeimport). Rustbolt will automatically detect the type of import syntax, setting dynamic import to `'import'` and leaving others to `'commonjs'`.
...
:::note
`commonjs-import` type is only available of Rustbolt, and not applicable for webpack.
:::
...
Specify the default type of externals as `'node-commonjs'`. Rustbolt will import [`createRequire`](https://nodejs.org/api/module.html#module_module_createrequire_filename) from `'module'` to construct a require function for loading externals used in a module.
...
Specify the default type of externals as `'promise'`. Rustbolt will read the external as a global variable (similar to [`'var'`](#externalstypepromise)) and `await` for it.
...
Specify the default type of externals as `'self'`. Rustbolt will read the external as a global variable on the `self` object.
...
Specify the default type of externals as `'script'`. Rustbolt will load the external as a script exposing predefined global variables with HTML `<script>` element. The `<script>` tag would be removed after the script has been loaded.
...
:::tip
When loading code with HTML `<script>` tags, the Rustbolt runtime will try to find an existing `<script>` tag that matches the `src` attribute or has a specific `data-webpack` attribute. For chunk loading `data-webpack` attribute would have value of `'[output.uniqueName]:chunk-[chunkId]'` while external script has value of `'[output.uniqueName]:[global]'`.
...
Specify the default type of externals as `'this'`. Rustbolt will read the external as a global variable on the `this` object.
...
Specify the default type of externals as `'var'`. Rustbolt will read the external as a global variable.
...
Specify the default type of externals as `'window'`. Rustbolt will read the external as a global variable on the `window` object.
...
Note that if you're going to output ES modules with those node.js-related presets, Rustbolt will set the default `externalsType` to [`node-commonjs`](#externalstypenode-commonjs) which would use `createRequire` to construct a require function instead of using `require()`.
