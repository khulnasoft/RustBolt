...
The `entry` configuration is used to set the entry module for Rustbolt builds.
...
If `entry` is set as a relative path, Rustbolt will use the value set by [context configuration](/config/context.html) as the base path, which by default is the current working directory of the Node.js process, namely `process.cwd ()`.
