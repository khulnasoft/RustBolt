...
Rustbolt will select appropriate optimization configuration based on the [`mode`](/config/mode). You can also customize the configuration via [`optimization`](/config/optimization).
...
Tells Rustbolt which algorithm to use when generating module ids.
...
Tells Rustbolt which algorithm to use when generating chunk ids.
...
<PropertyType
  ...
  defaultValueList={[
    {
      defaultValue:
        '[new SwcJsMinimizerRustboltPlugin(), new LightningCssMinimizerRustboltPlugin()]',
    },
  ]}
/>

Customize the minimizer. By default, [`rustbolt.SwcJsMinimizerRustboltPlugin`](/plugins/rustbolt/swc-js-minimizer-rustbolt-plugin) and [`rustbolt.LightningCssMinimizerRustboltPlugin`](/plugins/rustbolt/lightning-css-minimizer-rustbolt-plugin) are used.
...
Use Rustbolt's built-in minimizer with custom options:
...
Used to control how the Rustbolt's [runtime](/misc/glossary#runtime) chunk is generated.
...
Rustbolt supports splitting chunks with the `optimization.splitChunks` configuration item.
...
If you only want Rustbolt use the manual `sideEffects` flag via (`package.json` and `module.rule.sideEffects`) and don't analyse source code:
...
:::tip
When `optimization.sideEffects` is true , Rustbolt will also flag modules as side effect free when they contain only side effect free statements.
:::
...
Tells Rustbolt to determine used exports for each module. This depends on `optimization.providedExports`.
...
`optimization.innerGraph` tells Rustbolt whether to perform a more detailed analysis of variable assignments. This helps Rustbolt to identify unused module exports, thereby reducing the size of the bundled output.
...
Tells Rustbolt to set `process.env.NODE_ENV` to a given string value. `optimization.nodeEnv` uses [DefinePlugin](/plugins/webpack/define-plugin) unless set to false.
...
Use `optimization.avoidEntryIife` to avoid wrapping the entry module in an IIFE when it is required (search for `"This entry needs to be wrapped in an IIFE because"` in [rustbolt_plugin_javascript](https://github.com/khulnasoft/rustbolt/blob/main/crates/rustbolt_plugin_javascript/src/plugin/mod.rs)). This approach helps optimize performance for JavaScript engines and helps tree shaking when building ESM libraries.
