...
- Using `@rustbolt/cli`, `rustbolt build --json stats.json`.
- Using Rustbolt's JavaScript API, `stats.toJson(options)`, `stats.toString(options)`.
...
A fallback value for stats options when an option is not defined. It has precedence over local Rustbolt defaults.
...
When `stats.entrypoints` is set to `'auto'`, Rustbolt will decide automatically whether to display the entry points in the stats output.
...
> The runtime modules are built-in modules of Rustbolt, used to provide various runtime capabilities.
...
Defaults to `true` when executing `rustbolt build` in an environment that supports color output.
...
Whether to add information about the Rustbolt version used.
