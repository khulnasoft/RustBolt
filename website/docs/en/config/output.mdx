...
The top-level output key contains a set of options instructing Rustbolt on how and where it should output your bundles, assets, and anything else you bundle or load with Rustbolt.
...
:::tip
Although the `charset` attribute for `<script>` tag was [deprecated](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/script#Charset), rustbolt still adds it by default for compatibility with non-modern browsers.
:::
...
Note that these filenames need to be generated at runtime to send the requests for chunks. Because of this, placeholders like `[name]` and `[chunkhash]` need to add a mapping from chunk id to placeholder value to the output bundle with the Rustbolt runtime. This increases the size and may invalidate the bundle when placeholder value for any chunk changes.
...
The default value of this option depends on the [`target`](/config/target) and [`output.module`](#outputmodule) setting. For more details, search for "chunkFormat" [in the Rustbolt defaults](https://github.com/khulnasoft/rustbolt/blob/main/packages/rustbolt/src/config/defaults.ts).
...
The default value of this option depends on the [`target`](/config/target) and [`chunkFormat`](#chunkFormat) setting. For more details, search for `"chunkLoading"` [in the Rustbolt defaults](https://github.com/khulnasoft/rustbolt/blob/main/packages/rustbolt/src/config/defaults.ts).
...
Tells Rustbolt to check if to be emitted file already exists and has the same content before writing to the output file system.

:::warning
Rustbolt will not write output file when file already exists on disk with the same content.
:::
...
If `target` is `'web'`, Rustbolt will dynamically create `<script>` and `<link>` tags to load asynchronous JavaScript and CSS resources. Rustbolt will add the `crossorigin` attribute to the `<script>` and `<link>` tags if the URLs of these resources are on other domains and `crossOriginLoading` is not `false`.
...
When Rustbolt dynamically loads JavaScript resources, it will generate the following HTML:
...
List of chunk loading types enabled for use by entry points. Will be automatically filled by Rustbolt. Only needed when using a function as entry option and returning chunkLoading option from there.
...
Rustbolt performs code splitting optimizations on user input code, which may include, but are not limited to, code splitting, bundle splitting, or splitting implemented through other plugins. These splitting actions can result in multiple bundles being generated, so the filenames of the bundles need to be generated dynamically.
...
:::tip
Rustbolt uses the faster `xxhash64` algorithm by default since v1.1.
:::
...
Tells Rustbolt to add [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) wrapper around emitted code.
...
Now the variable `MyLibrary` will be bound with the exports of your entry file, and here's how to consume the Rustbolt bundled library:

```html
<script src="https://example.org/path/to/my-library.js"></script>
<script>
  MyLibrary.hello('rustbolt');
</script>
```

In the above example, we're passing a single entry file to `entry`, however, Rustbolt can accept [many kinds of entry point](/config/entry), e.g., an `array`, or an `object`.
...
2. If an `object` is provided as the `entry` point, all entries can be exposed using the `array` syntax of `library`:
   ...
   <script>
     MyLibrary.a.hello('rustbolt');
     MyLibrary.b.hello('rustbolt');
   </script>
   ```
...
:::tip
Wondering the difference between CommonJS and CommonJS2 is? While they are similar, there are some subtle differences between them that are not usually relevant in the context of Rustbolt. (For further details, please [read this issue](https://github.com/webpack/webpack/issues/1114).)
:::
...
System modules require that a global variable `System` is present in the browser when the Rustbolt bundle is executed. Compiling to `System.register` format allows you to `System.import('/bundle.js')` without additional configuration and has your Rustbolt bundle loaded into the System module registry.
...
When enabled, Rustbolt will set [`output.iife`](#outputiife) to `false`, [`output.scriptType`](#outputscripttype) to `'module'` and `terserOptions.module` to `true` internally.

If you're using Rustbolt to compile a library to be consumed by others, make sure to set [`output.libraryTarget`](#librarytarget-module) to `'module'` when `output.module` is `true`.
...
:::warning
The path must not contain an exclamation mark (`!`) as it is reserved by Rustbolt for loader syntax).
:::
...
Tells Rustbolt to include comments in bundles with information about the contained modules. This option defaults to `true` in `development` and `false` in `production` [mode](/config/mode) respectively. `'verbose'` shows more information like exports, runtime requirements and bailouts.
...
:::tip
The default value of this option depends on the [`target`](/config/target) setting. For more details, search for `"workerChunkLoading"` [in the Rustbolt defaults](https://github.com/khulnasoft/rustbolt/blob/main/packages/rustbolt/src/config/defaults.ts).
:::
