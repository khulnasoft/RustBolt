...
:::warning
**This option is not supported via the JavaScript API**: `extends` will have no effect when using the JavaScript API. `@rustbolt/cli` is required to use this feature.
:::
...
export default {
  extends: './base.rustbolt.config.mjs',
...
module.exports = {
  extends: './base.rustbolt.config.cjs',
  // Override or add to the base configuration
  output: {
    filename: '[name].bundle.js',
  },
...
```js title="rustbolt.config.mjs"
export default {
  extends: ['./base.rustbolt.config.mjs', './dev.rustbolt.config.mjs'],
...
You can also extend configurations from packages installed in your node_modules. The package should export a valid Rustbolt configuration:

```js title="rustbolt.config.mjs"
export default {
  extends: 'some-rustbolt-config-package',
...
```js title="base.rustbolt.config.mjs"
export default {
  extends: './core.rustbolt.config.mjs',
