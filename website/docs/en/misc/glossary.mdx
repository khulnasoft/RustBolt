...
Below are some common terms used with Rustbolt and webpack.
...
In bundling terminology, a chunk is a group of modules that get combined into a single data file. Rustbolt will bundle the modules that are related to each other into a chunk, and then generate a corresponding file.
...
In Rustbolt, first-class module types refer to module types that can be supported without relying on loaders or plugins, such as JavaScript, CSS, JSON, Assets, etc. However, module types that require loaders or plugins to support, such as TypeScript, HTML, Markdown, YAML, etc., are not first-class citizen modules.
...
A module's type determines how it will be parsed and handled by the bundler. For example, we can tell Rustbolt that the module is a JavaScript module by specifying the module type as JavaScript, and Rustbolt will use the JavaScript parser to parse the module. If the specified module type is CSS, then Rustbolt will use a CSS parser to parse the module.
...
Module resolution is the process of calculating the file path indicated by a module specifier. For example, an import statement includes a module specifier, and Rustbolt will use the module resolution algorithm to find the corresponding file path.
...
A plugin is a program module that can be used to extend the functionality of Rustbolt by means of extensibility hooks. It can be used to customize the build process, or to integrate with other tools. Rustbolt provides lots of hooks which you can use to customize the build process.

## runtime

The runtime is all the code Rustbolt needs to connect your modularized application while it's running in the browser or other environments. It contains the loading and resolving logic needed to connect your modules as they interact.
