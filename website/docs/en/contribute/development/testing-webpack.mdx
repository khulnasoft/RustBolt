...
Based on implementation differences and performance considerations, Rustbolt will integrate some webpack plugins internally. The test suite for the plugins will also be copied to the [rustbolt-plugin-ci](https://github.com/rustbolt-contrib/rustbolt-plugin-ci) for testing plugin compatibility.
...
- When a new webpack plugin is integrated into Rustbolt, the test cases for that plugin need to be copied.
- When there are differences between the artifacts of Rustbolt and webpack (e.g., different hashes), some test cases may need modification.

In scenarios other than those mentioned above, please follow the [Rustbolt Testing](./testing-rustbolt) guidelines for adding test cases.
