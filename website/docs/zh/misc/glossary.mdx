...
Chunk 是一组绑定在一起的模块。rustbolt 会将相互关联的模块打包成一个 chunk，然后生成对应的文件。
...
Rustbolt 中的一等公民模块指的是那些不需要依赖 loader 和 plugin 即可支持的模块类型，例如 JavaScript、CSS、JSON、资源等，而像 TypeScript、HTML、Markdown、YAML 等需要依赖 loader 和 plugin 才能支持的模块类型则不是一等公民模块。
...
模块类型是模块的一种属性，它们可以通过不同的方式进行解析和处理。我们可以通过指明模块的模块类型来告诉 Rustbolt 如何处理它们。例如，我们可以通过指定模块类型为 JavaScript 来告诉 Rustbolt 该模块是一个 JavaScript 模块，然后 Rustbolt 就会使用 JavaScript 解析器来解析该模块，如果指定的模块类型为 CSS，那么 Rustbolt 就会使用 CSS 解析器来解析该模块。
...
模块解析是指 Rustbolt 如何找到模块的过程。Rustbolt 会根据模块的路径来解析模块，例如，当我们在代码中使用 `import 'foo'` 时，Rustbolt 就会根据模块的路径来解析模块。
...
插件可以用来扩展 Rustbolt 的功能。它可以用来定制构建过程，或与其他工具集成。Rustbolt 提供了很多钩子，你可以用它们来定制构建过程。

## runtime

runtime（运行时）是指在浏览器或其他环境中运行过程中，Rustbolt 用来连接模块化应用所需的所有代码。它包含了连接模块所需的加载和解析逻辑。
...
Tree shaking 是一种允许你从包中删除未使用代码的技术。它是一种特殊的死代码优化方式。像 Rustbolt 这样的编译器将通过分析代码的静态语法，然后删除未使用的代码来完成此操作。
