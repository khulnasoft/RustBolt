...
3. 通过执行 `pnpm install && pnpm -w build:cli:dev` 构建 `@rustbolt/cli` 和 napi binding
4. 在 VS Code 的 `Run and Debug` 栏中, 选择 `Debug Rustbolt` 开始调试 `@rustbolt/cli` 的启动过程。 该任务可以在 `.vscode/launch.json` 中配置，会同时启动 Node 和 Rust 的调试器。

:::note
其他工具集成的一般是 Rustbolt 的 release 版本，其中并不包含调试信息，这时候需要使用对应的含有debug 的版本进行override替换，方可调试。
:::
...
只需在指定的 Rust 代码处设置断点后启动 `Debug Rustbolt` 即可进行调试。

#### 调试 JavaScript

启动 `Debug Rustbolt` 时，选择 `--inspect` 或 `--inspect-brk` 选项，然后启动 `Attach JavaScript` 并选择相应进程的 PID 即可。

#### 调试已启动的 Rustbolt 进程

当 Rustbolt 被集成到其他框架或工具（如 Nx）中时，可能难以独立使用 Launch 模式启动 Rustbolt。此时，可以通过 attach 模式调试代码。启动 `Attach Rust` 并选择 Rustbolt 所属进程的 PID 即可,启动 `Attach JavaScript` 即可调试 JavaScript。

#### 调试发生死锁的 Rustbolt 进程

当使用 `Attach Rust` 将调试器 attach 到 Rustbolt 所属进程后，点击 Debugger 的 Pause 按钮即可在死锁现场设置断点。
...
使用 `RUSTBOLT_PROFILE` 环境变量来展示 trace 信息。
...
`rustbolt_resolver` 为了调试也会有一些 tracing 信息。
...
例如，`examples/arco-pro` 运行崩溃了并且没有任何信息在[这个修复](https://github.com/khulnasoft/rustbolt/pull/3195/files) 之前:
