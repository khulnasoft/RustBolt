...
基于实现差异和性能考虑，Rustbolt 会将部分 webpack 生态插件内置。同样会将插件的测试套件复制到 [rustbolt-plugin-ci](https://github.com/rustbolt-contrib/rustbolt-plugin-ci) 下，用于测试插件的兼容性。

因此，为了保持这些测试用例与其原仓库的一致性，不建议对测试用例进行修改，仅当如下场景满足时可变更：

- 在 Rustbolt 中内置了新的 webpack 生态插件，此时需要复制该插件的用例。
- 因 Rustbolt 与 webpack 的产物差异（如 hash 不同），需要修改部分用例。

非以上场景时，请遵循 [Rustbolt 测试](./testing-rustbolt) 添加测试用例
