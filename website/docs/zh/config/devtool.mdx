...
Source Map 生成主要有 `source-map`、`eval`、`cheap`、`module`、`inline`、`nosources` 和 `hidden` 这几种风格，它们之间可以进行组合。

- `source-map` 是最基础的行为，表示生成 Source Map，当开启 Source Map 时会对构建性能有部分开销。

- `eval` 会通过 `eval()` 包裹模块生成的代码，因此 Rustbolt 可以在内部对模块的生成结果做缓存，所以当 `eval` 和 `source-map` 组合使用时会优化重新构建时的 Source Map 生成速度。
