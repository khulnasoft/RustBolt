...
详见[源代码](https://github.com/khulnasoft/rustbolt/blob/main/packages/rustbolt/src/config/defaults.ts#L381)。

```js title="rustbolt.config.mjs"
export default {
  module: {
    defaultRules: [
      '...', // 使用 "..." 来引用 Rustbolt 默认规则
    ],
  },
};
```
...
匹配的文件代码不会被 Rustbolt 转换，包括 `module.exports`, `require`, `import` 这些模块系统相关的语法。
...
设置 Rustbolt 对于 `json` 模块导出的分析程度。development 模式下此项默认值为 `1`，可大幅提升构建效率。
...
例如，下面的配置会将 rule 应用于 `package.json` 的 `name` 中包含 `'rustbolt'` 字符串的 JavaScript 资源。
...
需要注意的是，为了让 Rustbolt 能够正常匹配 `with` 语法，当你在使用 [builtin:swc-loader](/guide/features/builtin-swc-loader) 时，需要手动开启 `keepImportAttributes` 配置以保留 `import attributes`：
...
用于标记匹配的模块的类型，这会影响 Rustbolt 内置对于该模块的处理方式。

默认情况下，Rustbolt 会根据文件扩展名来决定模块的类型。例如，`.js` 和 `.mjs` 文件会被当作 `javascript/auto` 模块处理，`.json` 文件会被当作 `json` 模块处理。

例如，如果你想通过一个自定义 Loader 加载 `.json` 文件，你需要将类型设置为 `javascript/auto` 以绕过 Rustbolt 内置的 JSON 导入。
