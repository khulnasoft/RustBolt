...
基础目录：该选项用于设置 Rustbolt 构建时所依赖的基础路径。

`context` 是一个绝对路径，它被用作为 Rustbolt 配置中相对路径的基础路径，比如 [entry](config/entry) 和 [output](config/output) 等配置中包含的相对路径。

默认情况下，Rustbolt 会使用 Node.js 进程的当前工作目录作为基础目录。在大多数情况下，我们推荐手动设置一个基础目录，而不是依赖 Node.js 的当前工作目录。
