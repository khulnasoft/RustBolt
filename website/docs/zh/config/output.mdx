...
顶级的 `output` 包含一组选项，指导 Rustbolt 如何以及在哪里输出 bundles、assets，以及使用 Rustbolt 打包或加载的任何其他内容。
...
:::tip
尽管 `<script>` 标签的 `charset` 属性已被标记为[已废弃的](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/script#Charset)， 为了兼容旧浏览器，在默认情况下 rustbolt 依然会添加该属性。
:::
...
注意，这些文件名需要在运行时生成，以便发送 chunk 的请求。因此，像 `[name]` 和 `[chunkhash]` 这样的占位符需要在 Rustbolt 运行时将 chunk id 映射到占位符值添加到输出 bundle 中。这会增加大小，并且可能会在任何 chunk 的占位符值更改时使 bundle 的缓存失效。
...
:::tip
这个选项的默认值取决于 [`target`](/config/target) 和 [`output.module`](#outputmodule) 设置。要了解更多详情，请在 Rustbolt [默认设置中](https://github.com/khulnasoft/rustbolt/blob/main/packages/rustbolt/src/config/defaults.ts)搜索 `"chunkFormat"`。
:::
...
此选项决定了 Rustbolt 用于加载 chunk 的全局变量。
...
:::tip
这个选项的默认值取决于 [`target`](/config/target) 和 [`output.chunkFormat`](#chunkFormat) 设置。要了解更多详情，请在 Rustbolt [默认设置中](https://github.com/khulnasoft/rustbolt/blob/main/packages/rustbolt/src/config/defaults.ts)搜索 `"chunkLoading"`。
:::
...
告知 Rustbolt 在写入产物文件时检查文件是否已经存在并且拥有相同内容。

:::warning
当在文件系统中已经存在有相同内容的文件时，Rustbolt 将不会写入产物文件。
:::
...
当 `target` 为 `'web'` 时，Rustbolt 会动态创建 `<script>` 和 `<link>` 标签来加载异步的 JavaScript 和 CSS 资源。如果这些资源的 URL 在其他域名下，且 `crossOriginLoading` 不为 `false`，那么 Rustbolt 会为 `<script>` 和 `<link>` 标签添加 `crossorigin` 属性。
...
当 Rustbolt 在加载异步的 JavaScript 资源时，会生成如下的 HTML：
...
此选项确定 [`output.devtoolModuleFilenameTemplate`](/config/output#outputdevtoolmodulefilenametemplate) 使用的模块名称空间。未指定时的默认值为：[`output.uniqueName`](/config/output#outputuniquename)。在加载多个通过 Rustbolt 构建的 library 时，用于防止 source map 中源文件路径冲突。
...
启用用于入口 chunk 加载类型的列表。将由 Rustbolt 自动填充，仅在使用函数作为入口选项并从那里返回 chunkLoading 选项时需要。
...
指示 Rustbolt 可以在生成的运行时代码中使用哪种 ES 特性。
...
然而，当通过多个 Entry、代码拆分或各种插件创建多个包时，你应该使用以下替换之一来给每个包一个唯一的名字...

:::info 对于拆分成多个 bundle 的情况的描述

Rustbolt 会对用户输入的代码进行代码拆分优化，这些优化可能包括但不限于：code splitting、bundle splitting，或通过其他插件实现的代码拆分等。
...
Hash 的长度（`[hash]`、`[contenthash]` 或 `[chunkhash]`）可以使用 `[hash:12]` 来指定（默认为 16）。另外，可以使用 [output.hashDigestLength](#outputhashdigestlength) 来全局配置长度。
...
:::tip
Rustbolt 从 1.1 版本开始默认使用更快的 `xxhash64` 算法。
:::
...
让 Rustbolt 为生成的代码添加 [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) 包装器。
...
此时，变量 `MyLibrary` 将与你的入口文件所导出的文件进行绑定，下面是如何使用 Rustbolt 构建的库的实现：
...
在上面的例子中，我们为 `entry` 设置了一个入口文件，然而 Rustbolt 可以接受 [多个入口](/config/entry)，例如一个 `array` 或者一个 `object`。
...
:::tip
在考虑 CommonJS 与 CommonJS2 之间的区别？虽然它们很相似，但是它们之间有一些细微的差别，这些差别在 Rustbolt 的上下文中通常是不相关的。（要获取更多详细内容，请 [阅读这个 issue](https://github.com/webpack/webpack/issues/1114)。）
:::
...
`amd-require` 类型允许使用 AMD 依赖项，而不需要后续的单独调用。与 `amd` 类型一样，这取决于在加载 Rustbolt 输出的环境中是否有适当的 require 函数可用。
...
当启用时，Rustbolt 会将 [`output.iife`](#outputiife) 设置为 `false`， [`output.scriptType`](#outputscripttype) 设置为 `'module'`。

如果你正在使用 Rustbolt 编译一个供他人使用的库，请确保在 `output.module` 为 `true` 时将 [`output.library.type`](#outputlibrarytype) 设置为 `'module'`。
...
:::warning
路径中不能包含感叹号（!），因为感叹号在 Rustbolt 中被用作 loader 语法。
:::
...
让 Rustbolt 在 bundle 中通过注释输出模块的信息。此选项在 [mode](/config/mode) 为 `'development'` 下默认为 `true`，为 `'production'` 下默认为 `false`。`'verbose'` 会打印更多信息，例如模块导出、运行时信息和 `bailout` 原因。
...
告诉 Rustbolt 当 require 模块出现错误时，移除模块的缓存。
...
控制 [Trusted Types](https://web.dev/trusted-types) 兼容性。启用此选项，Rustbolt 将检测已实现了 Trusted Types，并使用 Trusted Types 来创建它动态加载的脚本 URL。应用在运行带有 [require-trusted-types-for](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/require-trusted-types-for) 内容安全策略指令下的应用程序。

默认情况下是不启用。

- 当设置为 `true` 时，Rustbolt 将使用 [`output.uniqueName`](#outputuniquename) 作为 Trusted Types 策略名称。
...
此选项决定了在全局环境下为防止多个 Rustbolt 运行时 冲突所使用的唯一名称。
...
:::tip
此选项的默认值取决于目标设置。要获取更多信息，请在 Rustbolt [默认设置中](https://github.com/khulnasoft/rustbolt/blob/main/packages/rustbolt/src/config/defaults.ts)搜索 `"workerChunkLoading"`。
:::
...
Rustbolt 会在 CSS 中添加一些元信息，用以解析 CSS modules，此配置决定是否压缩元信息。
