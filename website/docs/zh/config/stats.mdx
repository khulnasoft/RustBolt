...
- 使用 `@rustbolt/cli`，`rustbolt build --json stats.json`。
- 通过 Rustbolt 的 JavaScript API，`stats.toJson(options)`、`stats.toString(options)`。
...
当 `stats.entrypoints` 被设置为 `'auto'` Rustbolt 将自动决定是否在 stats 输出中展示入口信息。
...
> 运行时模块为 Rustbolt 内置模块，用于提供各种运行时能力。
...
在支持颜色输出的环境中执行 `rustbolt build` 时，它被默认设置为 `true`。
