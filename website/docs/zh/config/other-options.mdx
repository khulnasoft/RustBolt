...
:::info
与 webpack 不同，webpack 中 `amd` 选项的默认值为 `{}`， 即默认开启了 AMD 模块的依赖分析。但由于 AMD 模块的使用量正在逐步减少，Rustbolt 将 `amd` 选项的默认值设置为 `false`，即默认关闭了 AMD 模块的依赖分析。如果你的项目需要使用 AMD 模块，可以自行开启该选项。
:::
...
遇到第一个错误时退出。Rustbolt 在默认情况下会在命令行以及在 HMR 时的浏览器 console 中打印这些错误，并且继续编译。
...
:::tip
你可以覆盖 Loader 上下文中的属性，因为 Rustbolt 会将所有定义在 Loader 中的属性负责到 Loader 上下文中。
:::
