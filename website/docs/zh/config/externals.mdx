...
在上面 Rustbolt 配置中，`externals` 下指定的属性名称 `jquery` 表示 `import $ from 'jquery'` 中的模块 `jquery` 应该从打包产物中排除。为了替换这个模块，`jQuery` 值将用于检索全局 `jQuery` 变量，因为默认的外部库类型是 `var`，请参阅 [externalsType](#externalstype)。
...
将 externals 的默认类型指定为 `'commonjs'`。Rustbolt 将为模块中使用的外部生成类似 `const X = require('...')` 的代码。
...
将 externals 的默认类型指定为 `'global'`。Rustbolt 会将 external 视为 [`globalObject`](/config/output#outputglobalobject) 上的一个全局变量读取。
...
将 externals 的默认类型指定为 `'module'`。Rustbolt 将为模块中使用的 externals 生成类似 `import * as X from '...'` 的代码。

确保开启了 [`experiments.outputModule`](/config/experiments#experimentsoutputmodule)，否则 Rustbolt 会抛出错误。
...
将 externals 的默认类型指定为 `'import'`。Rustbolt 将为模块中使用的 externals 生成类似 `import('...')` 的代码。
...
将 externals 的默认类型指定为 `'module-import'`。这将结合 [`'module'`](#externalstypemodule) 和 [`'import'`](#externalstypeimport)。Rustbolt 将自动检测导入语法的类型，对于静态导入设置为 `'module'`，对于动态导入设置为 `'import'`。
...
当一个模块没有通过 `import` 或 `import()` 导入时，Rustbolt 将使用 `"module"` externals type 作为回退。如果你想使用不同类型的 externals 作为回退，你可以在 `externals` 选项中指定一个函数。例如：
...
将 externals 的默认类型指定为 `'commonjs-import'`。这将结合 [`'commonjs'`](#externalstypecommonjs) 和 [`'import'`](#externalstypeimport)。Rustbolt 将自动检测导入语法的类型，对于动态导入设置为 `'import'`，其他的导入设置为 `'commonjs'`。
...
:::note
`commonjs-import` 类型仅在 Rustbolt 中可用，webpack 并不支持此类型。
:::
...
将 externals 类型设置为 `'node-commonjs'`，Rustbolt 将从 `module` 中导入 [`createRequire`](https://nodejs.org/api/module.html#module_module_createrequire_filename) 来构造一个 require 函数，用于加载模块中使用的外部对象。
...
将 externals 的默认类型指定为 `'promise'`，Rustbolt 会将 external 视为全局变量（类似于 [`'var'`](#externalstypepromise)）并 await 它。
...
将外部的默认类型指定为 `'self'`。 Rustbolt 会将 external 作为 `self` 对象上的全局变量读取。
...
将 external 的默认类型指定为 `'script'`。Rustbolt 会使用 HTML `<script>` 标签加载外部资源，暴露预定义的全局变量。脚本加载完成后，`<script>` 标签将被移除。
...
:::tip
当使用 HTML `<script>` 标签加载代码时，Rustbolt 运行时会尝试查找与 `src` 属性匹配或具有特定 `data-webpack` 属性的现有 `<script>` 标签。对于 chunk 加载，`data-webpack` 属性的值将是 `'[output.uniqueName]:chunk-[chunkId]'`，而外部脚本的值将是 `'[output.uniqueName]:[global]'`。
:::

### externalsType.this

将 external 的默认类型指定为 `'this'`。Rustbolt 会将 external 作为 `this` 对象上的全局变量读取。
...
将 external 的默认类型指定为 `'var'`。Rustbolt 会将 external 作为全局变量读取。
...
将 external 的默认类型指定为 `'window'`。Rustbolt 会将 external 作为 `window` 对象上的全局变量读取。
...
请注意，如果你打算输出 ES 模块并使用这些 node.js 相关的预设值，Rustbolt 将会将默认的 `externalsType` 设置为 [`node-commonjs`](#externalstypenode-commonjs)，这将使用 `createRequire` 来构造一个 require 函数，而不是使用 `require()`。
