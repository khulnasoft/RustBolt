...
模式：该选项用于设置 Rustbolt 的构建模式，以启用对应模式下的默认优化策略。

## 用法

你可以在 Rustbolt 配置中直接设置 mode:
...
除此之外，你也可以在 Rustbolt 命令行中通过 `--mode` 选项来设置 mode:
...
:::info
命令行中的 `--mode` 选项优先级高于 Rustbolt 配置中的 `mode`。
:::
...
在 production 模式下，Rustbolt 会自动启用以下优化策略：
...
在 development 模式下，Rustbolt 会自动启用以下优化策略：
...
当 `mode` 被设置为 `'none'` 时，Rustbolt 不会启用任何默认优化策略。
