...
本节涵盖了使用 Rustbolt 编译代码时所有可用的变量。模块将能够通过 `module` 和其他变量访问来自编译过程的特定数据。
...
Rustbolt 会将 global 替换为代理对象并在其中处理兼容性问题。
...
> 请注意，Rustbolt 不支持直接访问 `import.meta`（将抛出警告）。因此，你应该访问其属性或使用解构赋值。
...
`import.meta.webpackContext` 是 Rustbolt 和 webpack 特有的一个函数，它允许你动态地 import 一组模块。

你可以在代码中使用 `import.meta.webpackContext`，Rustbolt 将在构建时进行解析并引用匹配的模块。
...
:::tip
Rustbolt 在编译时，会通过静态分析来解析 `import.meta.webpackContext()` 的参数，因此参数必须是[字面量](https://developer.mozilla.org/en-US/docs/Glossary/Literal)。
...
Rustbolt 能够为其加载的所有脚本添加 nonce，即一次性随机数。在入口文件中设置 `__webpack_nonce__` 变量以激活此功能。
...
生成一个不会被 Rustbolt 解析的 `require` 函数。配合全局可以获取到的 `require` 函数，可以完成一些酷炫操作。
...
<ApiMeta addedVersion="1.0.0" specific={['Rustbolt', 'Webpack']} />
...
### \_\_rustbolt_version\_\_

<ApiMeta specific={['Rustbolt']} />
当前使用的 Rustbolt 版本，默认从 `@rustbolt/core/package.json` 读取。可以通过
`experiments.rustboltFuture.bundlerInfo.version` 修改。

<Columns>
```js title=源码
__rustbolt_version__
```
...
### \_\_rustbolt_unique_id\_\_

<ApiMeta addedVersion="1.0.0" specific={['Rustbolt']} />

当前打包工具的 ID，值为 `{bundler}@{version}`。其中：

- `bundler`：默认为 `"rustbolt"` 可以通过 [experiments.rustboltFuture.bundlerInfo.bundler](/config/experiments#rustboltfuturebundlerinfo) 修改。
- `version`：默认从 `@rustbolt/core/package.json` 读取，可以通过 [experiments.rustboltFuture.bundlerInfo.version](/config/experiments#rustboltfuturebundlerinfo) 修改。

<Columns>
```js title=源码
__rustbolt_unique_id__
```
...
__webpack_require__.ruid = 'bundler=rustbolt@0.7.4';
```
