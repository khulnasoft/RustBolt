...
{`
flowchart TD
CallRustbolt("rustbolt()") --> CreateCompiler("创建 Compiler 实例")
...
HookAfterEnvironment --> ApplyRustboltPlugins(执行内部插件)
...
在 Rustbolt 选项中的 [`entry`](/config/entry) 被处理过之后调用。
...
:::note
这个钩子仅在调用 [compiler.run()](/api/javascript-api/index#compilerrun) 时触发（对应 `rustbolt build` 命令），在监听模式（watch mode）下不会执行。在监听模式下，你可以使用 [watchRun](#watchrun) 钩子。
:::
...
在 make 阶段结束后调用。在 make 阶段，Rustbolt 会以 entry 为起点构建模块依赖图，并使用 loader 处理各个模块，当这个过程完成时会触发此钩子。
