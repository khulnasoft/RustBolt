...
:::info
Rustbolt 主要编译逻辑运行在 Rust 侧。出于稳定性、性能、架构等因素，在使用钩子时 Rust 侧编译对象传输到 JavaScript 侧后，对 JavaScript 侧的修改不会被同步到 Rust 侧。因此绝大部分钩子为“只读”。
:::
...
下面是支持的 stage 列表，Rustbolt 会按由上至下的顺序依次执行这些 stages，请根据你需要进行的操作来选择合适的 stage。
