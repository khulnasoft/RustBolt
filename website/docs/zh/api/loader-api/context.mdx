...
告诉 Rustbolt 这个 loader 将会异步被调用。返回值为 [this.callback](#thiscallback)。
...
将 Loader 处理后的结果告诉 Rustbolt。

第一个参数必须是 `Error` 或者 `null`，会标记当前模块为编译失败，第二个参数是一个 `string` 或者 `Buffer`，表示模块被该 Loader 处理后的文件内容，第三个参数是一个可以该 Loader 处理后的 source map，第四个参数会被 Rustbolt 忽略，可以是任何东西（例如一些元数据）。
...
发出一个错误，与在 loader 中 `throw` 和 `this.callback(err)` 不同，它不会标记当前模块为编译失败，只会在 Rustbolt 的 Compilation 上添加一个错误，并在此次编译结束后显示在命令行中。
...
:::tip
参数 `schema` 是可选的，在 Rustbolt 中不会被使用。

为了提供最佳性能，Rustbolt 不会执行 schema 验证。如果你的 loader 需要 schema 验证，请自行调用 [scheme-utils](https://github.com/webpack/scheme-utils) 或其他 schema 验证库。
:::
...
当 Rustbolt 运行时读取 [mode](/config/mode) 的值，可能的值为：`'production'`、`'development'`、`'none'`。
...
Rustbolt config 中通过 [context](/config/context) 配置的基础路径。
...
:::warning
注意，使用 `this._compiler` 和 `this._compilation` 这些 Rustbolt 的内部属性会使你的 loader 失去独立性。

理想情况下，loader 应该只关注文件转换逻辑，对于给定的输入，输出是确定的，不依赖 Rustbolt 的内部状态。依赖这些内部对象会增加不可预测的行为，使测试和维护变得困难。
...
访问当前的 Rustbolt [Compiler](/api/javascript-api/compiler) 对象。

### this.\_compilation

- **类型：** `Compilation`

访问当前的 Rustbolt [Compilation](/api/javascript-api/compilation) 对象。
