...
2. Rustbolt 的缓存失效检测机制依赖于 `modifiedFiles` 和 `removedFiles` 参数。在启用缓存的情况下，如果你使用了自定义的 watcher 来监听文件变化，需要通过 `options` 参数将这两个值传递给 Rustbolt。
...
允许在 Rustbolt 中运行另一个 Rustbolt 实例。但是，子编译器会应用不同的设置和配置。他会从父 Compiler（或者顶级 Compiler）中复制所有的钩子（hook）和插件（plugin），并且创建一个子 Compiler 实例。 返回值为创建好的 Compiler 实例。
...
获取当前的 `@rustbolt/core` 的导出，以此来获取关联的内部对象。当你无法直接引用 `@rustbolt/core`，或者存在多个 Rustbolt 实例时很有用。

一个常见的例子是在 Rustbolt 插件中获取 [sources](/api/javascript-api/index#sources-对象) 对象：
...
等同于 `compiler.rustbolt`，该属性用于兼容 webpack 插件。

如果你开发的 Rustbolt 插件需要兼容 webpack，可以使用该属性代替 `compiler.rustbolt`。
...
- 通过 `rustbolt({})` 等创建时同于 [context 配置](/config/context)
...
`MultiCompiler` 可以让 Rustbolt 同时执行多份配置。如果传给 Rustbolt 的 JavaScript API 的配置为多个配置对象构成的数组，Rustbolt 会创建多个 compiler 实例，并且在所有 compiler 执行完毕后调用回调方法。
