...
:::warning 注意
在 Rustbolt 中，真实的 Compilation 运行在 Rust 侧，JavaScript Compilation 仅作为用于与 Rust Compilation 通信的**代理对象**。
...
允许在 Rustbolt 中运行另一个 Rustbolt 实例。但是，子编译器会应用不同的设置和配置。他会从父编译器（或者顶级编译器）中复制所有的钩子（hook）和插件（plugin），并且创建一个子 Compiler 实例。 返回值为创建好的 Compiler 实例。

```ts
createChildCompiler(
  name: string, // 子编译器名称
  outputOptions: OutputNormalized, // 子编译所使用的 Output 配置对象
  plugins: RustboltPluginInstance[] // 子编译所添加的插件
): Compiler;
```
...
获取此 Compilation 使用的完整配置，详见[配置 Rustbolt](/config/index)。
