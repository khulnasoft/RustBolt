...
Rustbolt 提供了一个内置的 `builtin:swc-loader`，它是 [swc-loader](https://github.com/swc-project/pkgs/tree/main/packages/swc-loader) 的 Rust 版本，旨在提供更优的性能，Loader 的 [选项](https://swc.rs/docs/configuration/compilation) 与 JS 版本的 `swc-loader` 保持一致。
...
你可以使用 `@rustbolt/core` 导出的 `SwcLoaderOptions` 类型来开启类型提示：
...
下面是部分 SWC 配置以及 Rustbolt 特有配置的介绍，完整选项请参考 [SWC 配置](https://swc.rs/docs/configuration/swcrc)。
...
Rustbolt 支持在 `builtin:swc-loader` 里加载 SWC 的 Wasm 插件, 你可以通过如下配置启用 Wasm 插件
...
这是 Wasm 插件的一个[示例](https://github.com/rustbolt-contrib/rstack-examples/blob/d4b8aaef9915ed0f540edbe504217c3d1afe8989/rustbolt/builtin-swc-loader/rustbolt.config.js#L45)。
