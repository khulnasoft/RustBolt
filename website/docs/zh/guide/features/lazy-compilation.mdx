...
对于使用 `@rustbolt/cli` 的用户，你可以通过 [experiments.lazyCompilation](/config/experiments#experimentslazycompilation) 配置开启懒编译，假设你正在开发一个包含多个入口的项目，当开发其中一个入口时，Rustbolt 只会构建你当前访问的入口。
...
只有当执行对应的入口或时，Rustbolt 才会编译对应的入口和 Module 以及他们的所有依赖。

## 使用自定义的开发服务器

在上面的配置中，`experiments.lazyCompilation` 配置实际上是由 `@rustbolt/cli` 消费，并向 `@rustbolt/dev-server` 中添加一个用于处理 `lazyCompilation` 客户端请求的[Express 风格的 middleware](https://expressjs.com/en/guide/using-middleware.html) 来处理懒编译的请求，如果你使用自己的开发服务器，你需要手动将该 middleware 的处理逻辑添加到你的开发服务器。
