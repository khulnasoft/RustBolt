...
完整示例可参考：[examples/preact](https://github.com/rustbolt-contrib/rstack-examples/blob/main/rustbolt/preact)

## Preact Refresh

需要开启 Preact Refresh 需要如下步骤：

- 添加 `@rustbolt/plugin-preact-refresh` 插件
- 添加代码转换相关 loader
...
- 代码注入：指注入与 `@prefresh/core` 和 `@prefresh/utils` 交互的代码，都已集成在 [@rustbolt/plugin-preact-refresh](https://github.com/rustbolt-contrib/rustbolt-plugin-preact-refresh) 插件中，可通过该插件实现
- 代码转换需要通过 loader 来完成：
...
:::warning
在 1.0.0 以下版本, Rustbolt 不支持在 `swc-loader` 中使用 preact 特有转换。

请使用 `builtin:swc-loader` 并在配置中添加 `rustboltExperiments.preact: {}` 以开启 preact 特有转换。
:::
...
完整示例可参考：[examples/preact-refresh](https://github.com/rustbolt-contrib/rstack-examples/blob/main/rustbolt/preact-refresh)
