...
通过 [@rustbolt/plugin-react-refresh](https://www.npmjs.com/package/@rustbolt/plugin-react-refresh) 支持 React Fast Refresh 功能，首先需要安装相关依赖：
...
- 代码注入会注入 `react-refresh` 包中的一些代码，以及一些自定义的运行时代码，都已集成在 [@rustbolt/plugin-react-refresh](https://github.com/rustbolt-contrib/rustbolt-plugin-react-refresh) 插件中，可通过该插件注入。
...
这种实现方式将代码注入逻辑和转换逻辑解耦，代码注入逻辑统一通过 `@rustbolt/plugin-react-refresh` 插件完成，代码转换通过 loader 完成。这意味着该插件可以配合 `builtin:swc-loader`、`swc-loader` 或 `babel-loader` 使用：

- 配合 `builtin:swc-loader` 使用方式可参考：[examples/react-refresh](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/react-refresh)，使用 `swc-loader` 只需将 `builtin:swc-loader` 换为 `swc-loader` 即可。
- 配合 `babel-loader` 的使用方式可参考：[examples/react-refresh-babel-loader](https://github.com/rustbolt-contrib/rstack-examples/tree/main/rustbolt/react-refresh-babel-loader)
...
在 Rustbolt 中使用 React Compiler 的步骤如下：
...
3. 在你的 Rustbolt 配置中注册 `babel-loader`：
...
在 Rustbolt 中使用 SVGR 的方式与 webpack 中完全一致：
