...
:::tip
当设置了 `optimization.minimizer` 时，默认的压缩器会被禁用，因此我们需要同时添加 [LightningCssMinimizerRustboltPlugin](/plugins/rustbolt/lightning-css-minimizer-rustbolt-plugin) 来压缩 CSS 文件。
:::
