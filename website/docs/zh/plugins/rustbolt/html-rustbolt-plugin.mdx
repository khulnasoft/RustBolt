...
`rustbolt.HtmlRustboltPlugin` 是使用 Rust 实现的高性能 HTML 插件，你可以使用它来为 Rustbolt 项目生成 HTML 文件。
...
在使用 `rustbolt.HtmlRustboltPlugin` 之前，请注意 `rustbolt.HtmlRustboltPlugin` 和社区的 [`html-webpack-plugin`](https://www.npmjs.com/package/html-webpack-plugin) 插件存在一些差异。

### 性能

由于 `rustbolt.HtmlRustboltPlugin` 是基于 Rust 实现的，因此它的构建性能显著高于 `html-webpack-plugin` 插件，尤其是在构建大量 HTML 文件的场景下。

### 功能

`rustbolt.HtmlRustboltPlugin` 的功能是 `html-webpack-plugin` 的子集。为了保证插件的性能，我们没有实现 `html-webpack-plugin` 提供的所有功能。

如果它提供的配置项无法满足你的需求，你也可以直接使用社区的 [`html-webpack-plugin`](https://www.npmjs.com/package/html-webpack-plugin) 插件。

:::warning
`rustbolt.HtmlRustboltPlugin` 不支持完整的 [EJS](https://github.com/mde/ejs/blob/main/docs/syntax.md) 语法, 仅支持 EJS 语法的一个子集，如果你对完整的 EJS 语法支持有需求，可以直接使用 `html-webpack-plugin`。为了和 `html-webpack-plugin` 的默认的插值语法对齐，
Rustbolt 修改了 EJS 的 escape 和 unescape 的默认语法，使其采用和 `html-webpack-plugin` 相同的语法。
:::
...
如果你的 Rustbolt 配置中有多个 entry points，它们的生成 `<script>` 标签都会被包含在这个 HTML 文件中。

如果你的构建产物中有一些 CSS 资源，它们将被包含在 HTML head 的 `<link>` 标签中。

## 选项

你可以向 `rustbolt.HtmlRustboltPlugin` 传递一些配置项，支持的选项如下：
...
最简单的方式是使用 `template` 选项，并传递一个自定义的 HTML 文件。`rustbolt.HtmlRustboltPlugin` 将会自动将所有需要的 JS、CSS 和 favicon 文件注入到 HTML 中。
...
- `htmlRustboltPlugin`: 插件的数据
  - `htmlRustboltPlugin.options`: 插件的配置对象
  - `htmlRustboltPlugin.tags`: 准备好的用于在模板中注入的标签信息
    - `htmlRustboltPlugin.tags.headTags`: 用于在 `<head>` 中注入的 `<base>`、`<meta>`、`<link>`、`<script>` 标签列表
    - `htmlRustboltPlugin.tags.bodyTags`: 用于在 `<body>` 中注入的 `<script>` 标签列表
  - `htmlRustboltPlugin.files`: 此次编译产生的产物文件信息
    - `htmlRustboltPlugin.files.js`: 此次编译产生的 JS 产物路径列表
    - `htmlRustboltPlugin.files.css`: 此次编译产生的 CSS 产物路径列表
    - `htmlRustboltPlugin.files.favicon`: 若配置了 `favicon`，此处为计算出的最终的 favicon 产物路径
    - `htmlRustboltPlugin.files.publicPath`: 产物文件的 publicPath
- `rustboltConfig`: 此次编译所使用的 Rustbolt 配置对象
- `compilation`: 此次编译的 compilation 对象

:::warning 警告
若使用 `htmlRustboltPlugin.tags` 在模板渲染时插入标签，请将 `inject` 配置为 `false`，否则会导致标签被注入两次。
:::
...
- `rustboltConfig` 对象目前仅支持获取 `mode`、`output.publicPath` 和 `output.crossOriginLoading` 属性
- `compilation` 对象目前仅支持在[使用模板生成函数](#使用模板生成函数)时使用
- 在模板中渲染标签列表（如 `htmlRustboltPlugin.tags.headTags`）或单个标签（如 `htmlRustboltPlugin.tags.headTags[0]`）时，需要使用 `toHtml()` 函数生成 HTML 代码
...
如果设置了 `meta`，HtmlRustboltPlugin 将注入 `<meta>` 标签。
...
如果设置了 `base`，HtmlRustboltPlugin 将注入 `<base>` 标签。
...
如果你有多个 entry points，并希望为每个 entry 生成一个 HTML 文件，那么你可以注册多个 `rustbolt.HtmlRustboltPlugin`：
...
HtmlRustboltPlugin 提供了一些 hooks，可以让你在构建过程中修改标签或 HTML 产物代码。可通过 `rustbolt.HtmlRustboltPlugin.getCompilationHooks` 来获取 hooks 对象：
