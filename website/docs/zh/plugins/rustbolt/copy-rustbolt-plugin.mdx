...
拷贝的输出路径，可以是绝对路径、相对路径或者是 Rustbolt 的模版字符串，例如 `'[name].[hash][ext]'`。当不指定 `to` 时，则相当于是 Rustbolt 的 [output.path](/config/output#outputpath)。
...
`context` 可以是一个绝对路径或相对路径。如果它是一个相对路径，则基于 Rustbolt 的 [context](/config/context) 转换为绝对路径。
...
指定 [to](#to) 的类型，可以是目录，文件或 Rustbolt 的模版名，若不指定则会自动推断。
