...
`rustbolt.experiments.SubresourceIntegrityPlugin` 是一个用于在 Rustbolt 中启用子资源完整性的插件。
...
支持与 [`HtmlRustboltPlugin`](/plugins/rustbolt/html-rustbolt-plugin) 和 [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin) 集成。它会自动为注入的标签设置 `integrity` 和 `crossorigin` 属性。
...
可以通过从 `@rustbolt/core` 中导入，目前该插件处于实验性阶段：
...
当使用 HTML 插件（[`HtmlRustboltPlugin`](/plugins/rustbolt/html-rustbolt-plugin.mdx) 或 [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin)）时，`integrity` 和 `crossorigin` 属性将被自动设置。

SubresourceIntegrityPlugin 默认会与 [`HtmlRustboltPlugin`](/plugins/rustbolt/html-rustbolt-plugin.mdx) 交互：
...
对于 [`HtmlRustboltPlugin`](/plugins/rustbolt/html-rustbolt-plugin.mdx)，模板的语法与标准 `.ejs` 稍有不同不同（参考[文档](/plugins/rustbolt/html-rustbolt-plugin#supported-ejs-syntax)），可以这样注入属性：

```ejs title="index.ejs"
<% for  in htmlRustboltPlugin.files.js { %>
  <script src="<%= htmlRustboltPlugin.files.js[index] %>"
...
:::tip 提示
当你在 `link` 和 `script` 标签上添加 `integrity` 属性时，你还需要设置 `crossorigin` 属性。建议将此属性设置为与 Rustbolt 的 `output.crossOriginLoading` 配置选项相同的值。
:::
...
- `auto` 是默认值，表示在 [Rustbolt mode](/config/mode) 为 `production` 或 `none` 时启用插件，在 `development` 时禁用插件。
...
- **类型：** `string`
- **默认值：** `"HtmlRustboltPlugin"`

HTML 插件的路径，默认为 `"HtmlRustboltPlugin"`，表示 Rustbolt 的原生 HTML 插件。如果你使用的是 [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin)，你可以将此选项设置为它的路径。建议设置绝对路径以确保能找到正确的插件实例。
