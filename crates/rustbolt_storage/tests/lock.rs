...
mod test_storage_lock {
  ...
  use rustbolt_fs::{FileMetadata, MemoryFileSystem, NativeFileSystem};
  use rustbolt_paths::{AssertUtf8, Utf8Path, Utf8PathBuf};
  use rustbolt_storage::{
  ...
  pub fn get_native_path(p: &str) -> (PathBuf, PathBuf) {
    let base = std::env::temp_dir()
      .join("rustbolt_test/storage/test_storage_lock")
      .join(p);
    (base.join("cache"), base.join("temp"))
  }

  pub fn get_memory_path(p: &str) -> (PathBuf, PathBuf) {
    let base = PathBuf::from("/rustbolt_test/storage/test_storage_lock/").join(p);
    (base.join("cache"), base.join("temp"))
  }
