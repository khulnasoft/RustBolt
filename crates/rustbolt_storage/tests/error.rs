...
mod test_storage_error {
  ...
  use rustbolt_fs::{MemoryFileSystem, NativeFileSystem};
  ...
  pub fn get_native_path(p: &str) -> (PathBuf, PathBuf) {
    let base = std::env::temp_dir()
      .join("rustbolt_test/storage/test_storage_error")
      .join(p);
    (base.join("cache"), base.join("temp"))
  }

  pub fn get_memory_path(p: &str) -> (PathBuf, PathBuf) {
    let base = PathBuf::from("/rustbolt_test/storage/test_storage_error/").join(p);
    (base.join("cache"), base.join("temp"))
  }
