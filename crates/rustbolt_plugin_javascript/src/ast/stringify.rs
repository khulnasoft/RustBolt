...
use rustbolt_core::rustbolt_sources::{self, encode_mappings, Mapping, OriginalLocation};
...
pub fn print(
  ...
  let map = if source_map_config.enable {
    ...
    let mut rustbolt_source_map = rustbolt_sources::SourceMap::new(
      mappings,
      combined_source_map
        .sources()
    ...
    rustbolt_source_map.set_file(combined_source_map.get_file().map(ToString::to_string));

    Some(rustbolt_source_map)
  } else {
    None
  };
