...
use rustbolt_core::ModuleType;
use rustbolt_error::TraceableError;
use swc_core::{
...
///
/// 2. We can't convert to [rustbolt_error::Error] at this point, because there is
///    no `path` and `source`
pub fn parse_js(
  fm: Arc<SourceFile>,
  target: EsVersion,
  ...
  match parse_with_lexer(lexer, module_type_to_is_module(module_type)) {
    ...
    Err(errs) => Err(
      errs
        .dedup_ecma_errors()
        .into_iter()
        .map(|err| ecma_parse_error_deduped_to_rustbolt_error(err, fm, module_type))
        .collect::<Vec<_>>(),
    ),
  }
}
