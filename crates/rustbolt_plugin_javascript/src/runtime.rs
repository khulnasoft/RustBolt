...
use rustbolt_util::diff_mode::is_diff_mode;
...
pub async fn render_chunk_modules(
  ...
  let module_sources = rustbolt_futures::scope::<_, _>(|token| {
...
pub async fn render_runtime_modules(
  ...
  let mut sources = ConcatSource::default();
  let runtime_module_sources = rustbolt_futures::scope::<_, Result<_>>(|token| {
    compilation
      .chunk_graph
