...
use rayon::prelude::*;
use rustbolt_collections::{IdentifierDashMap, IdentifierIndexSet, IdentifierMap, IdentifierSet};
use rustbolt_core::{
  concatenated_module::{
...
impl ModuleConcatenationPlugin {
  ...
  pub async fn process_concatenated_configuration(
    ...
    let modules = modules_set
      ...
      .map(|id| {
        ...
        let inner_module = ConcatenatedInnerModule {
          id: *id,
          size: module.size(
            Some(&rustbolt_core::SourceType::JavaScript),
            Some(compilation),
          ),
    ...
    let mut new_module = ConcatenatedModule::create(
      root_module_ctxt,
      modules,
      Some(rustbolt_hash::HashFunction::MD4),
      config.runtime.clone(),
      compilation,
    );
    new_module
      .build(
        rustbolt_core::BuildContext {
    ...
    // Rustbolt: if entry is an asset module, outputs a js chunk and a asset chunk
  ...
  async fn optimize_chunk_modules_impl(&self, compilation: &mut Compilation) -> Result<()> {
    let logger = compilation.get_logger("rustbolt.ModuleConcatenationPlugin");
