...
use rustbolt_paths::{AssertUtf8, Utf8Path};
...
async fn optimize_dependencies(&self, compilation: &mut Compilation) -> Result<Option<bool>> {
  let logger = compilation.get_logger("rustbolt.SideEffectsFlagPlugin");
    ...
    && !side_effects_optimize_artifact.is_empty()
  {
    ...
    let logger = compilation.get_logger("rustbolt.incremental.sideEffects");
    logger.log(format!(
