...
use rustbolt_collections::{IdentifierMap, IdentifierSet};
use rustbolt_core::{
    ...
    for export_name_or_spec in exports {
      let (name, can_mangle, terminal_binding, exports, from, from_export, priority, hidden) =
        match export_name_or_spec {
          ExportNameOrSpec::String(name) => (
            ...
            None::<&rustbolt_core::Nullable<Vec<Atom>>>,
      ...
      if let Some(from) = from {
        let changed = if hidden {
          ...
          let fallback = rustbolt_core::Nullable::Value(vec![name.clone()]);
...
async fn finish_modules(&self, compilation: &mut Compilation) -> Result<()> {
    ...
    .mutations_read(IncrementalPasses::PROVIDED_EXPORTS)
  {
    ...
    let logger = compilation.get_logger("rustbolt.incremental.providedExports");
    logger.log(format!(
