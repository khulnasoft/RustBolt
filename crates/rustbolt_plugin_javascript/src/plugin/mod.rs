...
use rustbolt_collections::{Identifier, IdentifierDashMap, IdentifierLinkedMap, IdentifierMap};
use rustbolt_core::{
  basic_function,
  concatenated_module::find_new_name,
  render_init_fragments,
  reserved_names::RESERVED_NAMES,
  rustbolt_sources::{BoxSource, ConcatSource, RawStringSource, ReplaceSource, Source, SourceExt},
...
use rustbolt_util::{diff_mode, fx_hash::FxDashMap};
...
#[derive(Debug, Clone)]
struct WithHash<T> {
  hash: Option<RustboltHashDigest>,
  value: T,
}
...
impl JsPlugin {
  ...
  pub async fn get_renamed_inline_module(
    ...
    let render_module_results = rustbolt_futures::scope::<_, _>(|token| {
