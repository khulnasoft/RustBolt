...
use regex::Captures;
use rustbolt_core::{ErrorSpan, SpanExt};
use rustbolt_error::miette::{Diagnostic, Severity};
...
impl WebpackCommentMap {
  ...
  pub fn get_webpack_include(&self) -> Option<RustboltRegex> {
    self.0.get(&WebpackComment::IncludeRegexp).map(|expr| {
      ...
      RustboltRegex::with_flags(expr, flags).unwrap_or_else(|_| {
        // test when capture
        unreachable!();
      })
    })
  ...
  pub fn get_webpack_exclude(&self) -> Option<RustboltRegex> {
          ...
          "webpackInclude" => {
            if captures.name("_6").is_some() {
              if let Some(regexp) = captures.get(9).map(|x| x.as_str()) {
                ...
                if RustboltRegex::with_flags(regexp, flags).is_ok() {
