use rustbolt_core::{ConstDependency, RuntimeGlobals, RuntimeRequirementsDependency, SpanExt};
use swc_core::{
  common::Spanned,
...
const RUSTBOLT_VERSION: &str = "__rustbolt_version__";
const RUSTBOLT_UNIQUE_ID: &str = "__rustbolt_unique_id__";
...
fn get_typeof_evaluate_of_api(sym: &str) -> Option<&str> {
  match sym {
    ...
    RUSTBOLT_VERSION => Some("string"),
    RUSTBOLT_UNIQUE_ID => Some("string"),
    _ => None,
  }
}
      ...
      RUSTBOLT_VERSION => {
        parser
          ...
          .push(Box::new(ConstDependency::new(
            ident.span.real_lo(),
            ident.span.real_hi(),
            format!("{}()", RuntimeGlobals::RUSTBOLT_VERSION).into(),
            Some(RuntimeGlobals::RUSTBOLT_VERSION),
          )));
        Some(true)
      }
      RUSTBOLT_UNIQUE_ID => {
        parser
          ...
          .push(Box::new(ConstDependency::new(
            ident.span.real_lo(),
            ident.span.real_hi(),
            format!("{}", RuntimeGlobals::RUSTBOLT_UNIQUE_ID).into(),
            Some(RuntimeGlobals::RUSTBOLT_UNIQUE_ID),
          )));
        Some(true)
      }
      _ => None,
