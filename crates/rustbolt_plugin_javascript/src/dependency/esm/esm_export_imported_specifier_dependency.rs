...
impl Dependency for ESMExportImportedSpecifierDependency {
    ...
    match mode.ty {
      ...
      ExportModeType::ReexportDynamicDefault => {
        ...
        Some(ExportsSpec {
          exports: ExportsOfExportsSpec::Array(vec![ExportNameOrSpec::ExportSpec(ExportSpec {
            ...
            export: Some(rustbolt_core::Nullable::Value(vec![Atom::from("default")])),
            from: from.cloned(),
            ..Default::default()
          })]),
          priority: Some(1),
      ...
      ExportModeType::ReexportNamespaceObject => {
        ...
        Some(ExportsSpec {
          exports: ExportsOfExportsSpec::Array(vec![ExportNameOrSpec::ExportSpec(ExportSpec {
            name: mode.name.unwrap_or_default(),
            export: Some(rustbolt_core::Nullable::Null),
            from: from.cloned(),
            ..Default::default()
          })]),
          priority: Some(1),
                ...
                items
                  ...
                  .map(|item| {
                    ExportNameOrSpec::ExportSpec(ExportSpec {
                      name: item.name,
                      from: from.cloned(),
                      export: Some(rustbolt_core::Nullable::Value(item.ids)),
  ...
  fn could_affect_referencing_module(&self) -> rustbolt_core::AffectType {
    rustbolt_core::AffectType::Transitive
  }
}
...
impl DependencyConditionFn for ESMExportImportedSpecifierDependencyCondition {
  fn get_connection_state(
    &self,
    _conn: &rustbolt_core::ModuleGraphConnection,
    runtime: Option<&RuntimeSpec>,
    module_graph: &ModuleGraph,
  ) -> ConnectionState {
