...
impl DependencyTemplate for PureExpressionDependency {
  fn apply(&self, source: &mut TemplateReplaceSource, ctx: &mut TemplateContext) {
    ...
    let condition = match &runtime_condition {
      rustbolt_core::RuntimeCondition::Boolean(true) => return,
      rustbolt_core::RuntimeCondition::Boolean(false) => None,
      rustbolt_core::RuntimeCondition::Spec(_spec) => Some(runtime_condition_expression(
