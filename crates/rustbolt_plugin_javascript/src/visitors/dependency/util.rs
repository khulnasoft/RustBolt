use rustbolt_core::{ConstDependency, ErrorSpan, SpanExt};
use rustbolt_error::{
  ...
  (
    Box::new(
      create_traceable_error(
        ...
        format!("{name} is not supported by Rustbolt."),
        file,
        expr.span().into(),
      )
...
pub fn context_reg_exp(
  ...
  parser: &mut JavascriptParser,
) -> Option<RustboltRegex> {
  if expr.is_empty() {
    return None;
  }
  let regexp = RustboltRegex::with_flags(expr, flags).expect("reg failed");
  ...
  if regexp.sticky() || regexp.global() {
    if let Some(error_span) = error_span {
      parser.warning_diagnostics.push(Box::new(
        ...
        .with_severity(rustbolt_error::RustboltSeverity::Warn),
      ));
    } else {
