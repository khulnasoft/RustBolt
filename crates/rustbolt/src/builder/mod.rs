...
use rustbolt_fs::{IntermediateFileSystem, ReadableFileSystem, WritableFileSystem};
use rustbolt_hash::{HashDigest, HashFunction, HashSalt};
use rustbolt_paths::{AssertUtf8, Utf8PathBuf};
...
///
/// [`Compiler`]: rustbolt_core::compiler::Compiler
#[derive(Default)]
...
impl CompilerBuilder {
  ...
  /// Set the mode in which Rustbolt should operate.
  ///
  ...
  ///
  /// ```rust
  /// use rustbolt::builder::{Builder as _, ModuleOptionsBuilder};
  /// use rustbolt_core::{Compiler, ModuleOptions};
  ///
  ...
  ///
  /// ```rust
  /// use rustbolt::builder::{Builder as _, OutputOptionsBuilder};
  /// use rustbolt_core::{Compiler, OutputOptions};
  ///
  /// // Using builder without calling `build()`
  ...
  ///
  /// ```rust
  /// use rustbolt::builder::{Builder as _, OptimizationOptionsBuilder};
  /// use rustbolt_core::{Compiler, Optimization};
  ///
  /// // Using builder without calling `build()`
  ...
  ///
  /// ```rust
  /// use rustbolt::builder::{Builder as _, NodeOptionBuilder};
  /// use rustbolt_core::{Compiler, NodeGlobalOption, NodeOption};
  ///
  /// // Using builder without calling `build()`
  /// let compiler =
  ...
  ///
  /// ```rust
  /// use rustbolt::builder::{Builder as _, ExperimentsBuilder};
  /// use rustbolt_core::{incremental::IncrementalPasses, Compiler, Experiments};
  ///
  /// // Using builder without calling `build()`
  /// let compiler = Compiler::builder()
...
impl CompilerBuilder {
  ...
  pub fn enable_loader_lightningcss(&mut self) -> &mut Self {
    self.plugin(Box::new(
      rustbolt_loader_lightningcss::LightningcssLoaderPlugin::new(),
    ))
  }
}
...
impl CompilerBuilder {
  ...
  pub fn enable_loader_swc(&mut self) -> &mut Self {
    self.plugin(Box::new(rustbolt_loader_swc::SwcLoaderPlugin::new()))
  }
}
...
impl CompilerBuilder {
  ...
  pub fn enable_loader_react_refresh(&mut self) -> &mut Self {
    self.plugin(Box::new(
      rustbolt_loader_react_refresh::ReactRefreshLoaderPlugin::new(),
    ))
  }
}
...
impl CompilerBuilder {
  ...
  pub fn enable_loader_preact_refresh(&mut self) -> &mut Self {
    self.plugin(Box::new(
      rustbolt_loader_preact_refresh::PreactRefreshLoaderPlugin::new(),
    ))
  }
}
...
pub struct CompilerOptionsBuilder {
  ...
  /// The mode in which Rustbolt should operate.
  mode: Option<Mode>,
  /// The type of externals.
...
impl CompilerOptionsBuilder {
  ...
  /// [`CompilerOptions`]: rustbolt_core::options::CompilerOptions
  pub fn build(&mut self, builder_context: &mut BuilderContext) -> Result<CompilerOptions> {
    ...
    if devtool_flags.source_map() {
      ...
      let options = rustbolt_plugin_devtool::SourceMapDevToolPluginOptions {
        ...
        module_filename_template: output_builder
          ...
          .map(|t| rustbolt_plugin_devtool::ModuleFilenameTemplate::String(t.as_str().to_string()))
          .clone(),
        append: hidden.then_some(rustbolt_plugin_devtool::Append::Disabled),
        columns: !cheap,
      ...
      let options = rustbolt_plugin_devtool::EvalDevToolModulePluginOptions {
        module_filename_template: output_builder
          .devtool_module_filename_template
          .map(|t| rustbolt_plugin_devtool::ModuleFilenameTemplate::String(t.as_str().to_string()))
          .clone(),
    ...
    use rustbolt_plugin_externals::ElectronTargetContext;
    ...
    if externals_presets.web() || externals_presets.web_async() || (externals_presets.node() && css)
    {
      builder_context
        .plugins
        .push(BuiltinPluginOptions::HttpExternalsRustboltPlugin((
          css,
...
///
/// [`NodeOption`]: rustbolt_core::options::NodeOption
#[derive(Debug)]
...
///
/// [`ModuleOptions`]: rustbolt_core::options::ModuleOptions
#[derive(Debug, Default)]
...
///
/// [`OutputOptions`]: rustbolt_core::options::OutputOptions
#[derive(Debug, Default)]
...
impl OutputOptionsBuilder {
  ...
  /// Tells Rustbolt to include comments in bundles with information about the contained modules.
  ///
  ...
  ///
  /// [`WasmLoadingType`]: rustbolt_core::options::WasmLoadingType
  ...
  /// A unique name of the Rustbolt build to avoid multiple Rustbolt runtimes to conflict when using globals.
  ...
  /// The global variable is used by Rustbolt for loading chunks.
  ///
  ...
  /// Set the global variable is used by Rustbolt for loading hot updates.
  ///
  /// Determined by output.uniqueName default.
  ...
  /// Set whether to tell Rustbolt to add IIFE wrapper around emitted code.
  ...
  fn build(
    ...
    let chunk_loading_global = f!(self.chunk_loading_global.take(), || {
      format!("webpackChunk{}", rustbolt_core::to_identifier(&unique_name))
    });
    ...
    let hot_update_global = f!(self.hot_update_global.take(), || {
      format!(
        "webpackHotUpdate{}",
        rustbolt_core::to_identifier(&unique_name)
      )
    });
...
///
/// [`OptimizationOptions`]: rustbolt_core::options::Optimization
#[derive(Debug, Default)]
pub struct OptimizationOptionsBuilder {
  ...
  /// Runtime chunk.
  runtime_chunk: Option<rustbolt_plugin_runtime_chunk::RuntimeChunkOptions>,
  // TODO: split chunks
}
...
impl OptimizationOptionsBuilder {
  ...
  pub fn minimizer(&mut self, value: Vec<BoxPlugin>) -> &mut Self {
    self.minimizer = Some(
      value
        .into_iter()
        .map(BuiltinPluginOptions::AnyMinimizerRustboltPlugin)
        .collect(),
    );
    self
  }
  ...
  ///
  /// After enabling, Rustbolt will analyze which exports the module provides, including re-exported modules.
  ///
  /// Default set to `true`.
  ...
  pub fn runtime_chunk(
    &mut self,
    value: rustbolt_plugin_runtime_chunk::RuntimeChunkOptions,
  ) -> &mut Self {
  ...
  ///
  /// [`Optimization`]: rustbolt_core::options::Optimization
  fn build(
    &mut self,
    builder_context: &mut BuilderContext,
    development: bool,
      ...
      "size" => {
        builder_context
          ...
          .push(BuiltinPluginOptions::OccurrenceChunkIdsPlugin(
            rustbolt_ids::OccurrenceChunkIdsPluginOptions {
    ...
    let minimizer = f!(self.minimizer.take(), || {
      if minimize {
        vec![
          BuiltinPluginOptions::AnyMinimizerRustboltPlugin(Box::new(
            rustbolt_plugin_swc_js_minimizer::SwcJsMinimizerRustboltPlugin::new(
              rustbolt_plugin_swc_js_minimizer::PluginOptions {
                test: None,
                include: None,
                exclude: None,
...
///
/// [`Experiments`]: rustbolt_core::options::Experiments
#[derive(Debug, Default)]
pub struct ExperimentsBuilder {
  ...
  /// Rustbolt future.
  rustbolt_future: Option<RustboltFuture>,
  /// Cache options.
...
impl From<Experiments> for ExperimentsBuilder {
  fn from(value: Experiments) -> Self {
    ExperimentsBuilder {
      ...
      rustbolt_future: Some(value.rustbolt_future),
...
impl From<&mut ExperimentsBuilder> for ExperimentsBuilder {
  fn from(value: &mut ExperimentsBuilder) -> Self {
    ExperimentsBuilder {
      ...
      rustbolt_future: value.rustbolt_future.take(),
      cache: value.cache.take(),
...
impl ExperimentsBuilder {
  ...
  fn build(
    ...
    let rustbolt_future = d!(self.rustbolt_future.take(), RustboltFuture {});
