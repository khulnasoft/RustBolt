...
use rustbolt_core::{Mode, RunnerContext};
use rustbolt_error::{error, AnyhowError, Diagnostic, Result};
use rustbolt_loader_runner::{Identifiable, Identifier, Loader, LoaderContext};
...
impl SwcLoader {
  ...
  fn loader_impl(&self, loader_context: &mut LoaderContext<RunnerContext>) -> Result<()> {
    ...
    let built = c
      .parse(None, |_| {
        transformer::transform(&self.options_with_additional.rustbolt_experiments)
      })
      .map_err(AnyhowError::from)?;
