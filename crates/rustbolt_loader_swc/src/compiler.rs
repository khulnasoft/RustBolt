...
use rustbolt_ast::javascript::{Ast as JsAst, Context as JsAstContext, Program as JsProgram};
use rustbolt_error::miette::{self, MietteDiagnostic};
use rustbolt_util::{itoa, swc::minify_file_comments};
  ...
  pub fn transform(&self, config: BuiltInput<impl Pass>) -> Result<Program, miette::Report> {
    ...
    let program = self.run(|| {
      ...
      match result {
        ...
        let swc_core_version = env!("RUSTBOLT_SWC_CORE_VERSION");
        ...
        let help_msg = formatdoc!{"
          ...
          The `swc_core` version of the current `rustbolt_core` is {swc_core_version}. 
          Please check the `swc_core` version of SWC Wasm plugin to make sure these versions are within the compatible range.
          See this guide as a reference for selecting SWC Wasm plugin versions: https://rustbolt.dev/errors/swc-plugin-version"};
