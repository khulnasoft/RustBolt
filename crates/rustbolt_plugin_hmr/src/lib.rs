...
use rustbolt_collections::{DatabaseItem, IdentifierSet, UkeyMap};
use rustbolt_core::{
  chunk_graph_chunk::ChunkId,
  rustbolt_sources::{RawStringSource, SourceExt},
...
use rustbolt_plugin_css::parser_and_generator::CssParserAndGenerator;
...
async fn process_assets(&self, compilation: &mut Compilation) -> Result<()> {
  ...
  for (chunk_id, (old_runtime, old_module_ids)) in &old_chunks {
    ...
    if !new_modules.is_empty() || !new_runtime_modules.is_empty() {
      ...
      //
      // However, in Rustbolt, we need to add HotUpdateChunk to compilation.chunk_by_ukey
      ...
      // Therefore, in Rustbolt, after the manifest is generated, we need to manually remove the HotUpdateChunk from compilation.chunks.
      ...
      //
      // In Rustbolt, we need to manually clean up the data in compilation.chunk_graph after HotUpdateChunk is used.
...
impl Plugin for HotModuleReplacementPlugin {
  fn name(&self) -> &'static str {
    "rustbolt.HotModuleReplacementPlugin"
  }
