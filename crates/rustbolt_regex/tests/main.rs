...
macro_rules! regex {
  ...
  ($pat:literal) => {
    ...
    RustboltRegex::with_flags($pat, "").expect("should compiled")
  };
  ($pat:literal, $flags:ident) => {
    // The macro will expand into the contents of this block.
    RustboltRegex::with_flags($pat, stringify!($flags)).expect("should compiled")
  };
}
...
mod test_regex {
  ...
  fn test_basic() {
    ...
    assert!(RustboltRegex::with_flags("test\\\\", "").is_ok());
  }
