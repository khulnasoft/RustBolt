...
use rustbolt_macros::MergeFrom;
use rustbolt_util::{atom::Atom, ext::CowExt, MergeFrom};
...
/// implements `Hash` and `Eq`, and its error type is `rustbolt_error::Error`.
///
...
pub trait FilenameFn: LocalFilenameFn<Error = rustbolt_error::Error> + Debug + Send + Sync {}
