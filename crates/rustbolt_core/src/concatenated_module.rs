...
use regex::Regex;
use rustbolt_ast::javascript::Ast;
...
use rustbolt_error::{Diagnosable, Diagnostic, DiagnosticKind, Result, TraceableError};
use rustbolt_hash::{HashDigest, HashFunction, RustboltHash, RustboltHashDigest};
...
use rustbolt_util::{ext::DynHash, itoa, source_map::SourceMapKind, swc::join_atom};
...
impl ConcatenatedModule {
  ...
  fn create_identifier(
    ...
    let mut hash = RustboltHash::new(&hash_function.unwrap_or(HashFunction::MD4));
...
impl Module for ConcatenatedModule {
  ...
  async fn code_generation(
    ...
    let tmp = rustbolt_futures::scope::<_, Result<_>>(|token| {
  ...
  async fn get_runtime_hash(
    ...
    let mut hasher = RustboltHash::from(&compilation.options.output);
    let runtime = if let Some(self_runtime) = &self.runtime
      && let Some(generation_runtime) = generation_runtime
    {
      Some(Cow::Owned(
    ...
    if let ModuleInfo::Concatenated(box info) = info {
      ...
      let program = match parse_file_as_module(
        ...
        Err(err) => {
          ...
          return Err(
            rustbolt_error::TraceableError::from_source_file(
              &fm,
              span.start as usize,
              span.end as usize,
...
) -> Vec<rustbolt_error::Diagnostic> {
  errors
    .into_iter()
    .map(|e| rustbolt_error::miette::Error::new(e).into())
    .collect()
}
