...
use rustbolt_collections::{Identifiable, IdentifierSet};
use rustbolt_error::{error, Diagnosable, Diagnostic, DiagnosticExt, NodeError, Result, Severity};
use rustbolt_hash::{RustboltHash, RustboltHashDigest};
use rustbolt_hook::define_hook;
use rustbolt_loader_runner::{run_loaders, AdditionalData, Content, LoaderContext, ResourceData};
...
impl NormalModule {
  ...
  fn init_build_hash(
    ...
    let mut hasher = RustboltHash::from(output_options);
...
impl Module for NormalModule {
  ...
  async fn build(
    ...
    let plugin = Arc::new(RustboltLoaderRunnerPlugin {
