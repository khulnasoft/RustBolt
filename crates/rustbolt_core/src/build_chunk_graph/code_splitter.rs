...
use rustbolt_error::{error, Diagnostic, Error, Result};
...
impl CodeSplitter {
  ...
  pub fn prepare_entry_input(
    ...
    let Some(entry_data) = compilation.entries.get(name) else {
      return Err(rustbolt_error::error!(format!(
        "entry '{}' is not found",
        name
      )));
    };
...
Or do you want to use the entrypoints '{name}' and '{runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint."
  ...
  pub fn prepare_entries(
    ...
    let logger = compilation.get_logger("rustbolt.buildChunkGraph");
  ...
  pub fn split(&mut self, compilation: &mut Compilation) -> Result<()> {
    ...
    {
      let logger = compilation.get_logger("rustbolt.incremental.buildChunkGraph");
      logger.log(format!(
        "{} chunk group created",
