...
mod tests {
  ...
  fn should_path_matcher_works() {
    ...
    let matcher = PathMatcher::Regexp(RustboltRegex::new("[0-9]").unwrap());
  ...
  fn should_snapshot_options_works() {
    let options = SnapshotOptions::new(
      vec![
        ...
        PathMatcher::Regexp(RustboltRegex::new("global/[A-Z]+").unwrap()),
      ],
      vec![
        PathMatcher::String("node_modules/test1".into()),
        PathMatcher::Regexp(RustboltRegex::new("test_modules/test.+").unwrap()),
      ],
      vec![
        PathMatcher::String("node_modules".into()),
        PathMatcher::Regexp(RustboltRegex::new("test_modules/.+").unwrap()),
      ],
    );
