...
use rustbolt_collections::{DatabaseItem, UkeyIndexMap, UkeyIndexSet, UkeyMap, UkeySet};
...
pub type ChunkContentHash = HashMap<SourceType, RustboltHashDigest>;

#[derive(Debug)]
pub struct ChunkHashesResult {
  hash: RustboltHashDigest,
  content_hash: ChunkContentHash,
}

impl ChunkHashesResult {
  pub fn new(hash: RustboltHashDigest, content_hash: ChunkContentHash) -> Self {
    Self { hash, content_hash }
  }

  pub fn hash(&self) -> &RustboltHashDigest {
    &self.hash
  }
...
impl Chunk {
  ...
  pub fn has_async_chunks(&self, chunk_group_by_ukey: &ChunkGroupByUkey) -> bool {
    ...
    // For a real case checkout the test: `rustbolt-test-tools/configCases/chunk-loading/depend-on-with-chunk-load`
  ...
  pub fn update_hash(&self, hasher: &mut RustboltHash, compilation: &Compilation) {
