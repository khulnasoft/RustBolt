...
/// However, rustbolt may create warning during parsing stage, taking CSS as an example.
#[derive(Debug, Error)]
#[error("{title}")]
...
impl rustbolt_error::miette::Diagnostic for CapturedLoaderError {
  fn code<'a>(&'a self) -> Option<Box<dyn std::fmt::Display + 'a>> {
...
pub fn map_box_diagnostics_to_module_parse_diagnostics(
  ...
  errors
    ...
    .map(|e| {
      ...
      let diagnostic: rustbolt_error::Diagnostic =
        rustbolt_error::miette::Error::new(ModuleParseError::new(e, loaders)).into();
