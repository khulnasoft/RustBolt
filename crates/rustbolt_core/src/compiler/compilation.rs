...
use rustbolt_collections::{
  DatabaseItem, Identifiable, IdentifierDashMap, IdentifierMap, IdentifierSet, UkeyMap, UkeySet,
};
use rustbolt_error::{
  error, miette::diagnostic, Diagnostic, DiagnosticExt, InternalError, Result, RustboltSeverity,
  Severity,
};
use rustbolt_fs::{IntermediateFileSystem, ReadableFileSystem, WritableFileSystem};
use rustbolt_hash::{RustboltHash, RustboltHashDigest};
use rustbolt_hook::define_hook;
use rustbolt_paths::ArcPath;
use rustbolt_sources::{BoxSource, CachedSource, SourceExt};
...
// NOTE: This is a Rustbolt-specific hook and has not been standardized yet. Do not expose it to the JS side.
...
define_hook!(CompilationChunkHash: AsyncSeries(compilation: &Compilation, chunk_ukey: &ChunkUkey, hasher: &mut RustboltHash));
define_hook!(CompilationContentHash: AsyncSeries(compilation: &Compilation, chunk_ukey: &ChunkUkey, hashes: &mut HashMap<SourceType, RustboltHash>));
...
pub struct Compilation {
  ...
  pub runtime_modules_hash: IdentifierMap<RustboltHashDigest>,
  ...
  pub hash: Option<RustboltHashDigest>,
...
impl Compilation {
  ...
  /// - module identifier
  /// - error offset
  ///   Rustbolt assumes for each offset, there is only one error.
  ...
  pub fn get_errors_sorted(&self) -> impl Iterator<Item = &Diagnostic> {
    let get_offset = |d: &dyn rustbolt_error::miette::Diagnostic| {
      d.labels()
  ...
  async fn code_generation(&mut self, modules: IdentifierSet) -> Result<()> {
    let logger = self.get_logger("rustbolt.Compilation");
  ...
  pub(crate) async fn code_generation_modules(
    ...
    for module in modules {
      let mut map: HashMap<RustboltHashDigest, CodeGenerationJob> = HashMap::default();
    ...
    let results = rustbolt_futures::scope::<_, _>(|token| {
    ...
    let results = results
      .into_iter()
      .map(|res| res.map_err(rustbolt_error::miette::Error::from_err))
  ...
  async fn create_chunk_assets(&mut self, plugin_driver: SharedPluginDriver) -> Result<()> {
      ...
      && !self.chunk_render_artifact.is_empty()
    {
      ...
      let logger = self.get_logger("rustbolt.incremental.chunksRender");
      logger.log(format!(
    ...
    let results = rustbolt_futures::scope::<_, Result<_>>(|token| {
    ...
    for result in results {
      let item: std::result::Result<(ChunkUkey, ChunkRenderResult), _> =
        result.map_err(rustbolt_error::miette::Error::from_err)?;
      let (key, value) = item?;
  ...
  fn collect_dependencies_diagnostics(&mut self) {
      ...
      && !self.dependencies_diagnostics_artifact.is_empty()
    {
      ...
      let logger = self.get_logger("rustbolt.incremental.dependenciesDiagnostics");
      logger.log(format!(
  ...
  pub async fn seal(&mut self, plugin_driver: SharedPluginDriver) -> Result<()> {
      ...
      && !self.cgm_hash_artifact.is_empty()
    {
      ...
      let logger = self.get_logger("rustbolt.incremental.modulesHashes");
      logger.log(format!(
        "{} modules are affected, {} in total",
      ...
      && !self.code_generation_results.is_empty()
    {
      ...
      let modules: IdentifierSet = mutations
        ...
        .collect();
      let logger = self.get_logger("rustbolt.incremental.modulesCodegen");
      logger.log(format!(
        "{} modules are affected, {} in total",
        modules.len(),
        self.get_module_graph().modules().len()
      ...
      && !self.cgm_runtime_requirements_artifact.is_empty()
    {
      ...
      let modules: IdentifierSet = mutations
        ...
        .collect();
      let logger = self.get_logger("rustbolt.incremental.modulesRuntimeRequirements");
      logger.log(format!(
        "{} modules are affected, {} in total",
        modules.len(),
        self.get_module_graph().modules().len()
      ...
      && !self.cgc_runtime_requirements_artifact.is_empty()
    {
      ...
      self
        .cgc_runtime_requirements_artifact
        .retain(|chunk, _| self.chunk_by_ukey.contains(chunk));
      let logger = self.get_logger("rustbolt.incremental.chunksRuntimeRequirements");
      logger.log(format!(
        "{} chunks are affected, {} in total",
        affected_chunks.len(),
        self.chunk_by_ukey.len()
      ...
      && !self.chunk_hashes_artifact.is_empty()
    {
      ...
      let logger = self.get_logger("rustbolt.incremental.chunksHashes");
      logger.log(format!(
        "{} chunks are affected, {} in total",
        chunks.len(),
        self.chunk_by_ukey.len(),
  ...
  pub async fn process_modules_runtime_requirements(
    ...
    let module_results = rustbolt_futures::scope::<_, Result<_>>(|token| {
      modules
        .into_iter()
  ...
  pub async fn create_hash(
    ...
    let logger = self.get_logger("rustbolt.Compilation");
    let mut compilation_hasher = RustboltHash::from(&self.options.output);

    fn try_process_chunk_hash_results(
      compilation: &mut Compilation,
      chunk_hash_results: Vec<Result<(ChunkUkey, (RustboltHashDigest, ChunkContentHash))>>,
    ) -> Result<()> {
    ...
    for runtime_chunk_ukey in full_hash_chunks {
      ...
      let new_chunk_hash = {
        ...
        let mut hasher = RustboltHash::from(&self.options.output);
  ...
  async fn process_chunk_hash(
    ...
    chunk_ukey: ChunkUkey,
    plugin_driver: &SharedPluginDriver,
  ) -> Result<(RustboltHashDigest, ChunkContentHash)> {
    let mut hasher = RustboltHash::from(&self.options.output);
...
pub struct AssetInfo {
  ...
  /// Related: packages/rustbolt/src/Compilation.ts
