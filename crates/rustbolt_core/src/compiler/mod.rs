...
use rustbolt_util::node_path::NodePath;
...
// should be SyncHook, but rustbolt need call js hook
...
// should be SyncBailHook, but rustbolt need call js hook
...
impl Compiler {
  ...
  async fn compile(&mut self) -> Result<()> {
    ...
    let logger = self.compilation.get_logger("rustbolt.Compiler");
