...
use rustbolt_collections::{DatabaseItem, Identifier, IdentifierSet, UkeySet};
...
impl Task<MakeTaskContext> for ExecuteTask {
  ...
  async fn main_run(self: Box<Self>, context: &mut MakeTaskContext) -> TaskResult<MakeTaskContext> {
    ...
    // the new chunk_graph
    // in rustbolt, if we decouple compilation and chunk_graph, we can't get exclusive ref
