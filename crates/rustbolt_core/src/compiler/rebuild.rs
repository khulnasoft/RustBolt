...
use rustbolt_collections::{DatabaseItem, IdentifierMap};
...
pub struct CompilationRecords {
  ...
  pub runtime_modules: IdentifierMap<RustboltHashDigest>,
  pub chunks: FxHashMap<ChunkId, (RuntimeSpec, FxHashSet<ModuleId>)>,
  pub modules: FxHashMap<ModuleId, RuntimeSpecMap<RustboltHashDigest>>,
...
impl CompilationRecords {
  ...
  fn record_hash(compilation: &Compilation) -> Option<RustboltHashDigest> {
    compilation.hash.clone()
  }

  fn record_modules(
    compilation: &Compilation,
  ) -> FxHashMap<ModuleId, RuntimeSpecMap<RustboltHashDigest>> {
    compilation
      .chunk_graph
  ...
  fn record_runtime_modules(compilation: &Compilation) -> IdentifierMap<RustboltHashDigest> {
    compilation
      .runtime_modules
      .keys()
      .map(|identifier| {
