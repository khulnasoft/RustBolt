...
use rustbolt_fs::ReadableFileSystem;
use rustbolt_loader_runner::DescriptionData;
use rustbolt_paths::AssertUtf8;
use rustbolt_util::location::try_line_column_length_to_offset_length;
...
/// Proxy to [nodejs_resolver::Error] or [rustbolt_resolver::ResolveError]
#[derive(Debug)]
pub enum ResolveInnerError {
  RustboltResolver(rustbolt_resolver::ResolveError),
}

/// Proxy to [rustbolt_resolver::ResolveOptions]
pub enum ResolveInnerOptions<'a> {
  RustboltResolver(&'a rustbolt_resolver::ResolveOptions),
}

impl fmt::Debug for ResolveInnerOptions<'_> {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    match self {
      Self::RustboltResolver(options) => {
        write!(f, "{options:?}")
      }
    }
  }
...
impl ResolveInnerOptions<'_> {
  pub fn is_enforce_extension_enabled(&self) -> bool {
    match self {
      Self::RustboltResolver(options) => matches!(
        options.enforce_extension,
        rustbolt_resolver::EnforceExtension::Enabled
      ),
    }
  }
  ...
  pub fn extensions(&self) -> impl Iterator<Item = &String> {
    match self {
      Self::RustboltResolver(options) => options.extensions.iter(),
    }
  }

  pub fn main_files(&self) -> impl Iterator<Item = &String> {
    match self {
      Self::RustboltResolver(options) => options.main_files.iter(),
    }
  }

  pub fn modules(&self) -> impl Iterator<Item = &String> {
    match self {
      Self::RustboltResolver(options) => options.modules.iter(),
    }
  }
}

/// Proxy to [rustbolt_resolver::Resolver]
///
/// Internal caches are shared.
#[derive(Debug)]
pub struct Resolver {
  resolver: rustbolt_resolver::ResolverGeneric<BoxFS>,
}

impl Resolver {
  pub fn new(options: Resolve, fs: Arc<dyn ReadableFileSystem>) -> Self {
    Self::new_rustbolt_resolver(options, fs)
  }

  fn new_rustbolt_resolver(options: Resolve, fs: Arc<dyn ReadableFileSystem>) -> Self {
    let options = to_rustbolt_resolver_options(options, false, DependencyCategory::Unknown);
    let boxfs = BoxFS::new(fs);
    let resolver = rustbolt_resolver::ResolverGeneric::new_with_file_system(boxfs, options);
    Self { resolver }
  }
  ...
  pub fn clone_with_options(
    ...
    let options = to_rustbolt_resolver_options(
      options,
  ...
  pub fn options(&self) -> ResolveInnerOptions<'_> {
    ResolveInnerOptions::RustboltResolver(self.resolver.options())
  }
  ...
  pub fn resolve(&self, path: &Path, request: &str) -> Result<ResolveResult, ResolveInnerError> {
    match self.resolver.resolve(path, request) {
      ...
      Err(rustbolt_resolver::ResolveError::Ignored(_)) => Ok(ResolveResult::Ignored),
      Err(error) => Err(ResolveInnerError::RustboltResolver(error)),
    }
  }
...
impl ResolveInnerError {
  pub fn into_resolve_error(self, args: &ResolveArgs<'_>) -> Box<dyn Diagnostic + Send + Sync> {
    match self {
      Self::RustboltResolver(error) => map_rustbolt_resolver_error(error, args),
    }
  }
}

fn to_rustbolt_resolver_options(
  options: Resolve,
  resolve_to_context: bool,
  dependency_type: DependencyCategory,
) -> rustbolt_resolver::ResolveOptions {
  let options = options.merge_by_dependency(dependency_type);
  let tsconfig = options.tsconfig.map(|c| c.into());
  let enforce_extension =
    options
      .enforce_extension
      .map_or(rustbolt_resolver::EnforceExtension::Auto, |e| match e {
        true => rustbolt_resolver::EnforceExtension::Enabled,
        false => rustbolt_resolver::EnforceExtension::Disabled,
      });
  ...
  let alias = options
    ...
    .map(|(key, value)| {
      let value = value
        .into_iter()
        .map(|x| match x {
          AliasMap::Path(target) => rustbolt_resolver::AliasValue::Path(target),
          AliasMap::Ignore => rustbolt_resolver::AliasValue::Ignore,
        })
        .collect();
      (key, value)
    })
  ...
  let restrictions = options
    ...
    .unwrap_or_default()
    .into_iter()
    .map(|s| rustbolt_resolver::Restriction::Path(s.into()))
    .collect();
  let roots = options
    .roots
    .unwrap_or_default()
...
fn map_rustbolt_resolver_error(
  error: rustbolt_resolver::ResolveError,
  args: &ResolveArgs<'_>,
) -> Box<dyn Diagnostic + Send + Sync> {
  match error {
    rustbolt_resolver::ResolveError::IOError(error) => diagnostic!("{}", error).boxed(),
    rustbolt_resolver::ResolveError::Recursion => map_resolver_error(true, args),
    rustbolt_resolver::ResolveError::NotFound(_) => map_resolver_error(false, args),
    rustbolt_resolver::ResolveError::JSON(error) => {
