...
use rustbolt_error::{error, Result};
use rustbolt_hook::define_hook;
use rustbolt_loader_runner::{get_scheme, Loader, Scheme};
use rustbolt_paths::Utf8PathBuf;
use rustbolt_util::MergeFrom;
...
pub struct NormalModuleFactoryHooks {
  ...
  /// Rustbolt resolves it when normalizing configuration.
  ...
  fn merge_global_parser_and_generator_options(
    ...
    let global_parser = self.options.module.parser.as_ref().and_then(|p| {
      ...
      match module_type {
        ModuleType::JsAuto | ModuleType::JsDynamic | ModuleType::JsEsm => {
          ...
          rustbolt_util::merge_from_optional_with(
        ...
        ModuleType::CssAuto | ModuleType::CssModule => rustbolt_util::merge_from_optional_with(
          p.get("css").cloned(),
          options,
    ...
    let global_generator = self.options.module.generator.as_ref().and_then(|g| {
      ...
      match module_type {
        ...
        ModuleType::Json => rustbolt_util::merge_from_optional_with(
          g.get("json").cloned(),
          options,
    ...
    let parser = rustbolt_util::merge_from_optional_with(
      global_parser,
      parser.as_ref(),
    ...
    let generator = rustbolt_util::merge_from_optional_with(
      global_generator,
      generator.as_ref(),
      |global, local| match (&global, local) {
