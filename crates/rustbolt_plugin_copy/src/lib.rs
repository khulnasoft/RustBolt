...
use regex::Regex;
use rustbolt_core::{
  rustbolt_sources::{RawSource, Source},
...
use rustbolt_error::{Diagnostic, DiagnosticError, Error, ErrorExt, Result};
use rustbolt_hash::{HashDigest, HashFunction, HashSalt, RustboltHash, RustboltHashDigest};
use rustbolt_hook::{plugin, plugin_hook};
use rustbolt_paths::{AssertUtf8, Utf8Path, Utf8PathBuf};
use rustbolt_util::infallible::ResultInfallibleExt as _;
use sugar_path::SugarPath;

#[derive(Debug)]
pub struct CopyRustboltPluginOptions {
...
impl CopyRustboltPlugin {
  ...
  fn get_content_hash(
    ...
    let mut hasher = RustboltHash::with_salt(function, salt);
...
#[plugin_hook(CompilationProcessAssets for CopyRustboltPlugin, stage = Compilation::PROCESS_ASSETS_STAGE_ADDITIONAL)]
async fn process_assets(&self, compilation: &mut Compilation) -> Result<()> {
  let logger = compilation.get_logger("rustbolt.CopyRustboltPlugin");
...
impl Plugin for CopyRustboltPlugin {
  fn apply(
    &self,
