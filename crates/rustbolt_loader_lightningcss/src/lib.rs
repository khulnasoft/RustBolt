...
use rustbolt_cacheable::{cacheable, cacheable_dyn, with::Skip};
use rustbolt_core::{
  rustbolt_sources::{encode_mappings, Mapping, OriginalLocation, SourceMap},
  Loader, LoaderContext, RunnerContext,
};
use rustbolt_error::Result;
use rustbolt_loader_runner::{Identifiable, Identifier};
...
impl LightningCssLoader {
  ...
  async fn loader_impl(&self, loader_context: &mut LoaderContext<RunnerContext>) -> Result<()> {
    ...
    let content_str = match &content {
      rustbolt_core::Content::String(s) => Cow::Borrowed(s.as_str()),
      rustbolt_core::Content::Buffer(buf) => String::from_utf8_lossy(buf),
    };
    ...
    let stylesheet = StyleSheet::parse(&content_str, option.clone())
      .map_err(|e| rustbolt_error::error!(e.to_string()))?;
    ...
    stylesheet
      ...
      .map_err(|e| rustbolt_error::error!(e))?;
    ...
    let content = stylesheet
      ...
      .map_err(|_| rustbolt_error::error!("failed to generate css"))?;
    ...
    if enable_sourcemap {
      ...
      let rustbolt_source_map = SourceMap::new(
        mappings,
        source_map
          .get_sources()
          .iter()
      ...
      loader_context.finish_with((content.code, rustbolt_source_map));
    } else {
...
impl Identifiable for LightningCssLoader {
  fn identifier(&self) -> rustbolt_loader_runner::Identifier {
    self.id
  }
}
