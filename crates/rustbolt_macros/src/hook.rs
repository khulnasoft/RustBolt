...
impl DefineHookInput {
  pub fn expand(self) -> Result<TokenStream> {
    ...
    let attr = if is_async {
      Some(quote! { #[::rustbolt_hook::__macro_helper::async_trait] })
    } else {
      None
    };
    ...
    Ok(quote! {
      ...
      pub struct #hook_name {
        ...
        interceptors: Vec<Box<dyn rustbolt_hook::Interceptor<Self> + Send + Sync>>,
      }

      impl rustbolt_hook::Hook for #hook_name {
        type Tap = Box<dyn #trait_name + Send + Sync>;

        fn used_stages(&self) -> rustbolt_hook::__macro_helper::FxHashSet<i32> {
          rustbolt_hook::__macro_helper::FxHashSet::from_iter(self.taps.iter().map(|h| h.stage()))
        }

        fn intercept(&mut self, interceptor: impl rustbolt_hook::Interceptor<Self> + Send + Sync + 'static) {
  ...
  pub fn return_type(&self) -> TokenStream {
    match self {
      Self::AsyncSeriesBail { ret } | Self::SyncSeriesBail { ret } => {
        if let Some(ret) = ret {
          quote! { rustbolt_hook::__macro_helper::Result<std::option::Option<#ret>> }
        } else {
          quote! { rustbolt_hook::__macro_helper::Result<std::option::Option<()>> }
        }
      }
      Self::AsyncSeriesWaterfall { ret } => {
        quote! { rustbolt_hook::__macro_helper::Result<#ret> }
      }
      _ => quote! { rustbolt_hook::__macro_helper::Result<()> },
    }
  }
