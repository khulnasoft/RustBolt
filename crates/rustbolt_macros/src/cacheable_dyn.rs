...
pub fn impl_trait(mut input: ItemTrait) -> TokenStream {
  ...
  input
    .supertraits
    .push(parse_quote! { rustbolt_cacheable::r#dyn::SerializeDyn });
  ...
  quote! {
      #input
        ...
        const _: () = {
            ...
            use rustbolt_cacheable::__private::rkyv::{
...
pub fn impl_impl(mut input: ItemImpl) -> TokenStream {
  ...
  quote! {
      const #dyn_id_ident: u64 = rustbolt_cacheable::xxhash_rust::const_xxh64::xxh64(concat!(module_path!(), ":", line!()).as_bytes(), 0);

      #input

      const _: () = {
          use rustbolt_cacheable::__private::{
              inventory,
