...
pub fn expand_merge_from_derive(input: DeriveInput) -> Result<TokenStream> {
  ...
  Ok(quote! {
    impl #impl_generics rustbolt_util::MergeFrom for #name #ty_generics #where_clause {
    ...
    Data::Struct(ref data) => match data.fields {
      Fields::Named(ref fields) => {
        let recurse = fields.named.iter().map(|f| {
          ...
          quote_spanned! {f.span()=>
            self.#name = rustbolt_util::MergeFrom::merge_from(self.#name, &other.#name);
          }
        });
        Ok(quote! {
          #(#recurse)*
      ...
      Fields::Unnamed(ref fields) => {
        let recurse = fields.unnamed.iter().enumerate().map(|(i, f)| {
          ...
          quote_spanned! {f.span()=>
            self.#index = rustbolt_util::MergeFrom::merge_from(self.#index, &other.#index);
          }
        });
        Ok(quote! {
          #(#recurse)*
            ...
            let (fields, merged_fields): (Vec<(TokenStream, TokenStream)>, Vec<TokenStream>) = f
              ...
              .map(|(i, f)| {
                ...
                (
                  ...
                  quote_spanned! {f.span()=>
                    #field_name: rustbolt_util::MergeFrom::merge_from(#a_name, #b_name)
                  },
                )
              })
              .unzip();
            ...
            let (fields, merged_fields): (Vec<(TokenStream, TokenStream)>, Vec<TokenStream>) = f
              ...
              .map(|(i, f)| {
                ...
                (
                  ...
                  quote_spanned! {f.span()=>
                    rustbolt_util::MergeFrom::merge_from(#a_name, #b_name)
                  },
                )
              })
              .unzip();
