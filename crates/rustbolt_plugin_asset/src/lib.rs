...
use rustbolt_core::{
  rustbolt_sources::{BoxSource, RawBufferSource, RawStringSource, SourceExt},
...
use rustbolt_error::{error, Diagnostic, IntoTWithDiagnosticArray, Result};
use rustbolt_hash::{RustboltHash, RustboltHashDigest};
use rustbolt_hook::{plugin, plugin_hook};
use rustbolt_util::{ext::DynHash, identifier::make_paths_relative};

mod asset_exports_dependency;

pub const AUTO_PUBLIC_PATH_PLACEHOLDER: &str = "__RUSTBOLT_PLUGIN_ASSET_AUTO_PUBLIC_PATH__";
...
impl AssetParserAndGenerator {
  ...
  fn decode_data_uri_content(encoding: &str, content: &str, source: &BoxSource) -> Vec<u8> {
    if encoding == "base64"
      && let Some(cleaned) = rustbolt_base64::clean_base64(content)
    {
      return rustbolt_base64::decode_to_vec(cleaned.as_bytes())
  ...
  fn hash_for_source(
    ...
    let mut hasher = RustboltHash::from(&compiler_options.output);
  ...
  fn get_encoded_content(
    ...
    if encoding == DEFAULT_ENCODING {
      return Ok(rustbolt_base64::encode_to_string(source.buffer()));
    }
...
impl ParserAndGenerator for AssetParserAndGenerator {
  ...
  async fn parse<'a>(
    &mut self,
    parse_context: rustbolt_core::ParseContext<'a>,
  ) -> Result<rustbolt_error::TWithDiagnosticArray<rustbolt_core::ParseResult>> {
    let ParseContext {
      source,
      build_meta,
      build_info,
    ...
    Ok(
      rustbolt_core::ParseResult {
        // different from webpack
        // Rustbolt: when set asset as entry, output a js chunk with default export
  ...
  fn get_concatenation_bailout_reason(
    &self,
    _module: &dyn rustbolt_core::Module,
    _mg: &ModuleGraph,
    _cg: &ChunkGraph,
...
impl Plugin for AssetPlugin {
  ...
  fn apply(
    ...
    ctx.context.register_parser_and_generator_builder(
      rustbolt_core::ModuleType::Asset,
    ...
    ctx.context.register_parser_and_generator_builder(
      rustbolt_core::ModuleType::AssetInline,
      Box::new(|_, _| Box::new(AssetParserAndGenerator::with_inline())),
    );

    ctx.context.register_parser_and_generator_builder(
      rustbolt_core::ModuleType::AssetResource,
    ...
    ctx.context.register_parser_and_generator_builder(
      rustbolt_core::ModuleType::AssetSource,
