...
extern crate rustbolt_allocator;
...
use rustbolt_collections::UkeyMap;
use rustbolt_core::{
...
pub use rsdoctor::*;
use rustbolt_tracing::{ChromeTracer, StdoutTracer, Tracer};
pub use runtime::*;
...
impl JsCompiler {
  ...
  pub fn new(
    ...
    let compiler_options: rustbolt_core::CompilerOptions = options
      .try_into()
      .map_err(|e| Error::from_reason(format!("{e}")))?;
    ...
    let rustbolt = rustbolt_core::Compiler::new(
      compiler_path,
      compiler_options,
      plugins,
    ...
    Ok(Self {
      compiler: Box::pin(Compiler::from(rustbolt)),
...
fn concurrent_compiler_error() -> Error {
  Error::new(
    napi::Status::GenericFailure,
    "ConcurrentCompilationError: You ran rustbolt twice. Each instance only supports a single concurrent compilation at a time.",
  )
}
...
fn init() {
  ...
  const ENV_BLOCKING_THREADS: &str = "RUSTBOLT_BLOCKING_THREADS";
...
fn print_error_diagnostic(e: rustbolt_error::Error, colored: bool) -> String {
  Diagnostic::from(e)
  ...
  GLOBAL_TRACE_STATE.with(|state| {
    ...
    if let TraceState::Off = *state {
      let mut tracer: Box<dyn Tracer> = match layer.as_str() {
        ...
        #[cfg(not(target_family = "wasm"))]
        "otel" => {
          use rustbolt_tracing::OtelTracer;
          use rustbolt_napi::napi::bindgen_prelude::within_runtime_if_available;
      ...
      if let Some(layer) = tracer.setup(&output) {
        if let Ok(default_level) = Level::from_str(&filter) {
          let filter = tracing_subscriber::filter::Targets::new()
            .with_target("rustbolt_core", default_level)
            .with_target("node_binding", default_level)
            .with_target("rustbolt_loader_swc", default_level)
            .with_target("rustbolt_loader_runner", default_level)
            .with_target("rustbolt_plugin_javascript", default_level)
            .with_target("rustbolt_resolver", Level::WARN);
