...
use rustbolt_core::{ResolveOptionsWithDependencyType, Resolver, ResolverFactory, ResourceData};
...
impl JsResolver {
  ...
  pub fn resolve_sync(
    ...
    match self.resolver.resolve(Path::new(&path), &request) {
      Ok(rustbolt_core::ResolveResult::Resource(resource)) => {
        Ok(Either::A(ResourceData::from(resource).into()))
      }
      Ok(rustbolt_core::ResolveResult::Ignored) => Ok(Either::B(false)),
