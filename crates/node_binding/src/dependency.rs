...
use rustbolt_core::{Compilation, CompilationId, DependencyId};
use rustbolt_napi::OneShotInstanceRef;
use rustbolt_plugin_javascript::dependency::{
...
pub struct Dependency {
  ...
  pub(crate) dependency: NonNull<dyn rustbolt_core::Dependency>,
}

impl Dependency {
  fn as_ref(&mut self) -> napi::Result<(&dyn rustbolt_core::Dependency, Option<&Compilation>)> {
    if let Some(compilation) = self.compilation {
      ...
      if let Some(dependency) = module_graph.dependency_by_id(&self.dependency_id) {
        self.dependency = {
          ...
          NonNull::new(dependency.as_ref() as *const dyn rustbolt_core::Dependency
            as *mut dyn rustbolt_core::Dependency)
          .unwrap()
        };
  ...
  fn as_mut(&mut self) -> napi::Result<&mut dyn rustbolt_core::Dependency> {
    // SAFETY:
    // We need to make users aware in the documentation that values obtained within the JS hook callback should not be used outside the scope of the callback.
    // We do not guarantee that the memory pointed to by the pointer remains valid when used outside the scope.
...
pub struct DependencyWrapper {
  ...
  dependency: NonNull<dyn rustbolt_core::Dependency>,
...
impl DependencyWrapper {
  pub fn new(
    dependency: &dyn rustbolt_core::Dependency,
    compilation_id: CompilationId,
    ...
    Self {
      ...
      dependency: NonNull::new(
        dependency as *const dyn rustbolt_core::Dependency as *mut dyn rustbolt_core::Dependency,
      )
      .unwrap(),
      compilation_id,
