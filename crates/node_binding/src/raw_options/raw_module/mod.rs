...
#[rustbolt_napi_macros::tagged_union]
...
impl TryFrom<RawRuleSetLogicalConditions> for rustbolt_core::RuleSetLogicalConditions {
  type Error = rustbolt_error::Error;

  fn try_from(value: RawRuleSetLogicalConditions) -> rustbolt_error::Result<Self> {
    Ok(Self {
      and: value
        .and
        .map(|i| {
          i.into_iter()
            ...
            .collect::<rustbolt_error::Result<Vec<_>>>()
        })
        .transpose()?,
      or: value
        .or
...
impl TryFrom<RawRuleSetCondition> for rustbolt_core::RuleSetCondition {
  type Error = rustbolt_error::Error;

  fn try_from(x: RawRuleSetCondition) -> rustbolt_error::Result<Self> {
    let result = match x {
      RawRuleSetCondition::string(s) => Self::String(s),
      RawRuleSetCondition::regexp(r) => {
        let reg = RustboltRegex::with_flags(&r.source, &r.flags)?;
        Self::Regexp(reg)
      }
      RawRuleSetCondition::logical(mut l) => {
        ...
        Self::Logical(Box::new(rustbolt_core::RuleSetLogicalConditions::try_from(
          l,
        )?))
      }
...
impl TryFrom<RawRuleSetCondition> for rustbolt_core::RuleSetConditionWithEmpty {
  type Error = rustbolt_error::Error;
...
pub struct RawModuleRule {
  ...
  pub rustbolt_resource: Option<RawRuleSetCondition>,
...
pub struct RawJavascriptParserOptions {
  ...
  #[napi(ts_type = "RegExp")]
  pub wrapped_context_reg_exp: Option<RustboltRegex>,
  ...
  /// This option is experimental in Rustbolt only and subject to change or be removed anytime.
  /// @experimental
...
impl TryFrom<RawModuleRule> for ModuleRule {
  ...
  fn try_from(value: RawModuleRule) -> rustbolt_error::Result<Self> {
    let uses = value.r#use.map(|raw| match raw {
      Either::A(array) => {
        let uses = array
          ...
          .collect::<Vec<_>>();
        Ok::<ModuleRuleUse, rustbolt_error::Error>(ModuleRuleUse::Array(uses))
      }
      Either::B(tsfn) => Ok::<ModuleRuleUse, rustbolt_error::Error>(ModuleRuleUse::Func(Box::new(
        move |ctx: FuncUseCtx| {
          let tsfn = tsfn.clone();
          Box::pin(async move {
    ...
    let description_data = value
      ...
      .map(|data| {
        data
          ...
          .collect::<rustbolt_error::Result<DescriptionData>>()
      })
      .transpose()?;
...
impl TryFrom<RawModuleOptions> for ModuleOptions {
  ...
  fn try_from(value: RawModuleOptions) -> rustbolt_error::Result<Self> {
    let rules = value
      .rules
      .into_iter()
      .map(|rule| rule.try_into())
      .collect::<rustbolt_error::Result<Vec<ModuleRule>>>()?;
    Ok(ModuleOptions {
      rules,
      parser: value
        .parser
        .map(|x| {
          x.into_iter()
            .map(|(k, v)| Ok((k, v.into())))
            .collect::<std::result::Result<ParserOptionsMap, rustbolt_error::Error>>()
        })
        .transpose()?,
      generator: value
        .generator
        .map(|x| {
          x.into_iter()
            .map(|(k, v)| Ok((k, v.into())))
            .collect::<std::result::Result<GeneratorOptionsMap, rustbolt_error::Error>>()
        })
        .transpose()?,
      no_parse: value
        .no_parse
