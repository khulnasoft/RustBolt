...
use rustbolt_core::{Filename, SourceType, DEFAULT_DELIMITER};
use rustbolt_napi::{string::JsStringExt, threadsafe_function::ThreadsafeFunction};
use rustbolt_plugin_split_chunks::ChunkNameGetter;
...
pub struct RawCacheGroupOptions {
  ...
  #[debug(skip)]
  pub r#type: Option<Either<RustboltRegex, JsString>>,
  #[napi(ts_type = "RegExp | string | ((layer?: string) => boolean)")]
  #[debug(skip)]
  pub layer: Option<Either3<RustboltRegex, JsString, ThreadsafeFunction<Option<String>, bool>>>,
  pub automatic_name_delimiter: Option<String>,
...
impl From<RawSplitChunksOptions> for rustbolt_plugin_split_chunks::PluginOptions {
  fn from(raw_opts: RawSplitChunksOptions) -> Self {
    use rustbolt_plugin_split_chunks::SplitChunkSizes;
      ...
      raw_opts
        ...
        .map(|v| {
          ...
          let r#type = v
            ...
            .unwrap_or_else(rustbolt_plugin_split_chunks::create_default_module_type_filter);

          let layer = v
            .layer
            .map(create_module_layer_filter)
            .unwrap_or_else(rustbolt_plugin_split_chunks::create_default_module_layer_filter);
          ...
          rustbolt_plugin_split_chunks::CacheGroup {
            ...
            chunk_filter: v.chunks.map(create_chunks_filter).unwrap_or_else(|| {
              overall_chunk_filter
                .clone()
                .unwrap_or_else(rustbolt_plugin_split_chunks::create_async_chunk_filter)
            }),
            min_chunks,
            min_size,
            min_size_reduction,
    ...
    rustbolt_plugin_split_chunks::PluginOptions {
      cache_groups,
      fallback_cache_group: rustbolt_plugin_split_chunks::FallbackCacheGroup {
        chunks_filter: fallback_chunks_filter.unwrap_or_else(|| {
          overall_chunk_filter
            .clone()
            .unwrap_or_else(rustbolt_plugin_split_chunks::create_all_chunk_filter)
        }),
...
fn create_module_type_filter(
  raw: Either<RustboltRegex, JsString>,
) -> rustbolt_plugin_split_chunks::ModuleTypeFilter {
  match raw {
...
fn create_module_layer_filter(
  raw: Either3<RustboltRegex, JsString, ThreadsafeFunction<Option<String>, bool>>,
) -> rustbolt_plugin_split_chunks::ModuleLayerFilter {
  match raw {
