...
pub struct RawHttpUriPluginOptions {
  ...
  pub allowed_uris: Option<Vec<Either<String, RustboltRegex>>>,
...
pub fn get_http_uri_plugin(options: RawHttpUriPluginOptions) -> Box<dyn rustbolt_core::Plugin> {
  // Use NativeFileSystem for HTTP caching
  let fs = Arc::new(rustbolt_fs::NativeFileSystem::new(false));
