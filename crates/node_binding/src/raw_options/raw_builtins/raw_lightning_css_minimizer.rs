...
use rustbolt_plugin_lightning_css_minimizer::{
...
#[derive(Debug)]
#[napi(object)]
pub struct RawLightningCssMinimizerRustboltPluginOptions {
...
impl TryFrom<RawLightningCssMinimizerRustboltPluginOptions> for PluginOptions {
  type Error = rustbolt_error::Error;

  fn try_from(value: RawLightningCssMinimizerRustboltPluginOptions) -> Result<Self> {
    Ok(Self {
      ...
      minimizer_options: MinimizerOptions {
        ...
        targets: value
          ...
          .targets
          .map(|t| {
            rustbolt_loader_lightningcss::lightningcss::targets::Browsers::from_browserslist(t)
          })
          .transpose()
          .map_err(|e| rustbolt_error::error!("Failed to parse browserslist: {}", e))?
          .flatten(),
