...
use rustbolt_core::{CompilationId, CompilerId, Module, ModuleIdentifier};
use rustbolt_napi::threadsafe_function::ThreadsafeFunction;
use rustbolt_plugin_lazy_compilation::{
  ...
  fn from(value: RawLazyCompilationTest) -> Self {
    match value.0 {
      Either::A(regex) => Self::Regex(
        RustboltRegex::with_flags(&regex.source, &regex.flags).unwrap_or_else(|_| {
...
impl Backend for JsBackend {
  async fn module(
    ...
    path: String,
  ) -> rustbolt_error::Result<ModuleInfo> {
