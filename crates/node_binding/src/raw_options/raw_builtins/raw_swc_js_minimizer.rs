...
use rustbolt_error::{miette::IntoDiagnostic, Result};
use rustbolt_plugin_swc_js_minimizer::{
...
pub struct RawSwcJsMinimizerRustboltPluginOptions {
...
impl TryFrom<RawSwcJsMinimizerRustboltPluginOptions> for PluginOptions {
  type Error = rustbolt_error::Error;

  fn try_from(value: RawSwcJsMinimizerRustboltPluginOptions) -> Result<Self> {
    let compress = try_deserialize_into::<
      BoolOrDataConfig<rustbolt_plugin_swc_js_minimizer::TerserCompressorOptions>,
    >(value.minimizer_options.compress)?
    .or(|| BoolOrDataConfig::from_bool(true));
    let mangle = try_deserialize_into::<
      BoolOrDataConfig<rustbolt_plugin_swc_js_minimizer::MangleOptions>,
