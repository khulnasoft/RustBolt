...
use raw_lightning_css_minimizer::RawLightningCssMinimizerRustboltPluginOptions;
use raw_sri::RawSubresourceIntegrityPluginOptions;
use rustbolt_core::{BoxPlugin, Plugin, PluginExt};
...
use rustbolt_plugin_asset::AssetPlugin;
use rustbolt_plugin_banner::BannerPlugin;
use rustbolt_plugin_circular_dependencies::CircularDependencyRustboltPlugin;
use rustbolt_plugin_context_replacement::ContextReplacementPlugin;
use rustbolt_plugin_copy::{CopyRustboltPlugin, CopyRustboltPluginOptions};
use rustbolt_plugin_css::CssPlugin;
use rustbolt_plugin_devtool::{
...
use rustbolt_plugin_dll::{
  DllEntryPlugin, DllReferenceAgencyPlugin, FlagAllModulesAsUsedPlugin, LibManifestPlugin,
};
use rustbolt_plugin_dynamic_entry::DynamicEntryPlugin;
use rustbolt_plugin_ensure_chunk_conditions::EnsureChunkConditionsPlugin;
use rustbolt_plugin_entry::EntryPlugin;
use rustbolt_plugin_externals::{
  electron_target_plugin, http_externals_rustbolt_plugin, node_target_plugin, ExternalsPlugin,
};
use rustbolt_plugin_hmr::HotModuleReplacementPlugin;
use rustbolt_plugin_html::HtmlRustboltPlugin;
use rustbolt_plugin_ignore::IgnorePlugin;
use rustbolt_plugin_javascript::{
...
use rustbolt_plugin_json::JsonPlugin;
use rustbolt_plugin_library::enable_library_plugin;
use rustbolt_plugin_lightning_css_minimizer::LightningCssMinimizerRustboltPlugin;
use rustbolt_plugin_limit_chunk_count::LimitChunkCountPlugin;
use rustbolt_plugin_merge_duplicate_chunks::MergeDuplicateChunksPlugin;
use rustbolt_plugin_mf::{
...
use rustbolt_plugin_no_emit_on_errors::NoEmitOnErrorsPlugin;
use rustbolt_plugin_progress::ProgressPlugin;
use rustbolt_plugin_real_content_hash::RealContentHashPlugin;
use rustbolt_plugin_remove_duplicate_modules::RemoveDuplicateModulesPlugin;
use rustbolt_plugin_remove_empty_chunks::RemoveEmptyChunksPlugin;
use rustbolt_plugin_rsdoctor::RsdoctorPlugin;
use rustbolt_plugin_runtime::{
...
use rustbolt_plugin_runtime_chunk::RuntimeChunkPlugin;
use rustbolt_plugin_schemes::{DataUriPlugin, FileUriPlugin};
use rustbolt_plugin_size_limits::SizeLimitsPlugin;
use rustbolt_plugin_sri::SubresourceIntegrityPlugin;
use rustbolt_plugin_swc_js_minimizer::SwcJsMinimizerRustboltPlugin;
use rustbolt_plugin_warn_sensitive_module::WarnCaseSensitiveModulesPlugin;
use rustbolt_plugin_wasm::{
  enable_wasm_loading_plugin, AsyncWasmPlugin, FetchCompileAsyncWasmPlugin,
};
use rustbolt_plugin_web_worker_template::web_worker_template_plugin;
use rustbolt_plugin_worker::WorkerPlugin;

pub use self::{
  raw_banner::RawBannerPluginOptions,
  raw_circular_dependency::RawCircularDependencyRustboltPluginOptions,
  raw_copy::RawCopyRustboltPluginOptions,
  raw_dll::{RawDllEntryPluginOptions, RawLibManifestPluginOptions},
  raw_html::RawHtmlRustboltPluginOptions,
  ...
  raw_swc_js_minimizer::RawSwcJsMinimizerRustboltPluginOptions,
};
use self::{
...
pub enum BuiltinPluginName {
  ...
  // rustbolt specific plugins
  // naming format follow XxxRustboltPlugin
  HttpExternalsRustboltPlugin,
  CopyRustboltPlugin,
  HtmlRustboltPlugin,
  ...
  RsdoctorPlugin,
  CircularDependencyRustboltPlugin,

  // rustbolt js adapter plugins
  // naming format follow XxxRustboltPlugin
  JsLoaderRustboltPlugin,
...
impl BuiltinPlugin {
  pub fn append_to(self, _env: Env, plugins: &mut Vec<BoxPlugin>) -> rustbolt_error::Result<()> {
    match self.name {
      // webpack also have these plugins
      ...
      BuiltinPluginName::SplitChunksPlugin => {
        use rustbolt_plugin_split_chunks::SplitChunksPlugin;
      ...
      BuiltinPluginName::HttpExternalsRustboltPlugin => {
        let plugin_options = downcast_into::<RawHttpExternalsRustboltPluginOptions>(self.options)?;
        let plugin = http_externals_rustbolt_plugin(plugin_options.css, plugin_options.web_async);
        plugins.push(plugin);
      }
      BuiltinPluginName::SwcJsMinimizerRustboltPlugin => {
        let plugin = SwcJsMinimizerRustboltPlugin::new(
          downcast_into::<RawSwcJsMinimizerRustboltPluginOptions>(self.options)?.try_into()?,
        )
        .boxed();
        plugins.push(plugin);
      }
      ...
      BuiltinPluginName::CssExtractRustboltPlugin => {
        let plugin = rustbolt_plugin_extract_css::plugin::PluginCssExtract::new(
          downcast_into::<RawCssExtractPluginOption>(self.options)?.into(),
        )
        .boxed();
        plugins.push(plugin);
      }
      BuiltinPluginName::CircularDependencyRustboltPlugin => plugins.push(
        CircularDependencyRustboltPlugin::new(
          downcast_into::<RawCircularDependencyRustboltPluginOptions>(self.options)?.into(),
        )
        .boxed(),
      ),
      ...
      BuiltinPluginName::LazyCompilationPlugin => {
        ...
        plugins.push(Box::new(
          rustbolt_plugin_lazy_compilation::plugin::LazyCompilationPlugin::new(
            options.cacheable,
            js_backend,
...
fn downcast_into<T: FromNapiValue + 'static>(o: JsUnknown) -> Result<T> {
  rustbolt_napi::downcast_into(o).into_rustbolt_result()
}
