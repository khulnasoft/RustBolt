...
use rustbolt_hook::{plugin, plugin_hook, Hook as _};
use rustbolt_plugin_html::HtmlRustboltPlugin;
use rustbolt_plugin_javascript::JsPlugin;
...
impl fmt::Debug for JsHooksAdapterPlugin {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(f, "rustbolt_plugin_js_hooks_adapter")
  }
}

#[async_trait]
impl rustbolt_core::Plugin for JsHooksAdapterPlugin {
  fn name(&self) -> &'static str {
    "rustbolt.JsHooksAdapterPlugin"
  }
...
async fn html_hooks_adapter_compilation(
  ...
  let mut hooks = HtmlRustboltPlugin::get_compilation_hooks_mut(compilation.id());
