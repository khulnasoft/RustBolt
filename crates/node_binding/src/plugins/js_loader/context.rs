...
use rustbolt_core::{LoaderContext, Module, RunnerContext};
use rustbolt_error::error;
use rustbolt_loader_runner::{LoaderItem, State as LoaderState};
use rustbolt_napi::threadsafe_js_value_ref::ThreadsafeJsValueRef;

use crate::{JsResourceData, JsRustboltError, ModuleObject};
...
pub struct JsLoaderContext {
  ...
  pub error: Option<JsRustboltError>,
...
impl TryFrom<&mut LoaderContext<RunnerContext>> for JsLoaderContext {
  ...
  fn try_from(
    cx: &mut rustbolt_core::LoaderContext<RunnerContext>,
    ...
    {
      use rustbolt_tracing::otel::{opentelemetry::global, tracing::OpenTelemetrySpanExt as _};
      use tracing::Span;
