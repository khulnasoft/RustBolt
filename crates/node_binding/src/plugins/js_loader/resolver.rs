...
use rustbolt_hook::plugin_hook;
use rustbolt_loader_lightningcss::{config::Config, LIGHTNINGCSS_LOADER_IDENTIFIER};
use rustbolt_loader_preact_refresh::PREACT_REFRESH_LOADER_IDENTIFIER;
use rustbolt_loader_react_refresh::REACT_REFRESH_LOADER_IDENTIFIER;
use rustbolt_loader_swc::{SwcLoader, SWC_LOADER_IDENTIFIER};
use rustbolt_paths::Utf8Path;
use rustc_hash::FxHashMap;
use tokio::sync::RwLock;

use super::{JsLoaderRustboltPlugin, JsLoaderRustboltPluginInner};
...
pub async fn get_builtin_loader(builtin: &str, options: Option<&str>) -> Result<BoxLoader> {
  ...
  if builtin.starts_with(SWC_LOADER_IDENTIFIER) {
    ...
    let loader = Arc::new(
      rustbolt_loader_swc::SwcLoader::new(options.as_ref())
        .map_err(|e| {
  ...
  if builtin.starts_with(LIGHTNINGCSS_LOADER_IDENTIFIER) {
    let config: rustbolt_loader_lightningcss::config::RawConfig =
  ...
  if builtin.starts_with(REACT_REFRESH_LOADER_IDENTIFIER) {
    return Ok(Arc::new(
      rustbolt_loader_react_refresh::ReactRefreshLoader::default().with_identifier(builtin.into()),
    ));
  }
  ...
  if builtin.starts_with(rustbolt_loader_testing::SIMPLE_ASYNC_LOADER_IDENTIFIER) {
    return Ok(Arc::new(rustbolt_loader_testing::SimpleAsyncLoader));
  }
  if builtin.starts_with(rustbolt_loader_testing::SIMPLE_LOADER_IDENTIFIER) {
    return Ok(Arc::new(rustbolt_loader_testing::SimpleLoader));
  }
  if builtin.starts_with(rustbolt_loader_testing::PITCHING_LOADER_IDENTIFIER) {
    return Ok(Arc::new(rustbolt_loader_testing::PitchingLoader));
  }
  if builtin.starts_with(rustbolt_loader_testing::PASS_THROUGH_LOADER_IDENTIFIER) {
    return Ok(Arc::new(rustbolt_loader_testing::PassthroughLoader));
  }
  if builtin.starts_with(rustbolt_loader_testing::NO_PASS_THROUGH_LOADER_IDENTIFIER) {
    return Ok(Arc::new(rustbolt_loader_testing::NoPassthroughLoader));
  }
  unreachable!("Unexpected builtin loader: {builtin}")
}

#[plugin_hook(NormalModuleFactoryResolveLoader for JsLoaderRustboltPlugin)]
