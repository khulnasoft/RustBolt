...
use rustbolt_loader_runner::State as LoaderState;

use super::{JsLoaderContext, JsLoaderRustboltPlugin, JsLoaderRustboltPluginInner};

#[plugin_hook(NormalModuleLoaderShouldYield for JsLoaderRustboltPlugin)]
...
#[plugin_hook(NormalModuleLoaderStartYielding for JsLoaderRustboltPlugin)]
...
pub(crate) fn merge_loader_context(
  ...
  let content = match from.content {
    Either::A(_) => None,
    Either::B(c) => Some(rustbolt_core::Content::from(Into::<Vec<u8>>::into(c))),
  };
  let source_map = from
    .source_map
    .as_ref()
    .map(|s| rustbolt_core::rustbolt_sources::SourceMap::from_slice(s))
    .transpose()
