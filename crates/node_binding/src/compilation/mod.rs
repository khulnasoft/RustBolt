...
use rustbolt_collections::{DatabaseItem, IdentifierSet};
use rustbolt_core::{
  rustbolt_sources::BoxSource, BoxDependency, Compilation, CompilationId, EntryOptions,
  FactorizeInfo, ModuleIdentifier,
};
use rustbolt_error::Diagnostic;
use rustbolt_napi::{napi::bindgen_prelude::*, NapiResultExt, OneShotRef};
use rustbolt_plugin_runtime::RuntimeModuleFromJs;
...
use crate::{
  ...
  JsModuleGraph, JsPathData, JsRustboltDiagnostic, JsRustboltError, JsStats,
...
impl JsCompilation {
  fn as_ref(&self) -> napi::Result<&'static Compilation> {
    // SAFETY: The memory address of rustbolt_core::Compilation will not change,
...
impl JsCompilation {
  ...
  pub fn set_asset_source(&mut self, name: String, source: JsCompatSource) -> Result<()> {
    ...
    match compilation.assets_mut().entry(name) {
      ...
      std::collections::hash_map::Entry::Vacant(e) => {
        e.insert(rustbolt_core::CompilationAsset::from(source));
      }
    };
    Ok(())
  }
  ...
  pub fn emit_asset(
    ...
    let asset_info: rustbolt_core::AssetInfo = js_asset_info.map(Into::into).unwrap_or_default();

    compilation.emit_asset(
      filename,
      rustbolt_core::CompilationAsset::new(Some(source.into()), asset_info),
    );
    Ok(())
  }
  ...
  #[napi]
  pub fn push_diagnostic(&mut self, diagnostic: JsRustboltDiagnostic) -> Result<()> {
    let compilation = self.as_mut()?;
  ...
  pub fn splice_diagnostic(
    ...
    replace_with: Vec<crate::JsRustboltDiagnostic>,
  ) -> Result<()> {
    let compilation = self.as_mut()?;
  ...
  #[napi]
  pub fn get_errors(&self) -> Result<Vec<JsRustboltError>> {
    let compilation = self.as_ref()?;
    ...
    Ok(
      compilation
        .get_errors_sorted()
        .map(|d| {
          JsRustboltError::try_from_diagnostic(d, colored)
            .expect("should convert diagnostic to `JsRustboltError`")
        })
        .collect(),
    )
  }
  ...
  #[napi]
  pub fn get_warnings(&self) -> Result<Vec<JsRustboltError>> {
    let compilation = self.as_ref()?;
  ...
  pub fn get_path_with_info(
    ...
    let mut asset_info = rustbolt_core::AssetInfo::default();
    let path =
    ...
    callbackify(env, callback, async {
      ...
      let res = module_executor
        .import_module(
          ...
          base_uri,
          original_module_context.map(rustbolt_core::Context::from),
