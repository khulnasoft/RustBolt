...
use rustbolt_core::{Resolve, ResolverFactory};
use rustbolt_fs::{NativeFileSystem, ReadableFileSystem};
...
impl JsResolverFactory {
  ...
  pub fn get(
    ...
    match r#type.as_str() {
      ...
      _ => {
        Err(napi::Error::from_reason(format!("Invalid resolver type '{}' specified. Rustbolt only supports 'normal', 'context', and 'loader' types.", r#type)))
      }
    }
  }
}
