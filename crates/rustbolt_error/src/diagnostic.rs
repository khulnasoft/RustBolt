...
use rustbolt_cacheable::{cacheable, with::Unsupported};
...
pub enum RustboltSeverity {
  #[default]
  Error,
  Warn,
}

pub type Severity = RustboltSeverity;

impl From<RustboltSeverity> for miette::Severity {
  fn from(value: RustboltSeverity) -> Self {
    match value {
      RustboltSeverity::Error => miette::Severity::Error,
      RustboltSeverity::Warn => miette::Severity::Warning,
    }
  }
}

impl From<miette::Severity> for RustboltSeverity {
  fn from(value: miette::Severity) -> Self {
    match value {
      miette::Severity::Error => RustboltSeverity::Error,
      miette::Severity::Warning => RustboltSeverity::Warn,
      miette::Severity::Advice => unimplemented!("Not supported miette severity"),
    }
  }
}

impl From<&str> for RustboltSeverity {
  fn from(value: &str) -> Self {
    let s = value.cow_to_ascii_lowercase();
    match s.as_ref() {
      "warning" => RustboltSeverity::Warn,
      _ => RustboltSeverity::Error,
    }
  }
}

impl fmt::Display for RustboltSeverity {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(
      f,
      "{}",
      match self {
        RustboltSeverity::Error => "error",
        RustboltSeverity::Warn => "warning",
      }
    )
  }
}
