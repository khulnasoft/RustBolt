...
use crate::runtime_module::{RustboltUniqueIdRuntimeModule, RustboltVersionRuntimeModule};
...
async fn additional_tree_runtime_requirements(
  ...
  if match &self.force {
    ...
    BundlerInfoForceMode::Auto => runtime_requirements.contains(RuntimeGlobals::RUSTBOLT_VERSION),
  } {
    runtime_requirements.insert(RuntimeGlobals::REQUIRE);
    runtime_requirements.insert(RuntimeGlobals::RUSTBOLT_VERSION);
  }

  if match &self.force {
    BundlerInfoForceMode::All => true,
    BundlerInfoForceMode::Partial(s) => s.get("uniqueId").is_some(),
    BundlerInfoForceMode::Auto => runtime_requirements.contains(RuntimeGlobals::RUSTBOLT_UNIQUE_ID),
  } {
    runtime_requirements.insert(RuntimeGlobals::REQUIRE);
    runtime_requirements.insert(RuntimeGlobals::RUSTBOLT_UNIQUE_ID);
  }
  Ok(())
}
...
async fn runtime_requirements_in_tree(
  ...
  if runtime_requirements.contains(RuntimeGlobals::RUSTBOLT_VERSION) {
    compilation.add_runtime_module(
      chunk_ukey,
      Box::new(RustboltVersionRuntimeModule::new(self.version.clone())),
    )?;
  }

  if runtime_requirements.contains(RuntimeGlobals::RUSTBOLT_UNIQUE_ID) {
    compilation.add_runtime_module(
      chunk_ukey,
      Box::new(RustboltUniqueIdRuntimeModule::new(
...
impl Plugin for BundlerInfoPlugin {
  ...
  fn apply(
    &self,
    ctx: PluginContext<&mut rustbolt_core::ApplyContext>,
