...
use rustbolt_core::{Compilation, CrossOriginLoading, Mode};
use rustbolt_dojang::{dojang::DojangOptions, Dojang, Operand};
use rustbolt_error::{miette, AnyhowError};
use rustbolt_paths::AssertUtf8;
use serde_json::Value;

use crate::{
  asset::HtmlPluginAssets,
  config::{HtmlRustboltPluginOptions, TemplateParameters},
  tag::HtmlPluginTag,
};
...
impl HtmlTemplate {
    ...
    if let Some(content) = &config.template_content {
      ...
      if config.template_fn.is_some() {
        ...
        fs::read_to_string(&resolved_template)
          .context(format!(
            "HtmlRustboltPlugin: could not load file `{}` from `{}`",
            ...
            Ok(new_data) => match serde_json::from_str(&new_data) {
              ...
              Err(err) => {
                return Err(miette::Error::msg(format!(
                  "HtmlRustboltPlugin: failed to parse template parameters: {err}"
                )));
              }
            },
            Err(err) => {
              return Err(miette::Error::msg(format!(
                "HtmlRustboltPlugin: failed to generate template parameters: {err}"
              )));
            }
          }
        }
  ...
  pub async fn render(
    ...
    match &self.render {
      TemplateRender::Template(content) => {
        ...
        dj.render(&self.url, parameters).map_err(|err| {
          miette::Error::msg(format!(
            "HtmlRustboltPlugin: failed to render template from string: {err}"
          ))
        })
      }
      ...
      .map_err(|err| {
        miette::Error::msg(format!(
          "HtmlRustboltPlugin: failed to render template from function: {err}"
        ))
      }),
    }
  }
