...
use crate::config::HtmlRustboltPluginOptions;

pub struct HtmlCompiler<'a> {
  config: &'a HtmlRustboltPluginOptions,
}

impl<'a> HtmlCompiler<'a> {
  pub fn new(config: &'a HtmlRustboltPluginOptions) -> Self {
    Self { config }
  }
  ...
  pub fn parse_file(&self, path: &str, source: String) -> Result<TWithDiagnosticArray<Document>> {
    ...
    let diagnostics: Vec<rustbolt_error::Diagnostic> = errors
      .into_iter()
...
pub fn html_parse_error_to_traceable_error(error: Error, fm: &SourceFile) -> rustbolt_error::Error {
  let message = error.message();
  let error = error.into_inner();
  let span: ErrorSpan = error.0.into();
  let traceable_error = rustbolt_error::TraceableError::from_source_file(
    fm,
    span.start as usize,
    span.end as usize,
