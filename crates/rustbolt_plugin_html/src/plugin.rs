...
use rustbolt_error::{miette, Diagnostic, Result};
...
use crate::{
  ...
  config::{HtmlInject, HtmlRustboltPluginOptions},
...
#[plugin]
#[derive(Debug)]
pub struct HtmlRustboltPlugin {
  config: HtmlRustboltPluginOptions,
}

impl HtmlRustboltPlugin {
  pub fn new(config: HtmlRustboltPluginOptions) -> Self {
...
async fn generate_html(
  ...
  let html = raw_html.cow_replace("$$RUSTBOLT_URL_AMP$$", "&");
...
#[plugin_hook(CompilationProcessAssets for HtmlRustboltPlugin, stage = Compilation::PROCESS_ASSETS_STAGE_OPTIMIZE_INLINE)]
async fn process_assets(&self, compilation: &mut Compilation) -> Result<()> {
  let config: &HtmlRustboltPluginOptions = &self.config;
  let hooks = HtmlRustboltPlugin::get_compilation_hooks(compilation.id());
...
impl Plugin for HtmlRustboltPlugin {
