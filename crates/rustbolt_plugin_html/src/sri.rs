...
use rustbolt_core::rustbolt_sources::{BoxSource, Source};
use serde::Serialize;
...
pub fn create_digest_from_asset(hash_func: &HtmlSriHashFunction, asset: &BoxSource) -> String {
  ...
  match hash_func {
    HtmlSriHashFunction::Sha384 => {
      ...
      format!("sha384-{}", rustbolt_base64::encode_to_string(digest))
    }
    HtmlSriHashFunction::Sha256 => {
      let mut hasher = Sha256::new();
      hasher.update(byte_content);
      let digest = &hasher.finalize()[..];
      format!("sha256-{}", rustbolt_base64::encode_to_string(digest))
    }
    HtmlSriHashFunction::Sha512 => {
      let mut hasher = Sha512::new();
      hasher.update(byte_content);
      let digest = &hasher.finalize()[..];
      format!("sha512-{}", rustbolt_base64::encode_to_string(digest))
    }
  }
}
