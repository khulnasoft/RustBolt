...
use rustbolt_core::{
  ...
  rustbolt_sources::{BoxSource, ConcatSource, RawStringSource, SourceExt},
  BooleanMatcher, ChunkUkey, Compilation, CrossOriginLoading, RuntimeGlobals, RuntimeModule,
  RuntimeModuleStage,
};
use rustbolt_plugin_runtime::{chunk_has_css, get_chunk_runtime_requirements, stringify_chunks};
...
impl RuntimeModule for CssLoadingRuntimeModule {
  ...
  async fn generate(&self, compilation: &Compilation) -> rustbolt_error::Result<BoxSource> {
    if let Some(chunk_ukey) = self.chunk {
      ...
      // only render chunk without css. See packages/rustbolt/tests/runtimeCases/runtime/split-css-chunk test.
