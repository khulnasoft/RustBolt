...
use rustbolt_core::{
  rustbolt_sources::{ConcatSource, RawStringSource},
...
use rustbolt_error::{
  error, miette::Diagnostic, DiagnosticExt, Result, RustboltSeverity, TraceableError,
};
...
pub const AUTO_PUBLIC_PATH_PLACEHOLDER: &str = "__RUSTBOLT_PLUGIN_CSS_AUTO_PUBLIC_PATH__";
...
impl<'a> LocalIdentOptions<'a> {
  ...
  pub fn get_local_ident(&self, local: &str) -> String {
    ...
    let hash = {
      let mut hasher = RustboltHash::with_salt(&output.hash_function, &output.hash_salt);
...
pub fn css_modules_exports_to_string<'a>(
  ...
  module: &dyn rustbolt_core::Module,
...
pub fn css_parsing_traceable_error(
  ...
  TraceableError::from_arc_string(
    ...
    match severity {
      RustboltSeverity::Error => "CSS parsing error".to_string(),
      RustboltSeverity::Warn => "CSS parsing warning".to_string(),
    },
    message.into(),
  )
