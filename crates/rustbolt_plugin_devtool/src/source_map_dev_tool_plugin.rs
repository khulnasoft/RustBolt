...
use rustbolt_core::{
  rustbolt_sources::{ConcatSource, MapOptions, RawStringSource, Source, SourceExt},
...
use rustbolt_error::{error, miette::IntoDiagnostic, Result};
...
fn create_debug_id(filename: &str, source: &[u8]) -> String {
  ...
  let mut hasher = RustboltHash::new(&rustbolt_hash::HashFunction::Xxhash64);
  hasher.write(source);
      ...
      .map(|(source_filename, source, source_map)| {
        async {
          ...
          if let Some(source_map_filename_config) = &self.source_map_filename {
            ...
            let base64 = rustbolt_base64::encode_to_string(source_map_json.as_bytes());
            asset.source = Some(
              ConcatSource::new([
                source.clone(),
...
async fn process_assets(&self, compilation: &mut Compilation) -> Result<()> {
  let logger = compilation.get_logger("rustbolt.SourceMapDevToolPlugin");
...
impl Plugin for SourceMapDevToolPlugin {
  fn name(&self) -> &'static str {
    "rustbolt.SourceMapDevToolPlugin"
  }
