...
use rustbolt_core::{
  rustbolt_sources::{BoxSource, MapOptions, RawStringSource, Source, SourceExt},
...
use rustbolt_util::identifier::make_paths_absolute;
...
const EVAL_SOURCE_MAP_DEV_TOOL_PLUGIN_NAME: &str = "rustbolt.EvalSourceMapDevToolPlugin";
...
async fn eval_source_map_devtool_plugin_render_module_content(
  ...
  if let Some(cached_source) = module_hash.and_then(|hash| self.cache.get(hash)) {
    ...
    let source = {
      ...
      let base64 = rustbolt_base64::encode_to_string(&map_buffer);
      let footer =
