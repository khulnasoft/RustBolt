use rayon::prelude::*;
use rustbolt_collections::{IdentifierIndexSet, IdentifierSet};
use rustbolt_core::{
...
use rustbolt_util::{comparators::compare_ids, itoa};
...
async fn module_ids(&self, compilation: &mut rustbolt_core::Compilation) -> Result<()> {
  ...
  if compilation
    ...
    && let Some(mutations) = &mutations
  {
    let logger = compilation.get_logger("rustbolt.incremental.moduleIds");
    logger.log(format!(
