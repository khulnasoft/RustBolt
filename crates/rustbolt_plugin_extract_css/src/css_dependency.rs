...
impl Dependency for CssDependency {
  ...
  fn dependency_type(&self) -> &rustbolt_core::DependencyType {
  ...
  // it can keep the right order, but Rustbolt uses HashSet,
  // when determining the postOrderIndex, Rustbolt uses
  // dependency span to set correct order
  fn range(&self) -> Option<&DependencyRange> {
    Some(&self.range)
  }

  fn get_layer(&self) -> Option<&rustbolt_core::ModuleLayer> {
    self.layer.as_ref()
  }
