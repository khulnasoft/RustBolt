...
use regex::Regex;
use rustbolt_cacheable::cacheable;
use rustbolt_collections::{DatabaseItem, IdentifierMap, IdentifierSet, UkeySet};
use rustbolt_core::{
  get_undo_path,
  rustbolt_sources::{
...
use rustbolt_error::{Diagnostic, Result};
...
use rustbolt_plugin_runtime::GetChunkFilenameRuntimeModule;
...
pub static PLUGIN_NAME: &str = "css-extract-rustbolt-plugin";
...
async fn runtime_requirement_in_tree(
  ...
  // different from webpack, Rustbolt can invoke this multiple times,
...
async fn content_hash(
  ...
  chunk_ukey: &ChunkUkey,
  hashes: &mut FxHashMap<SourceType, RustboltHash>,
) -> Result<()> {
  let module_graph = compilation.get_module_graph();
  ...
  let used_modules =
    rustbolt_plugin_css::CssPlugin::get_modules_in_order(chunk, rendered_modules, compilation)
      .0
      .into_iter();

  let mut hasher = hashes
    .entry(SOURCE_TYPE[0])
    .or_insert_with(|| RustboltHash::from(&compilation.options.output));
