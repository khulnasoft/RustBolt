...
use rustbolt_core::{
  impl_module_meta_info, impl_source_map_config, module_update_hash, rustbolt_sources::BoxSource,
...
use rustbolt_util::{ext::DynHash, itoa};
...
impl CssModule {
  ...
  fn compute_hash(&self, options: &CompilerOptions) -> RustboltHashDigest {
    let mut hasher = RustboltHash::from(&options.output);
...
impl Module for CssModule {
  ...
  fn readable_identifier(&self, context: &rustbolt_core::Context) -> std::borrow::Cow<str> {
