...
use rayon::prelude::*;
use rustbolt_collections::{DatabaseItem, IdentifierMap, UkeyMap, UkeySet};
use rustbolt_core::{
...
impl SplitChunksPlugin {
  ...
  pub(crate) async fn prepare_module_group_map(
    ...
    let module_group_results = rustbolt_futures::scope::<_, Result<_>>(|token| {
    ...
    .await
    .into_iter().map(|res| res.map_err(rustbolt_error::miette::Error::from_err))
