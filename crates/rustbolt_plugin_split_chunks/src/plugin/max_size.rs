...
use regex::Regex;
use rustbolt_collections::{DatabaseItem, UkeyMap};
use rustbolt_core::{
...
use rustbolt_util::identifier::make_paths_relative;
...
fn hash_filename(filename: &str, options: &CompilerOptions) -> String {
  let mut filename_hash = RustboltHash::from(&options.output);
  filename.hash(&mut filename_hash);
  let hash_digest: RustboltHashDigest = filename_hash.digest(&options.output.hash_digest);
...
impl SplitChunksPlugin {
  ...
  pub(super) async fn ensure_max_size_fit(
    ...
    let chunks_with_size_info_results = rustbolt_futures::scope::<_, Result<_>>(|token| {
