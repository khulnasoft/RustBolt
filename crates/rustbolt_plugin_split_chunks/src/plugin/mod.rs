...
use rustbolt_core::{ChunkUkey, Compilation, CompilationOptimizeChunks, Logger, Plugin};
...
impl SplitChunksPlugin {
  ...
  async fn inner_impl(&self, compilation: &mut Compilation) -> Result<()> {
    ...
    let start: rustbolt_core::StartTime = logger.time("ensure min size fit");
