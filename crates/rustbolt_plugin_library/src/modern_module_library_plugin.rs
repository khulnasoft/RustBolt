...
use rustbolt_core::{
  merge_runtime,
  rustbolt_sources::{ConcatSource, RawStringSource, SourceExt},
...
use rustbolt_error::{error_bail, Result};
...
const PLUGIN_NAME: &str = "rustbolt.ModernModuleLibraryPlugin";
...
impl ModernModuleLibraryPlugin {
  ...
  async fn optimize_chunk_modules_impl(&self, compilation: &mut Compilation) -> Result<()> {
    let module_graph: rustbolt_core::ModuleGraph = compilation.get_module_graph();
