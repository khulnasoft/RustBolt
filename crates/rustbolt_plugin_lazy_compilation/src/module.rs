...
use rustbolt_cacheable::{cacheable, cacheable_dyn, with::Unsupported};
use rustbolt_collections::Identifiable;
use rustbolt_core::{
  impl_module_meta_info, module_namespace_promise, module_update_hash,
  rustbolt_sources::{BoxSource, RawStringSource},
...
use rustbolt_plugin_javascript::dependency::CommonJsRequireDependency;
use rustbolt_util::{
  ext::DynHash,
  json_stringify,
...
pub(crate) struct LazyCompilationProxyModule {
  ...
  pub active: bool,
  pub data: String,
  /// The client field will be refreshed when rustbolt restart, so this field does not support caching
...
impl DependenciesBlock for LazyCompilationProxyModule {
  fn add_block_id(&mut self, block: rustbolt_core::AsyncDependenciesBlockIdentifier) {
  ...
  fn add_dependency_id(&mut self, dependency: rustbolt_core::DependencyId) {
    self.dependencies.push(dependency);
  }

  fn remove_dependency_id(&mut self, dependency: rustbolt_core::DependencyId) {
