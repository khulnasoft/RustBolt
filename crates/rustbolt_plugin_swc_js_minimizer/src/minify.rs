...
use rustbolt_error::{error, BatchErrors, DiagnosticKind, TraceableError};
use rustbolt_plugin_javascript::{
  ast::{parse_js, print, SourceMapConfig},
  utils::{ecma_parse_error_deduped_to_rustbolt_error, DedupEcmaErrors},
  ExtractedCommentsInfo, IsModule, SourceMapsConfig, TransformOutput,
};
use rustbolt_util::swc::minify_file_comments;
          ...
          .map_err(|errs| {
            BatchErrors(
              errs
                ...
                .into_iter()
                .map(|err| {
                  rustbolt_error::miette::Error::new(ecma_parse_error_deduped_to_rustbolt_error(
                    err,
                    &fm,
                    &ModuleType::JsAuto,
                  ))
...
// keep this private to make sure with_rustbolt_error_handler is safety
struct RustboltErrorEmitter {
  tx: mpsc::Sender<rustbolt_error::Error>,
  source_map: Arc<SourceMap>,
  title: String,
  kind: DiagnosticKind,
}

impl Emitter for RustboltErrorEmitter {
...
pub fn with_rustbolt_error_handler<F, Ret>(
  title: String,
  kind: DiagnosticKind,
  cm: Arc<SourceMap>,
  op: F,
  ...
  F: FnOnce(&Handler) -> std::result::Result<Ret, BatchErrors>,
{
  ...
  let emitter = RustboltErrorEmitter {
    title,
    kind,
    source_map: cm,
    tx,
