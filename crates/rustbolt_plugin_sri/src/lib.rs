...
async fn handle_compilation(
  ...
  if matches!(self.options.html_plugin, IntegrityHtmlPlugin::NativePlugin) {
    let mut html_plugin_hooks = HtmlRustboltPlugin::get_compilation_hooks_mut(compilation.id());
    html_plugin_hooks
...
impl Plugin for SubresourceIntegrityPlugin {
  fn name(&self) -> &'static str {
    "rustbolt.SubresourceIntegrityPlugin"
  }

  fn apply(
    &self,
    ctx: PluginContext<&mut rustbolt_core::ApplyContext>,
    options: &rustbolt_core::CompilerOptions,
  ) -> Result<()> {
