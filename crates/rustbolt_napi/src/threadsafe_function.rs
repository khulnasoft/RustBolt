...
use rustbolt_error::{miette::IntoDiagnostic, Error, Result};
...
impl<T: 'static + JsValuesTupleIntoVec, R> ThreadsafeFunction<T, R> {
  async fn resolve_error(&self, err: napi::Error) -> Error {
    let (tx, rx) = tokio::sync::oneshot::channel::<rustbolt_error::Error>();
    ERROR_RESOLVER
      .get()
      ...
      .call(Box::new(move |env| {
        let err = err.into_rustbolt_error_with_detail(&env);
    ...
    self
      ...
      .call_with_return_value(value, ThreadsafeFunctionCallMode::NonBlocking, {
        move |r: napi::Result<Unknown>, env| {
          let r = match r {
            Err(err) => Err(err.into_rustbolt_error_with_detail(&env)),
