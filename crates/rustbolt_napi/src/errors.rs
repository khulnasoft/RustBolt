...
use rustbolt_error::NodeError;

pub trait NapiErrorExt {
  fn into_rustbolt_error(self) -> rustbolt_error::Error;
  fn into_rustbolt_error_with_detail(self, env: &Env) -> rustbolt_error::Error;
}

pub trait NapiResultExt<T> {
  fn into_rustbolt_result(self) -> rustbolt_error::Result<T>;
  fn into_rustbolt_result_with_detail(self, env: &Env) -> rustbolt_error::Result<T>;
}
...
impl<T: 'static> NapiResultExt<T> for Result<T> {
  fn into_rustbolt_result(self) -> rustbolt_error::Result<T> {
    self.map_err(|e| e.into_rustbolt_error())
  }
  fn into_rustbolt_result_with_detail(self, env: &Env) -> rustbolt_error::Result<T> {
    self.map_err(|e| e.into_rustbolt_error_with_detail(env))
  }
}
