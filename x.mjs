...
program
	.name("Rustbolt Development CLI")
	.description("CLI for development of Rustbolt")
...
buildCommand
	...
	.action(async ({ a, b = a, j = a, r, f }) => {
		...
		try {
			...
			b && (await $`pnpm --filter @rustbolt/binding build:${mode}`);
			j && (await $`pnpm --filter "@rustbolt/*" build ${f ? "--force" : ""}`);
		} catch (e) {
...
watchCommand
	...
	.action(async ({ a, b = a, j = a, r }) => {
		const mode = r ? "release" : "dev";
		try {
			b && (await $`pnpm --filter @rustbolt/binding watch:${mode}`);
			j && (await $`pnpm --filter "@rustbolt/*" watch`);
		} catch (e) {
			process.exit(e.exitCode);
		}
	});
...
buildCommand
	...
	.action(async () => {
		await $`pnpm --filter @rustbolt/binding build:dev`;
	});
...
buildCommand
	...
	.action(async () => {
		await $`pnpm --filter "@rustbolt/*" build`;
	});
...
testCommand
	...
	.action(async () => {
		await $`./x build js`;
		await $`pnpm --filter "@rustbolt/*" test`;
	});
...
extractorCommand
	...
	.action(async () => {
		...
		await $`pnpm --filter "@rustbolt/*" api-extractor --local`;
	});
...
extractorCommand
	...
	.action(async () => {
		try {
			await $`pnpm --filter "@rustbolt/*" api-extractor:ci`;
		} catch (e) {
			console.error(
				`Api-extractor testing failed. Did you forget to update the snapshots locally?
Run the command below locally to fix this error (in the *ROOT* of rustbolt workspace).
...
// x rustbolt / x rs
const rustboltCommand = program.command("rustbolt").alias("rs").description(`
  $ x rustbolt -- [your-rustbolt-cli-args...]
  $ x rustbolt --debug -- build
  $ x rs -d -- build
  $ x rsd -- build
`);
...
rustboltCommand
	...
	.argument("[args...]", "Arguments pass through to rustbolt cli")
	.action(async ({ debug }) => {
		try {
			if (!debug) {
				await $`npx rustbolt ${getVariadicArgs()}`;
				return;
			}
			await launchRustboltCli(getVariadicArgs());
		} catch (e) {
			process.exit(e.exitCode);
		}
	});
...
// x rsd
program
	.command("rustbolt-debug")
	.alias("rsd")
	.description("Alias for `x rustbolt --debug`")
	.argument("[args...]", "Arguments pass through to rustbolt cli")
	.action(async () => {
		await launchRustboltCli(getVariadicArgs());
	});
